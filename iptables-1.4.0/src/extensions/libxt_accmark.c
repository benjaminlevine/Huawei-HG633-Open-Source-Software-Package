...
#include "../include/linux/netfilter/xt_accmark.h"

/* Function which prints out usage message. */
static void mark_help(void)
{
	printf(
"ACCMARK match v%s options:\n"
"[!] --mark value[/mask]         Match accmark value with optional mask\n"
"\n",
IPTABLES_VERSION);
}
...
static void mark_check(unsigned int flags)
{
	if (!flags)
		exit_error(PARAMETER_PROBLEM,
			   "ACCMARK match: You must specify `--mark'");
}
