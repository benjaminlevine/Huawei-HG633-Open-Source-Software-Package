...
#define IP_SPORT  '5'
#define IP_DPORT  '6'
#define IP_FLOWLBL   '7'  // IPv6 Flow Label
/* BEGIN: Added by h00169677, 2013/4/19   PN:DTS2013031909624*/
...
static struct option opts[] =
{
	...
	{ "ip6-flow-label"       , required_argument, 0, IP_FLOWLBL   },
	{ "ip6-flow"             , required_argument, 0, IP_FLOWLBL   },
/* BEGIN: Added by h00169677, 2013/4/19   PN:DTS2013031909624*/
#ifdef BRIDGE_EBT_IP_IPRANGE   	
    { "ip6-srcrange"         , required_argument, 0, IP_SRANGE },
    { "ip6-dstrange"         , required_argument, 0, IP_DRANGE },
#endif	
/* END:   Added by h00169677, 2013/4/19 */
	{ 0 }
};
...
static struct in6_addr *parse_ip6_mask(char *mask)
{
	...
	if (bits != 0) {
		...
		memset(p + ((bits - 1) / 8) + 1, 0, (128 - bits) / 8);
...
"--ip6-flow   [!] flowlabel     : ipv6 flow label specification\n"
"--ip6-proto  [!] protocol      : ipv6 protocol specification\n"
"--ip6-sport  [!] port[:port]   : tcp/udp source port or port range\n"
"--ip6-dport  [!] port[:port]   : tcp/udp destination port or port range\n"
/* BEGIN: Added by h00169677, 2013/4/19   PN:DTS2013031909624*/
#ifdef BRIDGE_EBT_IP_IPRANGE   
"--ip6-srcrange   ip-ip         : ip source range\n"
"--ip6-dstrange   ip-ip         : ip destination range\n"
#endif
/* END:   Added by h00169677, 2013/4/19 */
);
}
...
/* END:   Added by h00169677, 2013/4/19 */
	...
	case IP_FLOWLBL:
		check_option(flags, OPT_FLOWLBL);
		if (check_inverse(optarg))
			ipinfo->invflags |= EBT_IP6_FLOWLBL;
		i = strtol(optarg, &end, 16);
		if (i < 0 || i > 0x100000 || *end != '\0')
			print_error("Problem with specified IPv6 flow label");
		ipinfo->flowlbl = i;
		ipinfo->bitmask |= EBT_IP6_FLOWLBL;
		break;
...
/* BEGIN: Added by h00169677, 2013/4/19   PN:DTS2013031909624*/
    ...
    case IP_SRANGE:
        /* IPV6 src process */
        check_option(flags, OPT_SRANGE);
		end = strchr(optarg, '-');
		if (end == NULL)
            print_error("iprange match: Bad format for --src-range  `%s'", optarg);
        
        *end = '\0';
		ia = numeric_to_addr(optarg);
        if (ia == NULL)
          print_error("iprange match: Bad IPv6 address `%s'", optarg);
        memcpy(&ipinfo->ip6range_src.min_ip6, ia, sizeof(*ia));

        ia = numeric_to_addr(end+1);
		if (ia == NULL)
			print_error("iprange match: Bad IPv6 address `%s'", end+1);
		memcpy(&ipinfo->ip6range_src.max_ip6, ia, sizeof(*ia));
      
        ipinfo->bitmask |= EBT_IP6_SRANGE;
        break;
    case IP_DRANGE:
        /* IPV6 dest process */
        check_option(flags, OPT_DRANGE);
		end = strchr(optarg, '-');
		if (end == NULL)
            print_error("iprange match: Bad format for --dst-range  `%s'", optarg);
        
        *end = '\0';
		ia = numeric_to_addr(optarg);
        if (ia == NULL)
          print_error("iprange match: Bad IPv6 address `%s'", optarg);
        memcpy(&ipinfo->ip6range_dst.min_ip6, ia, sizeof(*ia));
        
        ia = numeric_to_addr(end+1);
		if (ia == NULL)
			print_error("iprange match: Bad IPv6 address `%s'", end+1);
		memcpy(&ipinfo->ip6range_dst.max_ip6, ia, sizeof(*ia));

        ipinfo->bitmask |= EBT_IP6_DRANGE;
        break;
#endif
/* END:   Added by h00169677, 2013/4/19 */
	default:
   ...
   const struct ebt_entry_match *match)
{
	...
	if (ipinfo->bitmask & EBT_IP6_FLOWLBL) {
		printf("--ip6-flow ");
		if (ipinfo->invflags & EBT_IP6_FLOWLBL)
			printf("! ");
		printf("0x%06X ", ipinfo->flowlbl);
	}
...
/* BEGIN: Added by h00169677, 2013/4/19   PN:DTS2013031909624*/
#ifdef BRIDGE_EBT_IP_IPRANGE 
    if (ipinfo->bitmask & EBT_IP6_SRANGE){
		printf("--ip6-srcrange ");
		if (ipinfo->invflags & EBT_IP6_SRANGE) {
			printf("! ");
		}
		printf("%s", ebt_ip6_to_numeric(&(ipinfo->ip6range_src.min_ip6)));
		printf("-");
		printf("%s ", ebt_ip6_to_numeric(&(ipinfo->ip6range_src.max_ip6)));

	}
    
    if (ipinfo->bitmask & EBT_IP6_DRANGE){
		printf("--ip6-dstrange ");
		if (ipinfo->invflags & EBT_IP6_DRANGE) {
			printf("! ");
		}
		printf("%s", ebt_ip6_to_numeric(&(ipinfo->ip6range_dst.min_ip6)));
		printf("-");
		printf("%s ", ebt_ip6_to_numeric(&(ipinfo->ip6range_dst.max_ip6)));
	}
#endif
/* END:   Added by h00169677, 2013/4/19 */
}
   ...
   const struct ebt_entry_match *m2)
{
	...
	if (ipinfo1->bitmask & EBT_IP6_FLOWLBL) {
		if (ipinfo1->flowlbl != ipinfo2->flowlbl)
			return 0;
	}
