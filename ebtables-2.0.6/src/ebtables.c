...
void print_mac_and_mask(const unsigned char *mac, const unsigned char *mask)
{
...
{
	if ( !(table = find_table(replace.name)) )      /* 默认是filter表 */
...
/* BRCM end */
{
	...
	struct ebt_u_entries *entries;
        ...
        /*start: 用于查询组件版本号(ebtables atpv),请产品不要修改或删除*/
    ...
    pfLockFile = ATP_UTIL_LockProc(EBTABLES_LOCK_FILE);
    if (NULL == pfLockFile) {
        printf("ebtables get lock failed\n");
        exit(-1);
    }
	...
	while ((c = getopt_long(argc, argv,
	   "-A:D:I:N:E:X::L::Z::F::P:Vhi:o:j:p:s:d:t:M:", ebt_options, NULL)) != -1) {
		...
		case 'd': /* destination mac */
			...
			i = strtol(argv[optind - 1], &buffer, 16);          /* 协议可以按照数字指定 */
