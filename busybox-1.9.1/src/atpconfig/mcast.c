                                ...
                                int mode)
{
    ...
    byteBuf = (unsigned char*)pucSrcBuf;
    for (idx = 0; idx < ulSrcLen; idx++) {
        ulCrc = (ulCrc >> 8) ^ m_aulCrc32Table[ (ulCrc ^ byteBuf[idx]) & 0xFF ];
    }
...
#if ENABLE_FEATURE_ATP_MCAST_USPACE

unsigned char* g_pucMcastBitMap =  NULL; 
unsigned long  g_ulMcastBitMapSize = 0;
unsigned int   g_uMcastBitInitial = 0;
unsigned int   ulSaveSeqNum  = 0;
...
static int atp_detect_ip_collision(struct ip_collision_detector *detect)
{
    ...
    // Setup signals
    {	
    	//should first unblock SIGALRM
		sigset_t childmask, savemask;
		struct sigaction sa;
		sigemptyset(&childmask);
		sigaddset(&childmask, SIGALRM);
		sigprocmask(SIG_UNBLOCK, &childmask, &savemask);
 ...
 * 输出参数:   无
 *
 * 返 回 值:   0:  第 nr 个 bit为0
               1:  第 nr 个 bit为1
 
 * 作   者 :   d00167548
 * 修改历史:   2010-12-01代码移植 
 ***************************************************************************/
static inline unsigned int test_bit(unsigned int nr, unsigned char * p)
{
    return (p[nr >> 3] >> (nr & 7)) & 1UL;
}
 ...
 * 修改历史:   2010-12-01代码移植  
 ***************************************************************************/
static inline void set_bit(unsigned int nr, unsigned char * addr)
{
    unsigned char * m = ((unsigned char *) addr) + (nr >> 3);
                                             ...
                                             const char *localFile)
{
    ...
    ulCalcCrc = upg_calc_crc((pcBuf + 8), (len - 8), 0);
	...
	if (0 == g_uMcastBitInitial)
	{
		printf("IMG length %d %d\n", ulTmpInt, g_ulTotalLen);
		g_ulMcastBitMapSize = ((ulTmpInt / MCAST_PACKET_LEN) >> 3) + 1;
		g_pucMcastBitMap  =(unsigned char *) malloc(g_ulMcastBitMapSize);

		if (NULL == g_pucMcastBitMap)
		{			 
			printf("Malloc McastBitMap fails\n");
			return 0;
		}

		memset(g_pucMcastBitMap, 0, g_ulMcastBitMapSize); 
		g_uMcastBitInitial = 1;
	}
	...
	ulSaveSeqNum = ulCurSeqNum;
	//Check already store
	if (test_bit(ulCurSeqNum, g_pucMcastBitMap))
	{	
		return 1;
	}
    ...
    // Check seq num
	ulCurSeqNum = MCAST_PACKET_LEN * ulCurSeqNum;
	if ((ulCurSeqNum + len) > g_ulTotalLen) {
		DBG_PRINT("Seq num %d too large\n", ulCurSeqNum);
		return 1;
	}
	DBG_PRINT("Store seq num %d len: %d total len: %d\n",
	  			ulCurSeqNum, len, g_lMcastLen);

	memcpy((void *)(g_pucMcastBuf + ulCurSeqNum),
	(void *)pstPkg, len);

	set_bit(ulSaveSeqNum, g_pucMcastBitMap);
...
#if ENABLE_FEATURE_ATP_MCAST_BOOTLOADER
#define BIG_ENDIAN_INT_TO_HOST(x, buf) {(x) = (buf)[3]; (x) = ((x) << 8); (x) += (buf)[2]; (x) = ((x) << 8); (x) += (buf)[1]; (x) = ((x) << 8); (x) += (buf)[0];}

#define BOOTLOADER_MCAST_HEADER_LEN_D       (62)
static int mcast_bootloader_detect_proc(char *pcBuf, unsigned int len)
{
    VOS_UINT32          ulFlag;
    VOS_UINT32          ulCrc;
    VOS_UINT32          ulCalcCrc;
    FILE                *pfFlag;

    if (len < BOOTLOADER_MCAST_HEADER_LEN_D)
    {
        DBG_PRINT("Packet too short %d!\n", len);
        return -1;
    }

    // Get Flag
    BIG_ENDIAN_INT_TO_HOST(ulFlag, pcBuf);
    // Get CRC
    BIG_ENDIAN_INT_TO_HOST(ulCrc, pcBuf + 8);

    len -= BOOTLOADER_MCAST_HEADER_LEN_D;
    if ((ulFlag & 0x1FFF) != len)
    {
        DBG_PRINT("Length (%x) not matched with %x!\n", ulFlag, len);
        return -1;
    }

    pcBuf += BOOTLOADER_MCAST_HEADER_LEN_D;
    DBG_PRINT("[%x %x %x %x].\n", pcBuf[0], pcBuf[1], pcBuf[2], pcBuf[3]);
    ulCalcCrc = upg_calc_crc(pcBuf, len, 2);
    if (ulCrc != ulCalcCrc)
    {
        DBG_PRINT("Crc (%x) not matched with %x, length %d!\n", ulCrc, ulCalcCrc, len);
        return -1;
    }

    DBG_PRINT("Bootloader multicast upgrade detected.\n");
    pfFlag = fopen("/var/bootloadermcast", "wb");
    if (NULL != pfFlag)
    {
        fclose(pfFlag);
    }
...
static void mcast_listen(int fd, const char *localFile,
                           const char *pcRemoteIp, int port, int mode, int lTimeout)
{
    fd_set                      hReadset;
    ...
    while (1)
    {
    ...
    #if ENABLE_FEATURE_ATP_MCAST_BOOTLOADER
        if (2 == mode) {
            lAddrLen = mcast_bootloader_detect_proc(acRecvBuf, (unsigned int)lBytesRecvd);
        }
    #endif
        ...
        if(xfunc_error_retval == ATP_TRANS_OK)
        {
            break;
        }
    }
...
int mcast_main(int argc, char **argv) MAIN_EXTERNALLY_VISIBLE;
int mcast_main(int argc, char **argv)
{
    ...
    opt = getopt32(argv, "gsdu:p:r:l:P:B:A:b:e:m:"ZIP_OPT_CHAR"zf:t:",
                         &dummy,        // Username
                         &dummy,        // Password
                         &dummy,        // Remote path
...
#undef ZIP_OPT_CHAR
	...
	} else if (opt & TRANS_TOOL_OPT_ZIP) {      // -z, for bootloader multicast detection
		mode = 2;       // Download from sub channel
		unlink("/var/bootloadermcast");
	}

    if (((NULL == localFile) && (2 != mode)) ||
        (NULL == argv[optind]))
        bb_show_usage();
    ...
    if (NULL != pcTimeout)
    {
        lTimeout = (int)atoi(pcTimeout);
    }
    mcast_listen(fd, localFile, argv[optind], opt, mode, lTimeout);
    if (NULL != g_pucMcastBuf) {
        munmap((caddr_t)g_pucMcastBuf, g_ulTotalLen);
        g_pucMcastBuf = NULL;
    }
