...
#include "atpconfig.h"
#ifdef SUPPORT_ATP_COMPONENT_REALTEK_WLAN_CHIP
...
RB_AUTOBOOT
	...
	for (i = 0; i < 4; i++)  {
		if (i == 0) {
            sprintf(cmd, "ifconfig %s down", ATP_WL0);     
			system(cmd);
            sprintf(cmd, "ifconfig %s down", ATP_WL1);     
			system(cmd);
		}
		else {
			sprintf(cmd, "ifconfig %s.%d down", ATP_WL0, i);     
			system(cmd);
            sprintf(cmd, "ifconfig %s.%d down", ATP_WL1, i);     
			system(cmd);
		}
	}
#endif
