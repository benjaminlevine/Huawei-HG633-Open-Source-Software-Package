				    ...
				    struct auth_serversupplied_info **server_info)
{
	...
	for (auth_method = auth_context->auth_method_list;auth_method; auth_method = auth_method->next) {
		...
		talloc_destroy(mem_ctx);
		/*BEGIN DTS2013090900631 h00250612 2014-11-29 added 防暴力破解,copy from euap samba*/
        DEBUG(10, ("[%d] max visit times [%d], retry sleep seconds [%d]\n", 
                sys_getpid(), lp_max_visit_times(), lp_retry_sleep_seconds()));
        if(lp_max_visit_times() > 0)
        {
            if (NT_STATUS_IS_OK(nt_status)
             || NT_STATUS_EQUAL(nt_status, NT_STATUS_WRONG_PASSWORD))
            {
                if(strlen(user_info->smb_name) > 0)
                {
                    /* Get MAC address by IP address on br0 */
                    iGetMacStatus = GetMac(LAN_DEVICE,  client_addr(), macAddr);
                    if(0 == iGetMacStatus)
                    {
                        memset(&newVisitor, 0, sizeof(newVisitor));
                        strncpy(newVisitor.name, user_info->smb_name, sizeof(newVisitor.name));
                        strncpy(newVisitor.mac, macAddr, sizeof(newVisitor.mac));
                        
                        SendMsgToParent(newVisitor, MSG_SMB_VISITOR_UPDATE);

                        for(msgLoop = 0; msgLoop < MSG_RSV_LOOP_TIMES; msgLoop++)
                        {
                            DEBUG(10, ("[%d] Receive message from parent [%d] times\n", sys_getpid(), msgLoop + 1));
                            
                            /* Receive message from parent */
                            message_dispatch();
                            
                            /* Check the visitor */
                            if(IsDenyVisitor(newVisitor))
                            {
                                return NT_STATUS_ACCESS_DENIED;
                            }
                        
                            usleep(MSG_RSV_USLEEP_TIME);
                        }

                        if(NT_STATUS_IS_OK(nt_status))
                        {
                            /* If successful authentication, remove the visitor node from visitor link. */
                            SendMsgToParent(newVisitor, MSG_SMB_VISITOR_REMOVE);
                        }
                    }
                }
            }
