...
bool interpret_string_addr(struct sockaddr_storage *pss, const char *str, int flags);
...
int open_socket_ipv6_in(int type,
		uint16_t port,
		int dlevel,
...
/*Start: added by z81004729, 2012-6-6 PN: UDK0000221758 PC 访问网关smb 服务器时,systemlog 应该记录成功具体是哪台PC Ipaddress 访问 */
       int n = 1; 
/*End: added by z81004729, 2012-6-6 PN: UDK0000221758 PC 访问网关smb 服务器时,systemlog 应该记录成功具体是哪台PC Ipaddress 访问 */
...
/*Start: added by z81004729, 2012-6-6 PN: UDK0000221758 PC 访问网关smb 服务器时,systemlog 应该记录成功具体是哪台PC Ipaddress 访问 */
        if (sock.ss_family == AF_INET6)
        {
            if( setsockopt(res, IPPROTO_IPV6, IPV6_V6ONLY, (char*)&n, sizeof(n)) == -1 ) {
                
                printf("setsockopt IPV6_V6ONLY failed[%i:%s].\n", errno, strerror(errno));
            }
        }
 /*End: added by z81004729, 2012-6-6 PN: UDK0000221758 PC 访问网关smb 服务器时,systemlog 应该记录成功具体是哪台PC Ipaddress 访问 */       
...
/* Modified by y00183935@2013.01.08 for CVE-2011-0719 */
#if 0
		if (sockets[i] == -1)
			continue;
#else
...
char *get_peer_addr(int fd)
{
	...
	if (getpeername(fd, &sa, &length) < 0) {
        /*Modified by lvxin00135113@20200506 for samba log*/
        #if 0
		DEBUG(0,("getpeername failed. Error was %s\n", strerror(errno) ));
        #endif
        /*Modified by lvxin00135113@20200506 for samba log*/
        ...
        /*modify by y00181549 for get ipv6 client address,at 2012.8.9*/
        length = sizeof(addr6);
        if ((AF_INET6 == sa.sa_family) && (0 == getpeername(fd, (struct sockaddr *) &addr6, &length)))
        {
            snprintf(addr_buf,sizeof(addr_buf),"%s",inet_ntop(AF_INET6,&addr6.sin6_addr, acAddr, sizeof(acAddr)));
        }
        else
        {
            fstrcpy(addr_buf,(char *)inet_ntoa(sockin->sin_addr));
...
/*BEGIN DTS2013090900631 h00250612 2014-11-29 added 防暴力破解,copy from euap samba*/
int GetMac(char *pDevice, char *pIpAddr, char *pMacAddr)
{
    int sockfd = 0;
    int ret = 0;
    char *ptr;
