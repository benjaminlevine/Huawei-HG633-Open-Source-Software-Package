...
#define SMBPASSWD_FILE		"/etc/samba/smbpasswd"
...
char *encode_acct_ctrl(uint32 acct_ctrl, size_t length)
{
    ...
    palloc = talloc_tos();
	result = talloc_strdup(palloc, acct_str);
...
static NTSTATUS add_smbpwd_entry( struct smb_passwd *newpwd)
{
	const char *pfile = SMBPASSWD_FILE;
	FILE *fp = NULL;
	int wr_len;
	int fd;
...
static NTSTATUS smbpasswd_add_account(struct smb_passwd *smb_pw)
{
/*	struct smbpasswd_privates *smbpasswd_state = (struct smbpasswd_privates*)my_methods->private_data;
	...
	/* add the entry */
	return add_smbpwd_entry(smb_pw);
}
...
/*	TALLOC_FREE( pwd );*/
	...
	DebugPrintLine(2);
	if (NT_STATUS_IS_OK(smbpasswd_add_account(&smbpwd))) 
    {
