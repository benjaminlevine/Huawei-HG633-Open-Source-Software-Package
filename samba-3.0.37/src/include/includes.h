...
/*defined by qichenghong 20091014 for debugging*/
//#define DEBUG_
/*changed to 2 by qichenghong 20091019*/
/*n 用来指定哪些需要打印调试的文件行信息*/
#ifdef DEBUG_
#define DebugPrintLine(n)	   do{if(n==6)  printf("\n==file:%s,line:%d,func:%s,pid:%d==\n",__FILE__,__LINE__,__FUNCTION__,sys_getpid());}while(0)	
/*defined by qichenghong 20091014 for debugging end*/

#define DebugPrintStr(n,s)	do{if((n==6) && (s)) printf("File:%s,line:%d,func:%s: %s=%s \n",__FILE__,__LINE__,__FUNCTION__,#s , s);}while(0)
#define DebugPrintInt(n,i)	       do{if(n==6) printf("File:%s,line:%d,func:%s:%s=%Ld \n",__FILE__,__LINE__,__FUNCTION__,#i,(i));}while(0)
#define DebugPrint(format, args...) do{printf("FILE: %s FUNC: %s LINE: %d:\n", __FILE__,__FUNCTION__, __LINE__);printf(format, ##args);}while(0)
#else
#define DebugPrintLine(n)	  
#define DebugPrintStr(n,s)	 
#define DebugPrintInt(n,i)	  
#define DebugPrint(format, args...)
#endif
/*defined by qichenghong 20091019 for cutting some codes of smbpasswd */
#define CUT_SMBPASSWD	1

#ifndef SUPPORT_ATP_SAMBA_PRINT
/*defined by qichenghong 20091029 for cutting printing*/
#define NOT_SUPPORT_PRINTING	1
#endif
/*defined by qichenghong 20091029 for cutting build options*/
#define NOT_HAVE_BUILDOPTIONS		1

/*defined by qichenghong 20091029 for cutting popt */
#define NOT_HAVE_POPT		1

//modify by y00181549 samba登陆控制宏，连续登陆失败5次之后则禁止60分钟登陆
#ifdef SUPPORT_ATP_SAMBA_LOGIN_CONTROL
#define SUPPORT_LOGIN_CONTROL
#endif

/*defined by qichenghong 20100106 for cutting RPC*/
#define CUT_RPC	1
#define NOT_CUT 1
...
#define CUT_WKSSVC  //defined by qichenghong 20100108

#define CUT_SRVSVC  //defined by qichenghong 20100108
#endif
//defined by qichenghong 20100106 for cutting security = domain
#define CUT_SECURITY_DOMAIN

#include "lib/replace/replace.h"

#define CONFIGFILE             "/etc/samba/smb.conf"
...
/*BEGIN DTS2013090900631 h00250612 2014-11-29 added 防暴力破解,copy from euap samba*/
#define LAN_DEVICE                 "br0"
#define MAC_ADDRESS_LEN            20
#define MAX_VISITOR_CNT            20

#define MSG_RSV_LOOP_TIMES         10
#define MSG_RSV_USLEEP_TIME        1000

typedef struct _visitor_info {
    fstring name;
    char mac[MAC_ADDRESS_LEN];
    struct timespec visit_time;
    int visit_times;
    struct _visitor_info *next;
}VISITOR_INFO;
/*END   DTS2013090900631 h00250612 2014-11-29 added 防暴力破解,copy from euap samba*/
/*
...
/*Start of modified by lvxin 00135113@20090525 for AU4D01765 samba user delete*/
/*define the max length of smbd buffer*/
#define SMBD_MAX_BUFFER_LEN  1024
#define SMBD_USER_PID_CMD_LEN  256

#define SMBD_USER_PID_ADD      1000
#define SMBD_USER_PID_DEL       1001
#define SMBD_USER_PID_FILE   "var/smbd_user_pid"
/*End of modified by lvxin 00135113@20090525 for AU4D01765 samba user delete*/

/*Modified by lvxin00135113@20110324 for samba ipv6*/
