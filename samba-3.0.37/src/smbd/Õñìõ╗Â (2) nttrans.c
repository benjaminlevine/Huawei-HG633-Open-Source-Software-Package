...
char dirpath[400] = {0}; //added by qichenghong 20100111
...
static const char *known_nt_pipes[] = {
	...
	"\\ntlsa",
	"\\ntsvcs",
#if 0   //deleted by qichenghong 20100304 for DT LSASS security
	"\\lsass",
#endif
	"\\lsarpc",
	"\\winreg",
...
{  
	...
	NTSTATUS status;
	SMB_BIG_UINT bsize, dfree, dsize , dfree_ret;
    	char *dpath = NULL ;
	START_PROFILE(SMBntcreateX);
printf("\n=file:%s,line:%d,func:%s=fname=%s=\n",__FILE__,__LINE__,__FUNCTION__,fname);
	if(fname && fname[0] != '\0')
      {
          char * p = strchr_m(fname , '/');
          memset(dirpath , 0 , sizeof(dirpath));
          if(p != NULL)
          {
              StrnCpy_fn(NULL , 0 ,  dirpath , fname , p - fname);
          }
          else
          {
              StrnCpy_fn(NULL , 0 , dirpath , fname , sizeof(dirpath) - 1);
          }
      }
	else
	{
  ...
  printf("\n=file:%s,line:%d,func:%s=dirpath=%s,create_disposition=%d=\n",__FILE__,__LINE__,__FUNCTION__,dirpath,create_disposition);
    #if 1
    //added by qichenghong 20100205 for 解决内存拷贝的问题
	if((FILE_CREATE == create_disposition) || (FILE_OVERWRITE_IF == create_disposition))
      {
      printf("\n=file:%s,line:%d,func:%s=conn->connectpath=%s=\n",__FILE__,__LINE__,__FUNCTION__,conn->connectpath);
            become_root();
            dfree_ret = SMB_VFS_DISK_FREE(conn,conn->connectpath,False,&bsize,&dfree,&dsize);
            unbecome_root();
            DebugPrintInt(5, dfree_ret);
            if(((SMB_BIG_UINT)0 == dfree_ret) || ((SMB_BIG_UINT)-1 == dfree_ret))
            {        
            printf("\n=file:%s,line:%d,func:%s=dpath=%s=\n",__FILE__,__LINE__,__FUNCTION__,dpath);
                  string_append(&dpath , conn->connectpath);
                  if(dpath)
                  {              
                  printf("\n=file:%s,line:%d,func:%s==\n",__FILE__,__LINE__,__FUNCTION__);
                      ...
                      {
                      #endif
                      printf("\n=file:%s,line:%d,func:%s=dirpath=%s=\n",__FILE__,__LINE__,__FUNCTION__,dirpath);
                    
                      string_append(&dpath , dirpath);
                      //}
                      printf("\n=file:%s,line:%d,func:%s=dpath=%s=\n",__FILE__,__LINE__,__FUNCTION__,dpath);
                      DebugPrint("dpath=%s\n", dpath);
                      if(dpath)
                      ...
                      printf("\n=file:%s,line:%d,func:%s==\n",__FILE__,__LINE__,__FUNCTION__);
                            become_root();
                            dfree_ret = SMB_VFS_DISK_FREE(conn,dpath,False,&bsize,&dfree,&dsize);
                            unbecome_root();
                            DebugPrintInt(5, dfree_ret);
                            free(dpath);
                            printf("\n=file:%s,line:%d,func:%s=dfree_ret=%llu=\n",__FILE__,__LINE__,__FUNCTION__,dfree_ret);
                            if(((SMB_BIG_UINT)0 == dfree_ret) || ((SMB_BIG_UINT)-1 == dfree_ret))
                            {                        
                            printf("\n=file:%s,line:%d,func:%s==\n",__FILE__,__LINE__,__FUNCTION__);
                                //reply_nterror(req, NT_STATUS_DISK_OPERATION_FAILED);
                                return ERROR_NT(NT_STATUS_DISK_OPERATION_FAILED);
                                //END_PROFILE(SMBntcreateX);
                                //return;
                            }
                      }
      ...
      //added by qichenghong 20100205 for 解决内存拷贝的问题end
      #endif
printf("\n=file:%s,line:%d,func:%s==\n",__FILE__,__LINE__,__FUNCTION__);
...
printf("\n=file:%s,line:%d,func:%s=fname=%s=\n",__FILE__,__LINE__,__FUNCTION__,fname);
	...
	printf("\n=ra_type=%d,line:%d=inbuf=%s=fname=%s=\n",ra_type,__LINE__,inbuf,fname);
		...
		srvstr_get_path(inbuf, fname, smb_buf(inbuf), sizeof(fname), 0, STR_TERMINATE, &status);
        printf("\n=file:%s,line:%d,func:%s=fname=%s,status=0x%x=\n",__FILE__,__LINE__,__FUNCTION__,fname,status);
		if (!NT_STATUS_IS_OK(status)) {
			END_PROFILE(SMBntcreateX);
			return ERROR_NT(status);
		}
...
printf("\n=file:%s,line:%d,func:%s=fname=%s=\n",__FILE__,__LINE__,__FUNCTION__,fname);
    ...
    printf("\n=file:%s,line:%d,=fname=%s,status=0x%x=\n",__FILE__,__LINE__,fname,status);
