...
{  
	...
	if(root_dir_fid != 0) {
        ...
        if((FILE_CREATE == create_disposition) || (FILE_OVERWRITE_IF == create_disposition))
        {
            ...
            if( '/' == fname[0] )
            {
                snprintf( tmpfilepath, sizeof( tmpfilepath ), "%s", fname );
            }
            else if( '/' == conn->connectpath[0] )
            {
                snprintf( tmpfilepath, sizeof( tmpfilepath ), "%s/%s", conn->connectpath, fname );
            }

            DEBUG ( 0, ( "Check whether write into memery: tmpfilepath:%s \n", tmpfilepath ) );
            DEBUG ( 0, ( "fname:%s conn->connectpath:%s \n", fname, conn->connectpath ) );

            while( strlen(tmpfilepath) > 0 )
            {
                if( ( access( tmpfilepath, F_OK ) ) != 0 )
                {
                    pTmp = strrchr( tmpfilepath, '/' );
                    if( pTmp != NULL )
                    {
                        *pTmp = '\0';
                    }
                ...
                else
                {
                    ...
                    statfsret = statfs( tmpfilepath, &statfsbuf );
                    if( statfsret >= 0 && ((statfsbuf.f_type) != 0x01021994) && ((statfsbuf.f_type) != 0x7275) && ((statfsbuf.f_type) != 0x858458f6))//tmpfs and romfs
                    {
                        diskposition = 1;
                        break;
                    }
...
static struct ea_list *read_nttrans_ea_list(TALLOC_CTX *ctx, const char *pdata, size_t data_size)
{
	...
	while (offset + 4 <= data_size) {
        ...
        /* Modified by y00183935@2013.09.17 for CVE-2013-4124 */
				  ...
				  char **ppdata, uint32 data_count, uint32 max_data_count)
{
	...
	case FSCTL_FIND_FILES_BY_SID: /* I hope this name is right */
	{
		...
		/*unknown = IVAL(pdata,0);*/
                ...
                /*Modifiled by lvxin00135113@20110316 for CVE-2010-3069*/
                #if 0
