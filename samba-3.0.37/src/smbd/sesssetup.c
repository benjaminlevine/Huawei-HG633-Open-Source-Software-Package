...
#ifdef SUPPORT_LOGIN_CONTROL //added by qichenghong 2009.12.03
#include "loginctl.h"
#include "atplog.h"
extern unsigned char g_ucLoginCtlFlag;
extern char    g_acUsername[256 + 1];
extern pid_t   g_CurPid ;
static time_t  g_tLasttime = 0;
#endif
#ifdef SUPPORT_ATP_SAMBA_GUEST//add by y00181549 for samba guest 账户，用户名"guest"，密码任意字符
extern BOOL there_is_guest_account;
#endif
...
#ifdef SUPPORT_ATP_SAMBA_GUEST //add by y00181549 for samba guest 账户，用户名"guest"，密码任意字符
              if((True == there_is_guest_account) && (NULL != user)&&(!(strcmp("guest",user))))
#else
		if (lp_map_to_guest() == MAP_TO_GUEST_ON_BAD_PASSWORD) 
#endif
              {
#ifdef SUPPORT_LOGIN_CONTROL  //added by qichenghong 2009.12.03 
                     ATP_LOG_LogItemPrint(ATP_SYS_LOG_UMOUNT_USER_LOGIN_INFO,user,client_addr() ,"SMB");
#endif
...
#ifdef SUPPORT_ATP_NAS
static unsigned int  NasSambaLoginFailSendEmail(char *username,char *addr)
{
    ...
    snprintf(stUsbVolumeResult.acLoginFailReason, sizeof(stUsbVolumeResult.acLoginFailReason), "User %s, %s login  failed with SMB", username,addr);
			     ...
			     AUTH_NTLMSSP_STATE **auth_ntlmssp_state)
{
	...
	status = auth_ntlmssp_update(*auth_ntlmssp_state,
					auth, &auth_reply);
#ifdef SUPPORT_LOGIN_CONTROL  //added by qichenghong 2009.12.03 
	if(g_ucLoginCtlFlag == 1)
	{
              time_t  tCurtime = 0;
		if(1)//!strcmp((*auth_ntlmssp_state)->ntlmssp_state->domain , get_global_sam_name()))//如果域名相同则进行检测
		{
			long lRet;
			char client_add[INET6_ADDRSTRLEN];
                   g_CurPid = sys_getpid();
                   snprintf(g_acUsername ,sizeof(g_acUsername),"%s",(*auth_ntlmssp_state)->ntlmssp_state->user);
                   if (('\0' != g_acUsername[0]) && ( 0 != strcmp((*auth_ntlmssp_state)->ntlmssp_state->user,"guest")))//guest 的登陆失败不记录
                    {
        			if(NT_STATUS_IS_OK(status))
        			{
                                    lRet = ATP_LoginCtlCheck((*auth_ntlmssp_state)->ntlmssp_state->user ,client_addr() , 
                                    ATP_LOGINTYPE_SAMBA , LOGIN_OK);
                                    /*start of add by y00181549 for W724v log at 20120718*/
                                    if(ATP_LOGINRET_OK == lRet)
                                    {
                                         ATP_LOG_LogItemPrint(ATP_SYS_LOG_UMOUNT_USER_LOGIN_INFO,(*auth_ntlmssp_state)->ntlmssp_state->user,client_addr() ,"SMB");
                                    }
                                    /*end of add by y00181549 for W724v log at 20120718*/
                                    if(lRet != ATP_LOGINRET_OK)
                                    {            
                                        status = NT_STATUS_LOGON_FAILURE;
                                        ATP_LOG_LogItemPrint(ATP_SYS_LOG_UMOUNT_LOCAL_USER_FAIL_LOGIN_5_TIMES,(*auth_ntlmssp_state)->ntlmssp_state->user);
                                    }
        			}
        			else 
        			{
                                    time(&tCurtime);    
                                    lRet = ATP_LoginCtlCheck((*auth_ntlmssp_state)->ntlmssp_state->user , client_addr() , ATP_LOGINTYPE_SAMBA , LOGIN_FAIL);
                                    if ( 1 < (tCurtime - g_tLasttime) && (VOS_OK == lRet))//避免windows系统1s之内的连续登陆,造成大量的log和email的情况 
                                    {
                                        
                                        g_tLasttime = tCurtime;
                                        /*start of add by y00181549 for W724v log at 20120718*/
                                        ATP_LOG_LogItemPrint(ATP_SYS_LOG_UMOUNT_USER_LOGIN_FAIL_INFO,(*auth_ntlmssp_state)->ntlmssp_state->user,client_addr(),"SMB");
                                    }
#ifdef SUPPORT_ATP_NAS
                                    if ((NT_STATUS_WRONG_PASSWORD == status) && (VOS_OK == lRet))
                                    {
                                        if (0 == NasSambaLoginFailSendEmail((*auth_ntlmssp_state)->ntlmssp_state->user,client_addr()))
                                        {
                                            printf("Nas send samba login fail email fail\r\n");
                                        }
                                    }
#endif
        			}
...
/* Modified by y00183935@2013.01.08 for CVE-2010-1635-1642 */
#if 0
	p2 = inbuf + smb_vwv13 + data_blob_len;
#else
    p2 = inbuf + smb_vwv13 + blob1.length;
#endif
/* Modified by y00183935@2013.01.08 for CVE-2010-1635-1642 */
