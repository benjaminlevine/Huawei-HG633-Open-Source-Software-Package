...
BOOL change_to_user(connection_struct *conn, uint16 vuid)
{
    ...
    /*add by y00181549 for fix bug that folder's permissions can not be controled*/
/*#if !defined (SUPPORT_ATP_SAMBA_SHARE) && !defined (SUPPORT_ATP_ADDITION_FOLDER )*/
#ifndef SUPPORT_ATP_ADDITION_FOLDER
#ifdef SUPPORT_ATP_SAMBA_SHARE
    if (SEC_SHARE != lp_security())
#endif
    {
        if (vuser)
        {
            pass = getpwnam_alloc(NULL, vuser->user.unix_name);
            if (!pass)
            {
                return NULL;
            }
            if(!strcmp(pass->pw_shell,"yes"))
            {
                conn->read_only =0;
            }
            TALLOC_FREE(pass);
