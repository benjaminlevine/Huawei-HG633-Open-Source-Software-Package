...
size_t srvstr_get_path(char *inbuf, char *dest, const char *src, size_t dest_len, size_t src_len, int flags, NTSTATUS *err)
{
	...
	SMB_ASSERT(dest_len == sizeof(pstring));
#endif
printf("\n=file:%s,line:%d,func:%s=src_len=%d,dest=%s=\n",__FILE__,__LINE__,__FUNCTION__,src_len,dest);
	if (src_len == 0) {
		ret = srvstr_pull_buf( inbuf, tmppath_ptr, src, dest_len, flags);
	} else {
		ret = srvstr_pull( inbuf, tmppath_ptr, src, dest_len, src_len, flags);
	}
printf("\n=file:%s,line:%d,func:%s=dest=%s=\n",__FILE__,__LINE__,__FUNCTION__,dest);
	if (SVAL(inbuf,smb_flg2) & FLAGS2_DFS_PATHNAMES) {
		/* 
		 * For a DFS path the function parse_dfs_path()
		 * will do the path processing, just make a copy.
...
int reply_ulogoffX(connection_struct *conn, char *inbuf,char *outbuf,int length,int bufsize)
{
	...
	if(g_ucLoginCtlFlag)
	{
		ATP_LoginCtlLogout(vuser->user.smb_name , ATP_LOGINTYPE_SAMBA);
	}
#endif
