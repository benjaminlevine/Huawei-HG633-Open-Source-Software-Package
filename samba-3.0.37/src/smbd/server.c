...
unsigned char g_ucLoginCtlFlag = 0;
char    g_acUsername[256 + 1];
pid_t   g_CurPid = 0;
#endif
...
extern const struct in_addr *iface_n_sockaddr_storage(int n);
extern TALLOC_CTX *talloc_stackframe(void);
extern BOOL next_token_talloc(TALLOC_CTX *ctx,
			const char **ptr,
			char **pp_buff,
			const char *sep);
/*Modified by lvxin00135113@20110324 for samba ipv6*/
...
static void sig_term(void)
{
	...
	if(g_ucLoginCtlFlag)
	{	    
            if((g_CurPid == sys_getpid())&& ('\0' != g_acUsername[0]))
            {
                ATP_LoginCtlLogout(g_acUsername, client_addr() ,ATP_LOGINTYPE_SAMBA);
               /*start of add by y00181549 fot W724v log at 20120718*/
               ATP_LOG_LogItemPrint(ATP_SYS_LOG_UMOUNT_USER_LOGOUT_INFO,g_acUsername,client_addr());
            }
	}
#endif
...
static BOOL open_sockets_inetd(void)
{
	...
	/* We will abort gracefully when the client or remote system 
	   goes away */
	/*Modifiled by lvxin00135113@20110316 for CVE-2011-0719*/
    #if 0
                ...
                void *buf, size_t len, void *private_data)
{
    ...
    {
        DEBUG(10, ("[%d] send MSG_SMB_VISITOR_ACCESS to children. name=[%s], mac=[%s]\n",
    ...
    {
        DEBUG(10, ("[%d] send MSG_SMB_VISITOR_DENY to children. name=[%s], mac=[%s]\n",
            sys_getpid(), NewVisitor.name, NewVisitor.mac));
		...
		*/
		...
		for(i = 0; i < num_interfaces; i++) {
			...
			for (ptr=ports; next_token(&ptr, tok, " \t,", sizeof(tok)); ) {
				...
				s = fd_listenset[num_sockets] = open_socket_ipv6_in(SOCK_STREAM, port, 0, ifip->s_addr, True);
                                /*Modifiled by lvxin00135113@20110316 for CVE-2011-0719*/
                                #if 0
        			if (s == -1)
                                    return(False);
		...
		for (sock_ptr=sock_addr;
			...
			for (ptr=ports; next_token_talloc(frame, &ptr, &tok, " \t,"); ) {
				...
				s = open_socket_ipv6_in(SOCK_STREAM,
						port,
						num_sockets == 0 ? 0 : 2,
						&ss,
						true);
...
#ifdef NOT_HAVE_POPT	//added by qichenghong 20091029
...
static void set_logfile(char *proname, const char * basedir)
{
	...
	if (asprintf(&logfile, "%s/log.%s", basedir, pname) < 0) {
		return;
	}
...
#ifdef NOT_HAVE_POPT	//cuted by liuyingbiao 00193021
	int ch;
#else
	poptContext pc;
...
#endif
    /*Start for DTS2014012206898 by l00192527 20140218设置ntfs-3g的CPU绑定与优先级*/
	/*设置CPU绑定和优先级*/
	//判断是否是HISI产品
    if (-1 != access("/home/cli/api/ecs/version", 0))
    {
        /*调低优先级*/	
		nice(10);
	}
    /*End for DTS2014012206898 by l00192527 20140218设置ntfs-3g的CPU绑定与优先级*/
	load_case_tables();
...
#endif
#ifdef NOT_HAVE_POPT	//cuted   by liuyingbiao 00193021
	while ((ch = getopt(argc, argv, "DLV")) != EOF) {
		switch(ch) {
		case 'D':
			is_daemon = true;
			break;
		...
		case 'L':
	    		...
	    		if(VOS_OK == ATP_LoginCtlInit(ATP_LOGINTYPE_SAMBA))
	    		{
	    		    	g_ucLoginCtlFlag = 1;
	    		}
	    		else
	    		{
	    		 	DebugPrintStr(3, "Login control init error!\n");
	    		}
	    		break;
	#endif
		case 'V':	
      ...
      set_logfile("smbd" , "/var");
	//set_logfile("smbd" , get_dyn_LOGFILEBASE());
    #if 0
