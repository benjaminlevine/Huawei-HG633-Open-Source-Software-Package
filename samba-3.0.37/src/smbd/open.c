	...
	if (!file_existed) {
		...
		if (ret == -1) {
			...
			return status;
                ...
                memset(acLongPath,0,sizeof(acLongPath));
//                sprintf(acLongPath,"/mnt/usb/%s/%s",parent_dir, name);
                snprintf(acLongPath,sizeof(acLongPath),"/mnt/%s/%s",parent_dir, name);
                #if 0
                if(ATP_UTIL_IsPathVfat(acLongPath))
                {
                     newFileMode = 00777;
                }
                else
                ...
                newFileMode = 00777;
                #endif
		//added by qichenghong 20100225 for DT file mode request
		#if 0
		if ((psbuf->st_mode & (S_IRWXU|S_IRWXG|S_IRWXO)) != unx_mode) {
			SMB_VFS_CHMOD(conn, path,
				      (psbuf->st_mode & ~(S_IRWXU|S_IRWXG|S_IRWXO)) | unx_mode);
		}
            #else
		if ((psbuf->st_mode & (S_IRWXU|S_IRWXG|S_IRWXO)) != newFileMode) {
			SMB_VFS_CHMOD(conn, path,newFileMode);
		}

            #endif
		//added by qichenghong 20100225 for DT file mode request end
	}
	...
	if (!(file_attributes & FILE_FLAG_POSIX_SEMANTICS)) {
		...
		if (mode & ~(S_IRWXU|S_IRWXG|S_IRWXO) && (mode & ~psbuf->st_mode)) {
			SMB_VFS_CHMOD(conn, name,
                ...
                memset(acLongPath,0,sizeof(acLongPath));
//                sprintf(acLongPath,"/mnt/usb/%s",name);   //for w723v
                snprintf(acLongPath,sizeof(acLongPath),"/mnt/%s",name);             //for atp
                #if 0
                if(ATP_UTIL_IsPathVfat(acLongPath))
                {
                     mode = 00777;
                ...
                #endif
		//added by qichenghong 20100225 for DT file mode request
		if ((psbuf->st_mode & (S_IRWXU|S_IRWXG|S_IRWXO)) != mode) {
                    #if 0
			SMB_VFS_CHMOD(conn, name,
				      (psbuf->st_mode & ~(S_IRWXU|S_IRWXG|S_IRWXO)) | mode);
                    #else
			SMB_VFS_CHMOD(conn, name,mode);
                    #endif
		}
		//added by qichenghong 20100225 for DT file mode request end
	}
