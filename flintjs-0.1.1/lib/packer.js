/*
 * script to compress a javascript file.
 *
 * Usage: ./packer.js infile.js outfile.js
 *
 * Derived from the WSH version of 'packer' by
 * Dean Edwards (http://dean.edwards.name/packer/)
 *
 * Converted to run within a shell and rhino environment
 * by David McNab <david@rebirthing.co.nz>
 *
 * Requirements: you need rhino and java installed
 *
 * Note - for dojo users, the compressed script gets a couple of paths
 * wrong - if you symlink your js/dojo/src directory to src in the toplevel
 * server directory, all should be fine.
 *
 * For me, it's compressing a >200k dojo.js file to about 117k
 *
 * Released under the original author's Creative Commons license:
...
function main() {
  ...
  if (argv.length < 2) {
    ...
    print("(go get a coffee or meal, or read Slashdot)");
...
// of his compressor/obfuscator

// don't look below this point if your value your sanity!!
