...
// www.JSLint.com
// Rhino Edition
// Command line argument-related modifications by Roman Nurik
"use strict";
...
(c%16).toString(16);})+'"';}
return'"'+this+'"';};function assume(){if(!option.safe){if(option.rhino){predefined.union(rhino);}
...
t=Object.create(t);if(type==='(string)'){if(jx.test(value)){warningAt("Script URL.",line,from);}}else if(type==='(identifier)'){t.identifier=true;if(option.nomen&&value.charAt(0)==='_'){warningAt("Unexpected '_' in '{a}'.",line,from,value);}}
...
if(xmode!=='stylestring'&&s.substr(0,1).isAlpha()){warningAt("Missing space after '{a}'.",line,character,t);}
if(c==='0'){d=t.substr(1,1);if(d.isDigit()){if(token.id!=='.'&&xmode!=='stylestring'){warningAt("Don't use extra leading zeros '{a}'.",line,character,t);}}else if(jsonmode&&(d==='x'||d==='X')){warningAt("Avoid 0x-. '{a}'.",line,character,t);}}
...
s='';token.comment=true;break;case'/*':if(src||(xmode&&xmode!=='script'&&xmode!=='style'&&xmode!=='stylestring')){warningAt("Unexpected comment.",line,character);}
...
character+=l;s=s.substr(l);return it('(regex)',c);case'\\':c=s.charAt(l);if(c<' '){warningAt("Unexpected control character in regular expression.",line,from+l);}else if(c==='<'){warningAt("Unexpected escaped character '{a}' in regular expression.",line,from+l,c);}
...
break;}else if(xmode==='stylestring'&&c==='('){c=s.charAt(0);character+=1;s=s.substr(1);t='';while(c!==')'){t+=c;c=s.charAt(0);character+=1;s=s.substr(1);if(c===''){errorAt("Missing ')'.",line,from+l,character);}}
return it('(string)',t);}
...
function reservevar(s){return reserve(s,function(){if(this.id==='this'){if(option.safe){warning("ADsafe violation.",this);}}
return this;});}
...
function statements(begin){var a=[];if(begin){if(option.strict&&nexttoken.type!=='(string)'){warning('Missing "use strict" statement.',nexttoken);}
if(nexttoken.type==='(string)'&&nexttoken.value==='use strict'){advance();advance(';');}}
...
function styleAttribute(){if(nexttoken.id==='*'||nexttoken.id==='#'||nexttoken.value==='_'){if(!option.css){warning("Unexpected '{a}'.",nexttoken,nexttoken.value);}
advance();}
if(nexttoken.id==='-'){if(!option.css){warning("Unexpected '{a}'.",nexttoken,nexttoken.value);}
advance('-');}
var v=nexttoken.value.toLowerCase();if(!nexttoken.identifier){error("Excepted a style attribute, and instead saw '{a}'.",nexttoken,v);}
advance();return v;}
function styleValue(){var v;for(;;){v=nexttoken.value.toLowerCase();if(option.adsafe){switch(v){case'url':warning("ADsafe style url violation.");break;case'expression':warning("ADsafe style url violation.");}}
if(nexttoken.id==='(end)'||nexttoken.id==='}'||nexttoken.id==='{'){error("Excepted a style value, and instead saw '{a}'.",nexttoken,v);}
advance();if(nexttoken.id===';'||nexttoken.id===':'){break;}}}
function substyle(){for(;;){if(nexttoken.id==='}'||nexttoken.id==='(end)'||xquote&&nexttoken.id===xquote){return;}
styleAttribute();advance(':');styleValue();advance(';');}}
...
function styles(){while(nexttoken.id!=='</'&&nexttoken.id!=='(end)'){stylePattern();xmode='stylestring';if(nexttoken.id===';'){advance(';');}else{advance('{');substyle();xmode='style';advance('}');}}}
...
if(option.adsafe){if(n==='html'){error("Current, ADsafe does not operate on whole HTML documents. It operates on <div> fragments and .js files.",token);}
...
function doAttribute(n,a,v){var u;if(a==='id'){u=typeof v==='string'?v.toUpperCase():'';if(ids[u]===true){warning("Duplicate id='{a}'.",nexttoken,v);}
ids[u]=true;if(option.adsafe){if(adsafe_id){if(v.slice(0,adsafe_id.length)!==adsafe_id){warning("ADsafe violation: An id must have a '{a}' prefix",nexttoken,adsafe_id);}else if(!/^[A-Z]+_[A-Z]+$/.test(v)){warning("ADSAFE violation: bad id.");}}else{adsafe_id=v;if(!/^[A-Z]+_$/.test(v)){warning("ADSAFE violation: bad id.");}}}}else if(a==='href'||a==='background'||a==='content'||a==='data'||a.indexOf('src')>=0||a.indexOf('url')>=0){if(option.safe&&ux.test(v)){error("ADsafe URL violation.");}
...
break;case'applet':case'body':case'embed':case'frame':case'frameset':case'head':case'iframe':case'img':case'object':case'param':if(option.adsafe){warning("ADsafe violation: Disallowed tag: "+n);}
break;}}
function closetag(n){return'</'+n+'>';}
function html(){var a,attributes,e,n,q,t,v,wmode;xmode='html';stack=null;for(;;){switch(nexttoken.value){case'<':xmode='html';advance('<');attributes={};t=nexttoken;if(!t.identifier){warning("Bad identifier {a}.",t,t.value);}
n=t.value;t.name=n;advance();if(!stack){stack=[];doBegin(n);}
...
a=nexttoken.value;advance();if(!option.cap&&a!==a.toLowerCase()){warning("Attribute '{a}' not all lower case.",nexttoken,a);}
a=a.toLowerCase();if(attributes.hasOwnProperty(a)){warning("Attribute '{a}' repeated.",nexttoken,a);}
if(a.slice(0,2)==='on'){if(!option.on){warning("Avoid HTML event handlers.");}
xmode='scriptstring';xquote='';advance('=');q=nexttoken.id;if(q!=='"'&&q!=="'"){error("Missing quote.");}
...
xmode='html';xquote='';advance(q);v=false;}else if(a==='style'){xmode='scriptstring';xquote='';advance('=');q=nexttoken.id;if(q!=='"'&&q!=="'"){error("Missing quote.");}
xmode='stylestring';xquote=q;advance(q);substyle();xmode='html';xquote='';advance(q);v=false;}else{if(nexttoken.id==='='){advance('=');v=nexttoken.value;if(nexttoken.type!=='(string)'){warning("Missing quotes around {a} attribute value.",token,a);}
advance();}else{v=true;}}
...
n=nexttoken.value;advance();if(!stack){error("Unexpected '{a}'.",nexttoken,closetag(n));}
...
xmode='outer';advance('>');break;case'<!':if(option.safe){error("ADsafe HTML violation.");}
xmode='outer';v=false;for(;;){advance();if(nexttoken.id==='>'){break;}
if(nexttoken.id==='<'||nexttoken.id==='(end)'){error("Missing '{a}'.",token,'>');}
if(nexttoken.id==='--'){v=!v;}}
if(v){warning("Misshapen HTML comment.");}
xmode='html';advance('>');break;case'(end)':return;default:if(nexttoken.id==='(end)'){error("Missing '{a}'.",nexttoken,'</html>');}else if(nexttoken.id!=='--'&&nexttoken.id!=='#'){error("Unexpected '{a}'.",nexttoken,nexttoken.value);}else{advance();}}
...
return v;}).fud=function(){nospace();var v=parse(0);advance(')',this);if(v){if(v.id==='function'){advance('(');return token.led(v);}}
nospace(prevtoken,token);return v;};infix('[',function(left){nospace();var e=parse(0),s;if(e&&e.type==='(string)'){if(option.safe&&banned[e.value]===true){warning("ADsafe restricted word '{a}'.",this,e.value);}
...
break;}}
advance(']',this);return;},160);(function(x){x.nud=function(){var b,i,s;if(nexttoken.id==='}'){advance('}');return;}
...
function doFunction(i){var s=scope;scope=Object.create(s);funct={'(name)':i||'"'+anonname+'"','(line)':nexttoken.line+1,'(context)':funct,'(breakage)':0,'(loopage)':0,'(scope)':scope};functions.push(funct);if(i){addlabel(i,'function');}
...
if(o.safe){o.browser=false;o.debug=false;o.eqeqeq=true;o.evil=false;o.forin=false;o.nomen=true;o.on=false;o.rhino=false;o.safe=true;o.sidebar=false;o.strict=true;o.sub=false;o.undef=true;o.widget=false;predefined.Date=false;predefined['eval']=false;predefined.Function=false;predefined.Object=false;predefined.ADSAFE=true;}
...
(f['(name)']||'')+'('+
(f['(params)']||'')+')</div>');detail('Closure',cl);detail('Variable',va);detail('<big><b>Unused</b></big>',un);detail('Label',la);detail('Outer',ou);detail('Global',gl);}
...
(function(a) {
  var e, i, input;
  if (!a[0]) {
    print("Usage: jslint.js file.js [file2.js ...]");
    quit(1);
  }
