...
static void *rtpproxy_main(void *arg) {
   ...
   char ucTos;
   char ctl_buf[64];
   int lATPOnlineState = 1;         //ATP平台的Online State始终为1
         ...
         if ( (rtp_proxytable[i].rtp_con_rx_sock != 0) &&
...
#else
            count=recvmsg(rtp_proxytable[i].rtp_con_rx_sock, &msg, MSG_WAITALL);
#endif
            ...
            if (count > 0) {
               ...
               if (rtp_proxytable[i].rtp_con_tx_sock != 0) {
                  ...
                  sipalg_rtpsock_setopt(rtp_proxytable[i].rtp_con_tx_sock, 
                        rtp_proxytable[i].t_online, rtp_proxytable[i].direction, ucTos, 1);
#else
                  //sts = setsockopt(rtp_proxytable[i].rtp_con_tx_sock, IPPROTO_IP, IP_TOS, (void*)&ucTos, sizeof(ucTos));
                  //if (sts != 0)
                  //{
                  //   printf("sockbind setsockopt IP_RECVTOS fail, sts %d %s\n", sts, strerror(errno));
                  //}  
                  sipalg_rtpsock_setopt(rtp_proxytable[i].rtp_con_tx_sock, 
                        lATPOnlineState, rtp_proxytable[i].direction, ucTos, 1);
         ...
         if ( (rtp_proxytable[i].rtp_rx_sock != 0) &&
            ...
            if (count > 0) {
               ...
               if (rtp_proxytable[i].rtp_tx_sock != 0) {
                  ...
                  /*消除开启宏时的pclint告警*/
                  lATPOnlineState = lATPOnlineState;
                  sipalg_rtpsock_setopt(rtp_proxytable[i].rtp_tx_sock, 
                  ...
                  //}  
                  sipalg_rtpsock_setopt(rtp_proxytable[i].rtp_tx_sock, 
                                       lATPOnlineState, rtp_proxytable[i].direction, ucTos, 0);
#endif /* SUPPORT_ATP_DT_QOS */
#endif
                  /* END:   Added by l00183184, 2013/7/24   PN:DTS2013071906746 alg升级 */
...
#endif                         
{
   ...
   for (i2 = (prev_used_port - configuration.rtp_port_low + 1);
      ...
      if (j == RTPPROXY_SIZE) {
         ...
         /* BEGIN: Added by l00183184, 2013/7/24   PN:DTS2013071906746 alg升级 */
         sock = sockbind(local_ipaddr, port, PROTO_UDP, 0, SOCK_CREATE_BY_RTP);

         /* END:   Added by l00183184, 2013/7/24   PN:DTS2013071906746 alg升级 */

         if (sock) {
            /* BEGIN: Added by l00183184, 2013/7/24   PN:DTS2013071906746 alg升级 */
            sock_con = sockbind(local_ipaddr, port+1, PROTO_UDP, 0, SOCK_CREATE_BY_RTCP);
