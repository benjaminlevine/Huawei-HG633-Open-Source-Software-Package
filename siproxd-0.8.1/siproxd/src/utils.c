...
/******************************************************************************
  ...
  被调函数  : 
  输出参数  : 无
  返 回 值  : pcNum是数字则返回真VOS_TRUE， 
              pcNum不是数字则返回假VOS_FALSE，
...
int ValidateNumber(char* pcNum)
{
    ...
    ulSize = strlen(pcNum);
   printsip("@@@@@ pcNum=%s @@@@@",pcNum);	
    for ( i = 0; i < ulSize; i++ )
    {
    	 printsip("@@@@@ pcNum[%d]=%c @@@@@",i,pcNum[i]);	
    ...
    {
        printsip("@@@@@ ulSize = [%d] , i=[%d] @@@@@",ulSize,i);	
        return 0;
    }
