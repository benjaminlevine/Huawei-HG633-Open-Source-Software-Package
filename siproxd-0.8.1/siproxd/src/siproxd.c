 ...
 */
   ...
   /* BEGIN: Added by l00183184, 2013/7/29   PN:DTS2013071906746 alg升级 */
   /* 需要注册SIGUSR1的处理函数，否则cms发出该信号给siproxd会异常终止掉siproxd*/
   if (sigaction(SIGUSR1, &act, NULL)) {
      ERROR("Failed to install SIGPIPE handler");
   }
   /* END:   Added by l00183184, 2013/7/29   PN:DTS2013071906746 alg升级 */
 ...
 */
   ...
   if (configuration.daemonize) {
      ...
      /* BEGIN: Added by l00183184, 2013/7/25   PN:DTS2013071906746 alg升级 */
      /* 不要关掉标准输入输出 */
#if 0
      log_set_stderr(0);
      ...
      close(i);
#endif
      /* END:   Added by l00183184, 2013/7/25   PN:DTS2013071906746 alg升级 */
 ...
 *****************************/
   while (!exit_program) {
      ...
      while ((sts = sipsock_waitfordata(buff, sizeof(buff)-1,
                                    &ticket.from, &ticket.protocol, &ticket.tos, &readfd)) <=0 ) 
#else
      while ((sts = sipsock_waitfordata(buff, sizeof(buff)-1,
                                    &ticket.from, &ticket.protocol, &readfd)) <=0 ) 
#endif
      ...
      /*z00164626_20120707_BEGIN:check the sip direction*/
      if (readfd == sip_udp_lan_socket)
      {
          ticket.direction    = MSG_IS_REQUEST(ticket.sipmsg) ? REQTYP_OUTGOING : RESTYP_OUTGOING;
      }
      else if (readfd == sip_udp_wan_socket)
      {
          ticket.direction    = MSG_IS_REQUEST(ticket.sipmsg) ? REQTYP_INCOMING : RESTYP_INCOMING;
      }
      /*z00164626_20120707_END:check the sip direction*/
      
      /* BEGIN: Added by l00183184, 2013/7/2   PN:sipalg 不解析domain，直接获取dst addr */
...
#endif
      /* END:   Added by l00183184, 2013/7/2   PN:sipalg 不解析domain，直接获取dst addr */
      ...
      if (MSG_IS_REGISTER(ticket.sipmsg) && 
          MSG_IS_REQUEST(ticket.sipmsg)) {
         /*外部的注册消息因为安全原因不作响应*/
         if (REQTYP_INCOMING == ticket.direction)
         {
            DEBUGC(DBCLASS_SIP, "Receive register packets from WAN interface, discard!");
            printf("\n SIP ALG Receive register packets from WAN interface, discard!");
            goto end_loop; 
         }
