...
&&&*/
      ...
      if (url->port) {
         ...
         /* start of AU4D01412 修改端口后又被转换成 5060 by d00107688 2009-02-18 */
         if (configuration.sip_listen_port)
         {
            port = configuration.sip_listen_port;
         }
         ...
         /* end of AU4D01412 修改端口后又被转换成 5060 by d00107688 2009-02-18 */
      }
#endif
   ...
   */
   ...
   sipsock_send(sendto_addr, port, ticket->protocol, buffer,
       buflen, is_t_online(request, i), type, ticket->tos); 
#else
   sipsock_send(sendto_addr, port, ticket->protocol, buffer, buflen, type);
#endif
   /* END:   Added by l00183184, 2013/7/24   PN:DTS2013071906746 alg升级 */
   osip_free (buffer);
...
#ifdef SUPPORT_ATP_TELMEX
   sipsock_send(sendto_addr, port, ticket->protocol, buffer,
         buflen, is_t_online(response, i), type, ticket->tos); 
#else
   sipsock_send(sendto_addr, port, ticket->protocol, buffer, buflen, type);
#endif
   /* END:   Added by l00183184, 2013/7/24   PN:DTS2013071906746 alg升级 */
      ...
      if (sdp_message_m_port_get(sdp, media_stream_no)) {
         ...
         if ((msg_port > 0) && (msg_port <= 65535)) {
            ...
            sts = rtp_start_fwd(osip_message_get_call_id(mymsg),
                                ...
                                addr_media, msg_port,
                                isrtp,
                                is_t_online(mymsg, ticket->index));
#else
            sts = rtp_start_fwd(osip_message_get_call_id(mymsg),
                                client_id,
                                rtp_direction, call_direction,
