...
/* BEGIN: Modified by l00183184, 2013/8/12   PN:DTS2013071906746 alg升级 */
/* 由于按需拨号必须绑定端口才能有路由，才能触发sendto成功，且绑定端口后能够确保转发的包的源地址正确 */
//int sip_udp_socket=0;
int sip_udp_lan_socket=0;
int sip_udp_wan_socket=0;
/* END:   Modified by l00183184, 2013/8/12   PN:DTS2013071906746 alg升级 */
...
int sipsock_listen (void) {
   ...
   /* BEGIN: Added by l00183184, 2013/8/12   PN:DTS2013071906746 alg升级 */
   struct in_addr lanipaddr;
   struct in_addr wanipaddr;
   /* END:   Added by l00183184, 2013/8/12   PN:DTS2013071906746 alg升级 */
   ...
   /* BEGIN: Modified by l00183184, 2013/7/24   PN:DTS2013071906746 alg升级 */
   //sip_udp_socket=sockbind(ipaddr, configuration.sip_listen_port, PROTO_UDP, 1, SOCK_CREATE_BY_SIP);
   sip_udp_lan_socket = sockbind(lanipaddr, configuration.sip_listen_port, PROTO_UDP, 1, SOCK_CREATE_BY_SIP);   
   sip_udp_wan_socket = sockbind(wanipaddr, configuration.sip_listen_port, PROTO_UDP, 1, SOCK_CREATE_BY_SIP);
   /* END:   Modified by l00183184, 2013/7/24   PN:DTS2013071906746 alg升级 */
   //if (sip_udp_socket == 0) return STS_FAILURE; /* failure */
   if ((sip_udp_lan_socket == 0) || (sip_udp_wan_socket == 0)) return STS_FAILURE; /* failure */
   ...
   if (configuration.sip_dscp) {
      ...
      if (geteuid()==0) {
         ...
         if (!(configuration.sip_dscp & ~0x3f)) {
            ...
            */
            /* BEGIN: Modified by l00183184, 2013/8/12   PN:DTS2013071906746 alg升级 */
            if(setsockopt(sip_udp_lan_socket, SOL_IP, IP_TOS, &tos, sizeof(tos))) {
               ERROR("sipsock_listen: setsockopt() failed while "
                     "setting DSCP value: %s", strerror(errno));
            }
            if(setsockopt(sip_udp_wan_socket, SOL_IP, IP_TOS, &tos, sizeof(tos))) {
               ERROR("sipsock_listen: setsockopt() failed while "
                     "setting DSCP value: %s", strerror(errno));
            }
            /* END:   Modified by l00183184, 2013/8/12   PN:DTS2013071906746 alg升级 */
   ...
   sip_tcp_socket=sockbind(ipaddr, configuration.sip_listen_port,
                           PROTO_TCP, 1, SOCK_CREATE_BY_SIP);
   ...
   DEBUGC(DBCLASS_NET,"bound UDP lan socket=%i, wan socket=%i, TCP socket=%i",
          sip_udp_lan_socket, sip_udp_wan_socket, sip_tcp_socket);
...
#endif
{
...
#if defined (TOS_RCV) &&defined(SUPPORT_ATP_TELMEX)
   char tos_temp = 0;
   char ctl_buf[64];
   struct msghdr msg;
   struct iovec iov;
   ...
   if (sip_udp_lan_socket > sip_tcp_socket) {
      highest_fd = sip_udp_lan_socket;
   } else {
      highest_fd = sip_tcp_socket;
   }
   if (sip_udp_wan_socket > highest_fd) {
      highest_fd = sip_udp_wan_socket;
   }
   /* END:   Added by l00183184, 2013/8/12   PN:DTS2013071906746 alg升级 */
...
for (i=0; i< highest_fd; i++) {
   ...
   if (0 == counter)
   {
      ...
      {
         udp_fd = sip_udp_lan_socket;
      }
      else if (FD_ISSET(sip_udp_wan_socket, &fdset))
      {
         udp_fd = sip_udp_wan_socket;
      }
      counter = 1;
   }
   else 
   ...
   *readfd = udp_fd;
   ...
   if (0 != udp_fd) {
...
#if defined (TOS_RCV) && defined(SUPPORT_ATP_TELMEX)
      length = recvmsg(udp_fd, &msg, MSG_WAITALL);
      if (length > 0) {
      ...
      *tos = tos_temp;
#else
      length=recvfrom(udp_fd, buf, bufsize, 0,
...
/* 打mark注意不要与其他的mark值发生冲突，比如qos的mark等，相关mark值可以查看atputil.h中的定义 */
static void sipsock_setmark(int lfd)
{
    int mark = 0;
    socklen_t len = sizeof(mark);

    if (getsockopt(lfd, SOL_SOCKET, SO_MARK, (char *)(&mark), &len) < 0)
    {
        printf("Siproxy get mark failed.\n");
    }

    /*打MARK供按需拨号使用*/
    mark |= 0x200000;
    if (setsockopt(lfd, SOL_SOCKET, SO_MARK, (char *)(&mark), sizeof(mark)) < 0)
    {
        printf("Siproxy set mark failed.\n");
    }
}
...
static void sipsock_setopt(int sock, int t_online, int direct, int tos)
{
   if (t_online) {
         ...
         if (0 != ATP_SIP_ALG_QoS_SetMark(sock, NFMARK_SIPALG, QOSTELMEX_TOS_VOIP_CTL)) {
         ...
         sts = setsockopt(sock, SOL_SOCKET, SO_MARK, (void*)&nfmark, sizeof(nfmark));
         if (sts != 0) {
      ...
      if (0 != ATP_SIP_ALG_QoS_SetMark(sock, 0, tos)) {
...
void sipalg_rtpsock_setopt(int sock, int t_online, int direct, int tos, int type)
{
   if (t_online) {
   	  if (direct == DIR_OUTGOING) {
	  	 if (type) { /* RTP */
            if (0 != ATP_SIP_ALG_QoS_SetMark(sock, NFMARK_RTP, QOSTELMEX_TOS_VOIP_CTL)) {
               fprintf(stderr, "can not set the nfmakr %d for SIP alg", NFMARK_RTP);
            }
	  	 }
		 else { /* RTCP */
            if (0 != ATP_SIP_ALG_QoS_SetMark(sock, NFMARK_RTCP, QOSTELMEX_TOS_VOIP_CTL)) {
...
#else
int sipsock_send(struct in_addr addr, int port, int protocol,
                 char *buffer, size_t size, int direct) 
#endif
{
   struct sockaddr_in dst_addr;
   int sts;
   int i;
   int lATPOnlineState = 1;
   int lATPTos = 0;

   /* first time: allocate a socket for sending */
   /* BEGIN: Added by l00183184, 2013/8/12   PN:DTS2013071906746 alg升级 */
   //if (sip_udp_socket == 0) {
   if ((sip_udp_lan_socket == 0) || (sip_udp_wan_socket == 0)) {
   ...
   if (protocol == PROTO_UDP) {
      ...
      /* BEGIN: Modified by l00183184, 2013/8/12   PN:DTS2013071906746 alg升级 */
      /* incoming的报文需要用sip_udp_lan_socket发，否则报文的源地址会变成网关的wan侧地址
         outgoing的报文需要用sip_udp_wan_socket发，否则无法触发按需拨号 */
      if (direct == REQTYP_INCOMING || direct == RESTYP_INCOMING)
      {
         /* 合入TELMEX的打mark的需求*/
#ifdef SUPPORT_ATP_TELMEX
        /*消除宏不开启时的pclint告警*/
         lATPOnlineState = lATPOnlineState;
         lATPTos = lATPTos;

         sipsock_setopt(sip_udp_lan_socket, t_online, direct, tos);
#else
        sipsock_setopt(sip_udp_lan_socket, lATPOnlineState, direct, lATPTos);
#endif
   
         sts = sendto(sip_udp_lan_socket, buffer, size, 0,
                   (const struct sockaddr *)&dst_addr,
                   (socklen_t)sizeof(dst_addr));
      }
      else if (direct == REQTYP_OUTGOING || direct == RESTYP_OUTGOING)
      {
...
#else
        sipsock_setopt(sip_udp_wan_socket, lATPOnlineState, direct, lATPTos);
#endif
#ifndef SUPPORT_ATP_TELMEX
         /* 触发按需拨号打的mark */
         sipsock_setmark(sip_udp_wan_socket);
#endif
      ...
      sipsock_setopt(sip_tcp_cache[i].fd, t_online, direct, tos);
#else
      sipsock_setopt(sip_tcp_cache[i].fd, lATPOnlineState, direct, lATPTos);
#endif
      /* END:   Added by l00183184, 2013/7/24   PN:DTS2013071906746 alg升级 */
