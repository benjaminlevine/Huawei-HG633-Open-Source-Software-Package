...
int ATP_SIP_ALG_QoS_SetMark(int ulSockfd, int nfmark, unsigned int Tos)
{
    VOS_UINT32 ulMark;
	...
	if (0 > setsockopt((VOS_INT32)ulSockfd, IPPROTO_IP, IP_TOS, &Tos, sizeof(VOS_UINT32)))
    {
        fprintf(stderr, "\r\nwarning:  IP_TOS  setsockopt error: %s\r\n", strerror(errno));
        return (-1);
    }
    
	if (0 > setsockopt(ulSockfd, SOL_SOCKET, SO_MARK, &ulMark, len))
    {
        fprintf(stderr, "\r\nwarning:  QoS  SO_MARK error: %s\r\n", strerror(errno));
        return (-1);
    }
...
/* TELMEX要求SIP ALG域名为ims.telmex.com的需要Qos，保证和网关自身一样的优先级 */
static int t_online_url(osip_uri_t *url)
{
	if (url != 0 && url->host != 0) {
		...
		if (pstr != 0) {
			pstr++;
			if (!strcasecmp(pstr, "ims.telmex.com")) {
				//printf("siproxd: is_t_online=true\n"); 
				return 1;
			}
			if (!strcasecmp(url->host, "ims.telmex.com")) {
				//printf("siproxd: is_t_online=true\n"); 
				return 1;
			}
		}
...
int compare_url(osip_uri_t *url1, osip_uri_t *url2) {
   ...
   /* BEGIN: Added by l00183184, 2013/7/31   PN:DTS2013071906746 alg升级 */
   int sts1 = STS_FAILURE;
   int sts2 = STS_FAILURE;
   /* END:   Added by l00183184, 2013/7/31   PN:DTS2013071906746 alg升级 */
...
#ifdef SUPPORT_ATP_TELMEX
   sipsock_send(addr, port, ticket->protocol, buffer, buflen, 0, 0, 0);
#else
   sipsock_send(addr, port, ticket->protocol, buffer, buflen, 0);
#endif
   /* END:   Added by l00183184, 2013/7/24   PN:DTS2013071906746 alg升级 */
