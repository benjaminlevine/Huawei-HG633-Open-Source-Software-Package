...
typedef struct tag_ICMP_RS_PACK_CHECKSUM_S
{
    ...
    unsigned char  *pcTemBuff;     /* icmp packet content */
...
IcmpRsCheckSum(ICMP_RS_PACK_CHECKSUM_S * pstCheckSum)
{
    ...
    for ( lLoopi = 0 ; lLoopi < pstCheckSum->usLength; lLoopi++ )
    {
        ...
        printf("%02x ",pstCheckSum->pcTemBuff[lLoopi]);    
    }
#endif
    ...
    if ( NULL != pstCheckSum->pcTemBuff )
    {
        for ( lLoopi = 0 ; lLoopi < pstCheckSum->usLength ; )
        {
            usTem = ((((unsigned short)pstCheckSum->pcTemBuff[lLoopi])<<8)&0xff00) | pstCheckSum->pcTemBuff[lLoopi + 1];
#if RADVD_DEBUG            
            printf("\r\n usTem : %#x \r\n",usTem);
#endif
...
{                 
    ...
    stCheckSum.pcTemBuff = (unsigned char  *)malloc(len + 2);

    if (NULL == stCheckSum.pcTemBuff)
    {
        return -1;
    }
    
    stCheckSum.usLength = len;
    memcpy(stCheckSum.pcTemBuff,msg,len);
#if RADVD_DEBUG
    /* BEGIN: Added by z67728, 2009/11/28 */
    ...
    for ( i = 0; i < len ; i++ )
    {
        ...
        printf("%02x ",stCheckSum.pcTemBuff[i]);
    }
        ...
        switch(cmsg->cmsg_type)
        {
              ...
              case IPV6_HOPLIMIT:
                if ((cmsg->cmsg_len == CMSG_LEN(sizeof(int))) && 
                    (*(int *)(int)CMSG_DATA(cmsg) >= 0) && 
                    (*(int *)(int)CMSG_DATA(cmsg) < 256))
                {
                  *hoplimit = *(int *)(int)CMSG_DATA(cmsg);
                }
                else
                {
                  flog(LOG_ERR, "received a bogus IPV6_HOPLIMIT from the kernel! len=%d, data=%d",
                  	cmsg->cmsg_len, *(int *)(int)CMSG_DATA(cmsg));
                  free(stCheckSum.pcTemBuff);
                  return (-1);	
                }  
                break;
#endif /* IPV6_HOPLIMIT */
            case IPV6_PKTINFO:
                ...
                ((struct in6_pktinfo *)(int)CMSG_DATA(cmsg))->ipi6_ifindex)
                {
                    *pkt_info = (struct in6_pktinfo *)(int)CMSG_DATA(cmsg);
                    ...
                    if ( 1 == ulSum && 1 == ((unsigned char*)(*pkt_info))[i - 1] )
                    {
                        ...
                        cmsg->cmsg_len, ((struct in6_pktinfo *)(int)CMSG_DATA(cmsg))->ipi6_ifindex);
#endif
                        free(stCheckSum.pcTemBuff);
                        return (-1);
                    }
