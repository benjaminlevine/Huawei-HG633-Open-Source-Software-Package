...
#ifdef SUPPORT_ATP_W724V_RA_CONFIG
struct AdvPrefix g_stAdvPrefix;
...
timer_handler(void *data)
{
...
#ifdef SUPPORT_ATP_W724V_RA_CONFIG   
        struct AdvPrefix *pstTempAdvPrefix;
#endif
	...
	dlog(LOG_DEBUG, 4, "timer_handler called for %s", iface->Name);
        ...
        if ( 1 == g_lWanDownFlag )
        {   
            if( 0 != g_stAdvPrefix.AdvValidLifetime )
            {
                g_stAdvPrefix.next = iface->AdvPrefixList;
                iface->AdvPrefixList = &g_stAdvPrefix;
            }
        }
#endif
...
#ifdef SUPPORT_ATP_W724V_RA_CONFIG
        if (iface->init_racount == MAX_INITIAL_RTR_ADVERTISEMENTS - 1 && NULL != iface->AdvPrefixList && (&g_stAdvPrefix == iface->AdvPrefixList))
        {
            pstTempAdvPrefix = iface->AdvPrefixList->next;       
            iface->AdvPrefixList = pstTempAdvPrefix;   
            iface->init_racount++;
        }
#endif
...
disable_oldprefix(char *pcBridgeName)
{
	...
	int lRet = 0;
    	 ...
    	 */
#ifdef SUPPORT_ATP_W724V_RA_CONFIG     
        memset(&g_stAdvPrefix , 0, sizeof(struct AdvPrefix)); 
#endif		
...
#ifdef SUPPORT_ATP_W724V_RA_CONFIG                                                           
                                    memcpy(&g_stAdvPrefix, prefix, sizeof(struct AdvPrefix));
#endif									
				}				
				else				
				{
