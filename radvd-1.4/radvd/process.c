/*
 *   $Id: process.c,v 1.1 2009/08/22 07:51:00 y42304 Exp $
 *
 *   Authors:
	...
	struct sockaddr_in6 *addr, struct in6_pktinfo *pkt_info, int hoplimit)
{
	...
	icmph = (struct icmp6_hdr *) (int)msg;
	...
	struct sockaddr_in6 *addr)
{
	...
	radvert = (struct nd_router_advert *) (int)msg;
	...
	while (len > 0)
	{
		...
		{
		case ND_OPT_MTU:
			mtu = (struct nd_opt_mtu *)(int)opt_str;
			if (len < sizeof(*mtu))
				return;
		...
		case ND_OPT_PREFIX_INFORMATION:
			pinfo = (struct nd_opt_prefix_info *) (int)opt_str;
			if (len < sizeof(*pinfo))
				return;
		...
		case ND_OPT_RDNSS_INFORMATION:
			rdnssinfo = (struct nd_opt_rdnss_info_local *) (int)opt_str;
