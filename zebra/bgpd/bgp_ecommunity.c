...
ecommunity_hash_alloc (struct ecommunity *val)
{
  ...
  ecom->size = val->size;
  ecom->val = XMALLOC (MTYPE_ECOMMUNITY_VAL, val->size * ECOMMUNITY_SIZE);
  memcpy (ecom->val, val->val, val->size * ECOMMUNITY_SIZE);
...
/* Transform RFC2547 routing distinguisher to extended communities
   type.  */
void
ecommunity_rd2com (struct ecommunity_val *rd, u_char type)
{
  rd->val[0] = rd->val[1];
  rd->val[1] = type;
}
