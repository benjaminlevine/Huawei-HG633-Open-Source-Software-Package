...
int main(int argc, char *argv[])
{
    ...
    /*Start for DTS2014011503126 by y00105954 20140218设置ntfs-3g的CPU绑定与优先级*/
	/*设置CPU绑定和优先级*/
    if (-1 != access("/home/cli/api/ecs/version", 0))
    {
        ...
        /*绑定进程到指定CPU*/
        __CPU_ZERO(&mask);
        __CPU_SET(1, &mask);  //CPU1
        status = sched_setaffinity(getpid(), sizeof(mask), &mask);
        if (status != 0)
        {
            printf("ntfs-3g bind cpu 0 fail!");
        }
    #endif
        /*调低优先级*/	
		nice(10);
	}
    /*End for DTS2014011503126 by y00105954 20140218设置ntfs-3g的CPU绑定与优先级*/
	/*
