...
static int create_tunnel(struct l2tp_parm *p)
{
	...
	if (rtnl_talk(&genl_rth, &req.n, 0, 0, NULL, NULL, NULL) < 0)
		return -2;
...
static int get_session(struct l2tp_data *p)
{
	...
	if (rtnl_dump_filter(&genl_rth, session_nlmsg, p, NULL, NULL) < 0) {
...
static int get_tunnel(struct l2tp_data *p)
{
	...
	if (rtnl_dump_filter(&genl_rth, tunnel_nlmsg, p, NULL, NULL) < 0) {
		fprintf(stderr, "Dump terminated\n");
		exit(1);
	}
...
int genl_ctrl_resolve_family(const char *family)
{
	...
	if (rtnl_talk(&genl_rth, &req.n, 0, 0, &req.n, NULL, NULL) < 0) {
