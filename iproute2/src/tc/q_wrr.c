...
static int wrr_parse_opt(struct qdisc_util *qu, int argc, char **argv, struct nlmsghdr *n)
{
  if(n->nlmsg_flags & NLM_F_CREATE) {
	...
	/* HG586V100R001 c40163 2007-8-14 BT modify begin 
    int sour,dest,ip,mac,masq;
    */
    int sour,dest,ip,mac,masq,fw;
	/* HG586V100R001 c40163 2007-8-14 BT modify end */ 
	...
	/* HG586V100R001 c40163 2007-8-14 BT add begin */
    fw=!strcmp(argv[1],"fw");
	/* HG586V100R001 c40163 2007-8-14 BT add end */ 
    ...
    */
    if(!ip && !mac && !masq && !fw) {
      fprintf(stderr,"ip, masq, mac or fw must be specified\n");
      return -1;
    }	
	/* HG586V100R001 c40163 2007-8-14 BT modify end */ 
    ...
    if(qdisc) {
      ...
      if(argc<1) {
        fprintf(stderr,"... <mac>|<ip>|<masq>|<fw> ...\n");
        return -1;
      }
      memset(opt.addr,0,sizeof(opt.addr));

      /* c04163 modify */
      if (sscanf(argv[0], "%i:%x:%x", &a0, &a1, &a2) == 3) {
        if ((a0 <= 0) || (a0 > 255) || (a1 <= 0) || (a2 <= 0)) {
          fprintf(stderr,"Invalid value of fw mark\n");
          return -1;
        }
        opt.addr[0]=a0; opt.addr[1]=a1; opt.addr[2]=a2;
      }
      else if (sscanf(argv[0],"%i.%i.%i.%i",&a0,&a1,&a2,&a3) == 4) {
        opt.addr[0]=a0; opt.addr[1]=a1;
        opt.addr[2]=a2; opt.addr[3]=a3;
	  }
      else if (sscanf(argv[0],"%x:%x:%x:%x:%x:%x",&a0,&a1,&a2,&a3,&a4,&a5) == 6) {
        opt.addr[0]=a0; opt.addr[1]=a1; opt.addr[2]=a2;
        opt.addr[3]=a3; opt.addr[4]=a4; opt.addr[5]=a5;
      }
      else {
        fprintf(stderr,"Wrong format of mac, ip address or fw mark\n");
        return -1;
      }
