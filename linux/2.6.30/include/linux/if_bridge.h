...
#define BRCTL_SET_DEV_RELAY 35
#endif

#define BRCTL_SET_FLOWS              28  //add from bcm4.12L07 
...
extern void hi_br_fdb_put(struct net_bridge_fdb_entry *ent);
#else
extern int hi_kenrel_br_get_port(int ifindex, unsigned char *addr); 
extern int hi_kernel_br_find_mac(struct net_device *dev, unsigned char *addr);
extern struct net_bridge_fdb_entry *hi_kernel_br_find_fdb(struct net_device *dev, unsigned char *addr);
extern void hi_kernel_br_fdb_put(struct net_bridge_fdb_entry *ent);
#endif
#endif
#endif
