...
#define SKBSHINFO_DIRTYP_ACK         0x20000000
#define IS_SKBSHINFO_DIRTYP_ACK(d)  (0xa0000000 == (0xf0000000 & (uint32)(d)))
#define SET_SKBSHINFO_DIRTYP_ACK(d) do { \
...
typedef struct ctf_brc_hot {
    ...
    uint16_t            idx; /* table idx */
    uint16_t            prio_bitmap;
    uint32_t            wl_handle;
	uint8_t             in_use; /* Is this entry in-use */
} __attribute__((packed)) ctf_brc_hot_t;
#else
typedef struct ctf_brc_hot {
    struct _macaddr       ea;     /* Dest mac addr */
    struct net_device *tx_dev;    /* Dev to be sent */
...
#define WLAN_DEVICE_MAX 2  // supported max wlan adapter number

typedef struct pktc_devhandle {
    uint32_t   handle;  /* wlan handle */ 
    uint32_t   dev;     /* dev associated with the wlan handle */
} pktc_devhandle_t;
...
#define CTF_BRC_HOT_HASH(da)    ((((uint8_t *)da)[1] ^ ((uint8_t *)da)[3] ^ ((uint8_t *)da)[5]) % ((TOTAL_CHAIN_ENTRY_NUM)/2))
#define CTF_BRC_HOT_HASH_2(da)  (((((uint8_t *)da)[0] ^ ((uint8_t *)da)[2] ^ ((uint8_t *)da)[4] ) % ((TOTAL_CHAIN_ENTRY_NUM)/2)) + ((TOTAL_CHAIN_ENTRY_NUM)/2))
...
/*--- members common to fkbuff: end here ---*/
	 ...
	 */
	char			cb[48];
#if 1//defined(CONFIG_IMQ) || defined(CONFIG_IMQ_MODULE)
...
#if defined(CONFIG_MIPS_BRCM) && defined(CONFIG_BLOG) && ((defined(CONFIG_PPPOL2TP) && defined(CONFIG_L2TP_FAST_FORWARD)) || defined(CONFIG_PPTP_TUNNEL))
	/*控制报文是否走加速, 默认0需要走加速*/
...
#ifdef CONFIG_BUILD_SD5610T  //fix me to CONFIG_HSAN future
...
#endif
#ifdef CONFIG_HSAN
#define ATP_LOG_SIZE 128
#else
#define ATP_LOG_SIZE 1024
#endif
...
extern struct sk_buff *skb_copy(const struct sk_buff *skb,
				gfp_t priority);
extern struct sk_buff *skb_copy_extend_header(const struct sk_buff *skb,
				gfp_t priority, int len);
...
extern unsigned int	skb_unlink_tpj(struct sk_buff *skb, struct sk_buff_head *list);
static inline unsigned int __skb_unlink_tpj(struct sk_buff *skb, struct sk_buff_head *list)
{
	struct sk_buff *next, *prev;
	...
	next	   = skb->next;
	prev	   = skb->prev;
	if (NULL == next || NULL == prev)//组播加速死机问题发生时，这两个指针有一个为NULL
    {
...
//为规避组播加速死机问题，暂时新添函数
extern struct sk_buff *skb_dequeue_tpj(struct sk_buff_head *list);
static inline struct sk_buff *__skb_dequeue_tpj(struct sk_buff_head *list)
{
	struct sk_buff *skb = skb_peek(list);
	if (skb)
	{
		if (0 == (__skb_unlink_tpj(skb, list)))
		{
		    return NULL;
		}
	}
