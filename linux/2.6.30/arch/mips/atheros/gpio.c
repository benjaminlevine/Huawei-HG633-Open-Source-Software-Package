...
#define SIMPLE_CONFIG_INGRESS 4
#define SIMPLE_CONFIG_INGRESS_ERROR 5
#define SIMPLE_CONFIG_OVERLAP 6
/* END ADD: c00217102 2012-8-12 FOR WS323 */

#define RED_LED_GPIO 					2
#define GREEN_LED_GPIO 					12
#define BLUE_LED_GPIO 					14
#define WLAN_SIGNAL_HIGH_LED_GPIO 		17
#define WLAN_SIGNAL_MID_LED_GPIO 		13
#define WLAN_SIGNAL_LOW_LED_GPIO 		15
...
static int WPSled = WPS_LED_ON, sec = 0, on=2,time=9,off=5,change=0;
...
static void wps_success_func()
{
	syswatch_nl_send(ATP_MSG_MONITOR_EVT_WPS_SUCCESS, NULL, 0);
}
...
int ath_get_wlan_ctrl_state(void)
{
	return (LED_STATE_OFF == simple_config_led_state) ? 0 : 1;
}
EXPORT_SYMBOL(ath_get_wlan_ctrl_state);
