...
static int __init bcm6368_hw_init(void)
{
    ...
    if( BpGetGPIOverlays((unsigned short *)&GPIOOverlays) == BP_SUCCESS ) {
...
static int __init bcm6362_hw_init(void)
{
    ...
    if ( BpGetFemtoResetGpio(&gpio) == BP_SUCCESS ) {
        kerSysSetGpio(gpio, kGpioActive);
    }
...
static int __init bcm63268_hw_init(void)
{
    ...
    /*Added by l00135113@20120321 暂时去掉对dsl节能模块*/
#if 0
	...
	/* HG658b STC: Ethernet灯由软件处理，禁止硬件控制*/
    ...
    if( BpGetGPIOverlays(&GPIOOverlays) == BP_SUCCESS ) 
	{
		...
		/*start of modify by y00181549 for W724v USB power supply at 2012.5.28*/
		...
		/*end of modify by y00181549 for W724v USB power supply at 2012.5.28*/
    }

    /*Start add for HG659v3 lsw spi cs signal change to SSB5 by w00182844 @20130328*/
    if(BpGetEthernetMacInfo(&EnetInfo[0], BP_MAX_ENET_MACS) == BP_SUCCESS)
    {
        if (EnetInfo[1].usConfigType == BP_ENET_CONFIG_HS_SPI_SSB_5)
        {
            GPIO->GPIOMode |= GPIO_MODE_HS_SPI_SS_5;
        }
    }
    ...
    /*End add for HG659v3 lsw spi cs signal change to SSB5 by w00182844 @20130328*/


// 这里设置SW 	MAC 地址，后续需要使用使用本方法
#if 0
    {
...
/*Added by l00135113@20120811 for bcm 63268 CoC to shutdown 1v8 register*/
#if defined(CONFIG_BCM_1V8REG_ALWAYS_SHUTDOWN)
    printk("Internal 1P8 VREG is forced to be shutdown\n");
...
/*Added by l00135113@20120811 for bcm 63268 CoC to shutdown 1v8 register*/
	...
	// Enable GPIO of Internet-LED
	*(volatile unsigned int *)(0xB00000D8) &= ~(1 << 12);
    
    ATP_LED_CtrlLed(kLedPower, kLedStateOn);
...
static int __init bcm6318_hw_init(void)
{
...
#if defined(CONFIG_USB)
    //START ADD: l81004871 FOR 解决没有USB的版本在不断电的情况下切换到带USB版本在内核启动到USB时挂掉问题 2013-01-30
    ...
    if( 0 == strcmp("96318_hg532d", boardIdStr) )
    {
	    setPinMuxGpio(BP_GPIO_40_AH);
	    GPIO->GPIODir |= GPIO_NUM_TO_MASK(BP_GPIO_40_AH);
        GPIO->GPIOio ^= GPIO_NUM_TO_MASK(BP_GPIO_40_AH);
    }
    ...
    if (EnetInfo[0].sw.port_map & (1 << 0)) {
		...
		LED->ledXorReg &= ~(1 << EPHY0_ACT_LED);  /*modify for BCM96318B0 chips by w00182844 @10/29/2012*/
    ...
    if (EnetInfo[0].sw.port_map & (1 << 1)) {
		...
		LED->ledXorReg &= ~(1 << EPHY1_ACT_LED);  /*modify for BCM96318B0 chips by w00182844 @10/29/2012*/
    ...
    if (EnetInfo[0].sw.port_map & (1 << 2)) {
		...
		LED->ledXorReg &= ~(1 << EPHY2_ACT_LED);  /*modify for BCM96318B0 chips by w00182844 @10/29/2012*/
    ...
    if (EnetInfo[0].sw.port_map & (1 << 3)) {
		...
		LED->ledXorReg &= ~(1 << EPHY3_ACT_LED);  /*modify for BCM96318B0 chips by w00182844 @10/29/2012*/
