...
{
#ifdef CONFIG_HIMEM_DUMPFILE
       char acSymbolWhere[KSYM_SYMBOL_LEN]={0};
       char acSymbolFrom[KSYM_SYMBOL_LEN]={0};
...
#ifdef CONFIG_HIMEM_DUMPFILE    
       sprint_symbol(acSymbolWhere, where);      
       sprint_symbol(acSymbolFrom, from);
#endif       
    
#ifdef CONFIG_HIMEM_DUMPFILE
       hw_ssp_set_dump_info("[<%08lx>] (%s)", where, acSymbolWhere);
       hw_ssp_set_dump_info("from [<%08lx>] (%s)\n", from, acSymbolFrom);
#endif
...
#ifdef CONFIG_HIMEM_DUMPFILE
       hw_ssp_set_dump_info("%s(0x%08lx to 0x%08lx)\n", str, bottom, top);
#endif
	for (p = bottom & ~31; p < top;) {
		printk("%04lx: ", p & 0xffff);
#ifdef CONFIG_HIMEM_DUMPFILE
              hw_ssp_set_dump_info("%04lx: ", p & 0xffff);
#endif
...
#ifdef CONFIG_HIMEM_DUMPFILE                     
                    hw_ssp_set_dump_info(i == 0 ? "(%0*x) " : "%0*x ", width, val);
#endif                  
              }
		else {
			printk("bad PC value.");
#ifdef CONFIG_HIMEM_DUMPFILE
                     hw_ssp_set_dump_info("bad PC value.");
#endif                    
			break;
		}
	}
...
{
#ifdef CONFIG_HIMEM_DUMPFILE
      hw_ssp_set_dump_info(KERN_CRIT"kernel BUG at %s:%d!\n", file, line);
#endif        
