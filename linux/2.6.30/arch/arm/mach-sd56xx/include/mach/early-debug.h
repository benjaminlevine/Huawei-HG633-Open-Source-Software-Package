...
#define hsan_trace(level,s,param...)

#else /* (CONFIG_HSAN_EDB_LEVEL<255) */


#define edb_init()	do{ \
				edb_putstr("Kernel Early-Debug on Level "); \
				edb_putul(CONFIG_HSAN_EDB_LEVEL);\
				edb_putstr(" \n"); \
			}while(0)
...
#define EDB_TRACE_LEVEL CONFIG_HSAN_EDB_LEVEL
...
#define hsan_trace(level,s,param...) do{ \
		if(level >= EDB_TRACE_LEVEL)\
			printk(KERN_INFO "[%s, %d]\t" s "\n",__FUNCTION__,__LINE__, param);	}while(0)

#endif /* (CONFIG_HSAN_EDB_LEVEL==255) */
