						    ...
						    struct jffs2_inode_cache *ic)
{
	...
	if ((ofs & ~3) > c->sector_size - jeb->free_size) {
		...
		if(0 != jffs2_scan_dirty_space(c, jeb, (ofs & ~3) - (c->sector_size - jeb->free_size))) {
			printk("%s %d %s: failed to call jffs2_scan_dirty_space() \n", __FILE__, __LINE__, __func__);
		}
	}
...
int jffs2_sum_write_sumnode(struct jffs2_sb_info *c)
{
	...
	jeb = c->nextblock;
	if(0 != jffs2_prealloc_raw_node_refs(c, jeb, 1)) {
		printk("%s %d %s: failed to call jffs2_prealloc_raw_node_refs() \n", __FILE__, __LINE__, __func__);
	}
