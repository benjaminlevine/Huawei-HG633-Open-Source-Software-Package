	...
	__acquires(rq->lock)
{
	...
	for (;;) {
		...
		/* Modified by y00183935 for avoiding the problem: Kernel panic for CPU id changed to be an invalid value when the task wake up. Test operation: plug out usb storage disk when write a big file into a ntfs partition with ntfs-3g. */
