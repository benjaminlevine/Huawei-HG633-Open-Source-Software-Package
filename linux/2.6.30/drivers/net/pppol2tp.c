...
static int pppol2tp_xmit(struct ppp_channel *chan, struct sk_buff *skb)
{
    ...
    inet->daddr = session->tunnel_addr.addr.sin_addr.s_addr;
    inet->dport  = session->tunnel_addr.addr.sin_port;
	...
	if (sk_tun->sk_no_check == UDP_CSUM_NOXMIT)
		skb->ip_summed = CHECKSUM_NONE;
	//else if (skb->dst && !(skb->dst->dev->features & NETIF_F_V4_CSUM)) {
	else if ((skb->dst && skb->dst->dev) && /* 内核bug，没有判断skb->dst非空，导致挂死 */
		 (!(skb->dst->dev->features & NETIF_F_V4_CSUM))) {
     
		skb->ip_summed = CHECKSUM_COMPLETE;
		csum = skb_checksum(skb, 0, udp_len, 0);
		uh->check = csum_tcpudp_magic(skb->rtable->rt_src, skb->rtable->rt_dst, /* 由于inet->saddr不是实际的源IP，导致udp校验和不对 */
					      udp_len, IPPROTO_UDP, csum);
		if (uh->check == 0)
			uh->check = CSUM_MANGLED_0;
	} else {
					...
					int fd, u16 tunnel_id, int *error)
{
	 ...
	 */
	 /* 调用函数pppol2tp_prepare_tunnel_socket之后还会进行tunnel->ref_count增加，去掉此处增加 */
			    ...
			    int sockaddr_len, int flags)
{
	...
	if ((sp->pppol2tp.s_session == 0) && (sp->pppol2tp.d_session == 0)) {
		...
		if (tunnel_sock == NULL)
			goto end;
        /*start by d00191326 需要把地址重新指定，否则目的地址始终是0  */
        inet = inet_sk(tunnel_sock);
        inet->daddr = sp->pppol2tp.addr.sin_addr.s_addr;
        inet->dport = sp->pppol2tp.addr.sin_port;
        /*End by d00191326 需要把地址重新指定，否则目的地址始终是0  */
