...
static int bond_mode	= BOND_MODE_MAINDEV;
...
static const char *bond_mode_name(int mode)
{
	static const char *names[] = {
        ...
        [BOND_MODE_MAINDEV] = "main device prior",		
	};

	if (mode < 0 || mode > BOND_MODE_MAINDEV)
		return "unknown";
...
static int bond_xmit_maindevprior(struct sk_buff *skb, struct net_device *bond_dev)
{
	struct bonding *bond = netdev_priv(bond_dev);
	struct slave *slave, *start_at;
	int i, res = 1;
	...
	/*
	 * transmit from the main device when the DSL threshold is not overflow,
	 * otherwise pick up a backeup device random.
	 */

    start_at = bond->first_slave;
	bond_for_each_slave_from(bond, slave, i, start_at) {
		/* !HSI */
	    if (!(((hi_kernel_eth0_cb_s*)(skb->hi_cb))->u_rx_lineinfo.bit.ui_hsi))
		{
			/* select the main device */
            if (!IS_MAIN(slave->dev))
            {
                continue;
            }
		}
		...
		{
			if (COLOR_GREEN != ((hi_kernel_eth0_cb_s*)(skb->hi_cb))->u_rx_lineinfo.bit.ui_color)
			{
			...
			skb->accelmark |= GRE_MARK;
		}


        if (slave && slave->dev && slave->dev->name)
...
static int bond_start_xmit(struct sk_buff *skb, struct net_device *dev)
{
    ...
    case BOND_MODE_MAINDEV:
        return bond_xmit_maindevprior(skb, dev);        
	default:
