...
修改记录:
问题单号             修改时间    修改人         修改描述
DTS2013011003247    20130110    x81004082     b880未进行数据加速处理
DTS2013011408161    20130125    z00185914     将hw_cdc_driver中的中断处理移到ndistty中，新增hw_cdc_driver.h文件
DTS2013020502561    20130205    x81004082     第一次配置profile时产生内核警告
DTS2013020502449    20130205    x81004082     单板插上miniUSB时内核产生空指针
...
/* <DTS2013011003247   x81004082 2013/1/10 begin*/
...
/* DTS2013011003247   x81004082 2013/1/10 end> */
/* DTS2013011408161 move urb request to ndistty z00185914 20130125 DELETE 15 lines */
/* <DTS2013011003247   x81004082 2013/1/10 begin*/
#define IP4_HDR     (0x45)
#define IP6_HDR     (0x60)
/* DTS2013011003247   x81004082 2013/1/10 end> */
...
static const char driver_name [] = "rmnet_balong";
/* <DTS2013011003247   x81004082 2013/1/10 begin*/
...
/* DTS2013011408161 move urb request to ndistty z00185914 20130125 DELETE 40 lines */
...
/* < DTS2013011706280 improve NDIS dial up slowly z00185914 20130119 delete 10 row */
...
/* DTS2013011408161 move urb request to ndistty z00185914 20130125 DELETE 100 lines */
...
/* DTS2013011408161 move urb request to ndistty z00185914 20130125 DELETE 60 lines */
...
/* DTS2013011408161 move urb request to ndistty z00185914 20130125 DELETE 20 lines */
////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
/* <DTS2013010905391 add ndistty driver z00185914 20130112 BEGIN */
    ((void)printk(KERN_ERR "Hw_cdc_driver LINE:%d ######: " fmt "\n" , __LINE__,## arg))
...
//#ifdef CONFIG_USB_QMITTY_MODULE
     set_rmnet_intr_ep(&(out->desc));
     set_rmnet_device(dev->udev, alt);
/* <DTS2013011408161 move urb request to ndistty z00185914 20130125 BEGIN */
    set_hwcdc_device(dev, alt);
/* DTS2013011408161 move urb request to ndistty z00185914 20130125 END> */
//#endif
...
static void rx_tlp_parse(struct hw_cdc_net *dev, struct sk_buff *skb)
{
    ...
    while (remain_bytes > 0){
        switch (dev->hw_tlp_buffer_state){
            ...
            {
                pktlen = *((unsigned short*)dev->hw_tlp_tmp_buf.buffer);
                payload_ptr = cur_ptr;
                reset_tlp_tmp_buf(dev);
                /*validate the tlp packet header*/
                if (HW_TLP_BITS_SYNC != (pktlen & HW_TLP_MASK_SYNC)){
                    devdbg(dev, "The pktlength is error");                
                    dev->hw_tlp_buffer_state = HW_TLP_BUF_STATE_ERROR;
                    break;
                }
                pktlen &= HW_TLP_MASK_LENGTH;
                if ( (payload_ptr + pktlen) <= buf_end){
                    submit_skb(dev, payload_ptr, pktlen);
                    cur_ptr = payload_ptr + pktlen;
                    remain_bytes = buf_end - cur_ptr; 
...
int hw_get_settings (struct net_device *net, struct ethtool_cmd *cmd)
{
    ...
    printk("\nComehere===LINE = %d; FILE = %s; FUNCTION = %s=cmd=[%d]==\n",__LINE__,__FILE__,__FUNCTION__,cmd->cmd);
...
/* <DTS2013010905391 add ndistty driver z00185914 20130112 BEGIN */
    snprintf(info->driver, sizeof(info->driver), "%s", driver_name);
/* DTS2013010905391 add ndistty driver z00185914 20130112 END> */
    snprintf(info->version, sizeof(info->version), "%s", DRIVER_VERSION);
    snprintf(info->fw_version, sizeof(info->fw_version), "%s", dev->driver_desc);
...
//#endif
    ...
    /* <DTS2013020502449   x81004082 2013/2/5 begin*/
    netif_carrier_off(dev->net);
    netif_device_detach (dev->net);
    /* DTS2013020502449   x81004082 2013/2/5 end> */
...
//    DECLARE_MAC_BUF(mac);
    ...
    g_usbnet = alloc_netdev(sizeof(struct hw_cdc_net), "rmnet_balong%d", ether_setup);
    if (!g_usbnet) {
           printk ("hw_cdc_init:can't kmalloc dev\n");
           goto out;
    }
...
//#if !(LINUX_VERSION_CODE < KERNEL_VERSION(2, 6, 30))
    g_usbnet->netdev_ops = &hw_netdev_ops;
//#else
    g_usbnet->change_mtu = hw_change_mtu;
    g_usbnet->get_stats = hw_get_stats;
    g_usbnet->hard_start_xmit = hw_start_xmit;
    g_usbnet->open = hw_open;
    g_usbnet->stop = hw_stop;
    g_usbnet->tx_timeout = hw_tx_timeout;
//#endif
    g_usbnet->watchdog_timeo = TX_TIMEOUT_JIFFIES;
    g_usbnet->ethtool_ops = &hw_ethtool_ops;
    ...
    if (status){
        printk ("hw_cdc_init:can't register netdev\n");
        goto out3;
    }
    ...
    netif_carrier_on(g_usbnet);
    /*zhangtai*/
    trigger_tty();
#if 0

    printk("\nbcdDevice=0x%x\n",le16_to_cpu(dev->udev->descriptor.bcdDevice));
    printk("\nbInterfaceSubClass=0x%x\n",udev->cur_altsetting->desc.bInterfaceSubClass);
    if (HW_JUNGO_BCDDEVICE_VALUE != le16_to_cpu(dev->udev->descriptor.bcdDevice) 
...
static int hw_cdc_bind(struct hw_cdc_net *dev, struct usb_interface *intf)
{
    ...
    while (len > 3) {
        ...
        case USB_CDC_HEADER_TYPE:
            if (info->header) {
                devdbg(&intf->dev, "extra CDC header\n");
                goto bad_desc;
            }
            info->header = (void *) buf;
            if (info->header->bLength != sizeof *info->header) {
                devdbg(&intf->dev, "CDC header len %u\n",
                    info->header->bLength);
                goto bad_desc;
            }
            break;
        case USB_CDC_UNION_TYPE:
            if (info->u) {
                devdbg(&intf->dev, "extra CDC union\n");
                goto bad_desc;
            }
            info->u = (void *) buf;
            if (info->u->bLength != sizeof *info->u) {
                devdbg(&intf->dev, "CDC union len %u\n",
                    info->u->bLength);
                goto bad_desc;
            }
            ...
            if (info->control != intf) {
                devdbg(&intf->dev, "bogus CDC Union\n");
        ...
        case USB_CDC_ETHERNET_TYPE:
            if (info->ether) {
                devdbg(&intf->dev, "extra CDC ether\n");
                goto bad_desc;
            }
            info->ether = (void *) buf;
            if (info->ether->bLength != sizeof *info->ether) {
                devdbg(&intf->dev, "CDC ether len %u\n",
                    info->ether->bLength);
                goto bad_desc;
            }
        ...
        case USB_CDC_NCM_TYPE:
            if (dev->ncm_ctx->ncm_desc){
                devdbg(&intf->dev, "extra NCM descriptor\n");
            }else{
...
next_desc:
    ...
    if (!info->header || !info->u || (!dev->is_ncm &&!info->ether) ||
        ...
        devdbg(&intf->dev, "missing cdc %s%s%s%sdescriptor\n",
            info->header ? "" : "header ",
            info->u ? "" : "union ",
            info->ether ? "" : "ether ",
    ...
    if(dev->is_ncm)
    {
        ...
        if (ctx->skb_pool == NULL) {
            devdbg(&intf->dev, "failed allocating the SKB pool\n");
            goto error2;
        }
        ...
        for (i = 0; i < ctx->skb_pool_size; i++) {
            ...
            if (ctx->skb_pool[i] == NULL) {
                devdbg(&intf->dev, "failed allocating an SKB for the "
                    "SKB pool\n");
                goto error3;
            }
        }
...
static int __init hw_cdc_init(void)
{
    ...
    ret = register_netdev (g_usbnet);
    if (ret){
        printk ("hw_cdc_init:can't register netdev\n");
        goto out1;
    }
#endif
    ret = usb_register(&hw_ether_driver);

    if(ret)
    {
        printk ("hw_cdc_init:can't register usbdev\n");
//        goto out2;
    }
