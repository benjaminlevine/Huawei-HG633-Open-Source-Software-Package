...
<修改人>      <时间>      <问题单号>         <简要描述>
x00100244     2014/09/24  DTS2014091903319   HG255s新产品开发，部分装备命令适配修改
===========================================================================*/
								...
								int *eof, void *data)
{
    ...
    len += sprintf (page + len, "%s","Diskname Vendor       ID       Type   ConnectionType Capacity Port\r\n");
    for(i = 0;i < MAX_DISKS;i++)
    {
        if(1 == ((struct usb_diskinfo *)diskinfos+i)->isvalid)
        {
            ...
            len += sprintf (page + len, "%-14s ", replacespace(disk->connectiontype));
            len += sprintf (page + len, "%-8u ", disk->capacity);
#ifdef SUPPORT_ATP_USBPORT_ONLYONE
            len += sprintf (page + len, "%-6s ", "1-1");
#else
            len += sprintf (page + len, "%-6s ", disk->usbport);
#endif
...

        {
          ...
          //add by zhouyu 2010-11-25 解决相同ID的USB设备web上只能识别一个的问题
              ...
              if (us->pusb_dev->serial)
              {
                  ...
                  {
                      n = 4 - length;
                      /* <DTS2014091903319 x00100244 2014/9/24 begin, 安全编码修改 */
                      snprintf(serialnum ,sizeof(serialnum),"%s" , us->pusb_dev->serial);                      
                      strncpy(sero , "0000" , n);
                      strncat(serialnum , sero,(sizeof(serialnum)-strlen(serialnum)-1));
					  /* DTS2014091903319 x00100244 2014/9/24 end> */
                  }
                  else
                  {
                      /* <DTS2014091903319 x00100244 2014/9/24 begin, 安全编码修改 */
          ...
          //delete by zhouyu 20101213统一LAN方式，经处理后的ID仍然相同则不再作加1处理
            ...
            //BT项目，为了和lan信息统一，把ID中的字母规范为大写字母
              ...
              //最终写入proc文件的ID       
              /* <DTS2014091903319 x00100244 2014/9/24 begin, 安全编码修改 */
			  memset(disk->model,0,STRLEN);
              snprintf(disk->model,STRLEN , "%s",DeviceId);
              /* DTS2014091903319 x00100244 2014/9/24 end> */
              printk("     device id: %s\n", disk->model);
          //add by zhouyu 2010-11-25 解决相同ID的USB设备web上只能识别一个的问题
             ...
             /*diskname,sda,sdb,...*/            
            	printk("       diskname: %s\n", gd->disk_name);
            /* <DTS2014091903319 x00100244 2014/9/24 begin, 安全编码修改 */
			memset(disk->diskname,0,SHORT_STRLEN);
        	snprintf(disk->diskname,SHORT_STRLEN-1,"%s",gd->disk_name);
            /* DTS2014091903319 x00100244 2014/9/24 end> */
...
#endif			
            /* <DTS2014091903319 x00100244 2014/9/24 begin, 安全编码修改 */
            memset(disk->vendor,0,STRLEN);
        	snprintf(disk->vendor,STRLEN-1,string);
            
            
			memset(disk->serial,0,STRLEN);
            snprintf(disk->serial , STRLEN, "%d",giInterfaceClass);
            /* DTS2014091903319 x00100244 2014/9/24 end> */
...
/* <DTS2014091903319 x00100244 2014/9/24 begin, HG255s新产品开发，部分装备命令适配修改 */
#if (defined CONFIG_HSAN)
             memset(disk->connectiontype,0,SHORT_STRLEN);
			 if(USB_SPEED_SUPER == us->pusb_dev->speed )
			 {
                 snprintf(disk->connectiontype,SHORT_STRLEN - 1,"%s","USB3.0");
			 }
			 else if(USB_SPEED_WIRELESS == us->pusb_dev->speed )
			 {
                 snprintf(disk->connectiontype,SHORT_STRLEN - 1,"%s","USB2.5");
			 }
			 else
#else
			 if(USB_SPEED_VARIABLE == us->pusb_dev->speed )
			 {
                 snprintf(disk->connectiontype,SHORT_STRLEN - 1,"%s","USB2.5");
			 }
			 else
             ...
             if(USB_SPEED_HIGH == us->pusb_dev->speed )
             {
                 ...
                 snprintf(disk->connectiontype,SHORT_STRLEN - 1,"%s","USB2.0");
             }
             else 
             {
                 snprintf(disk->connectiontype,SHORT_STRLEN - 1,"%s","USB1.1");
             }
/* DTS2014091903319 x00100244 2014/9/24 end> */
...
#endif
/*added by w00190448 @20120714 解决读取u盘容量问题。之前一直是读取的0.end*/
/*add by y181549 for identification of different usb port for equipment at 20120914*/
            /* <DTS2014091903319 x00100244 2014/9/24 begin, 安全编码修改 */
            memset(disk->usbport,0,SHORT_STRLEN);
            snprintf(disk->usbport,SHORT_STRLEN,"%s",dev_name(&(us->pusb_dev->dev)));
            /* DTS2014091903319 x00100244 2014/9/24 end> */
            printk("insert in %s\r\n",dev_name(&(us->pusb_dev->dev)));
/*add by y181549 for identification of different usb port for equipment at 20120914*/
       }
...
#endif
/*Add end, l67944, 20100625*/
#if defined(CONFIG_BCM96362) || defined(CONFIG_BCM963268) || defined(CONFIG_BCM96318) || defined(CONFIG_HISI_CHIP)
	g_USB_PROBE = 1;
    printk("\n\n\n\nsd_probe: USB insert!!\n\n\n\n");
#endif
