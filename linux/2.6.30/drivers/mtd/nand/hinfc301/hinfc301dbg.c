...
{
#  ifdef CONFIG_MTD_HISI_SD5115_NFC_DBG_PROC_FILE
	printk("nand debug: CONFIG_MTD_HISI_SD5115_NFC_DBG_PROC_FILE\n");
#  endif /* defined(CONFIG_MTD_HISI_SD5115_NFC_DBG_PROC_FILE) */

#  ifdef CONFIG_MTD_HISI_SD5115_NFC_DBG_EC_NOTICE 
	printk("nand debug: CONFIG_MTD_HISI_SD5115_NFC_DBG_EC_NOTICE\n");
#  endif /* CONFIG_MTD_HISI_SD5115_NFC_DBG_EC_NOTICE */
}
#elif defined(CONFIG_HISI_SD5610)
static void dbg_nand_ec_init(void)
{
#  ifdef CONFIG_MTD_HISI_SD5610_NFC_DBG_PROC_FILE
	printk("nand debug: CONFIG_MTD_HISI_SD5610_NFC_DBG_PROC_FILE\n");
#  endif /* defined(CONFIG_MTD_HISI_SD5610_NFC_DBG_PROC_FILE) */

#  ifdef CONFIG_MTD_HISI_SD5610_NFC_DBG_EC_NOTICE 
	printk("nand debug: CONFIG_MTD_HISI_SD5610_NFC_DBG_EC_NOTICE\n");
#  endif /* CONFIG_MTD_HISI_SD5610_NFC_DBG_EC_NOTICE */
}
#endif
/*****************************************************************************/
static void dbg_nand_ec_notice(struct hinfc_host *host)
{
	...
	val = (oob[0] & oob[1] & oob[2] & oob[3] & oob[4] & oob[5] & oob[6] & oob[7]);
	...
	{
#if defined(CONFIG_MTD_HISI_SD5115_NFC_DBG_EC_NOTICE) || defined(CONFIG_MTD_HISI_SD5610_NFC_DBG_EC_NOTICE)
		int ix = 0;
		int jx = 0;
		int nBits = 0;
		...
		printk("!!! nand uncorrectable error, page:0x%08X, (%d/256 bits zero)\n",
...
******************************************************************************/
#if defined(CONFIG_MTD_HISI_SD5115_NFC_DBG_PROC_FILE) || defined(CONFIG_MTD_HISI_SD5610_NFC_DBG_PROC_FILE)

#  include <linux/proc_fs.h>
#  include <linux/module.h>
#  include <linux/moduleparam.h>

#  ifndef CONFIG_HINFC301_DBG_NAND_NUM_OF_LOGS
#    define CONFIG_HINFC301_DBG_NAND_NUM_OF_LOGS      (80)
#  endif /* CONFIG_HINFC301_DBG_NAND_NUM_OF_LOGS */

#  ifndef CONFIG_HINFC301_DBG_NAND_LOG_LENGTH
#    define CONFIG_HINFC301_DBG_NAND_LOG_LENGTH       (40)
#  endif /* CONFIG_HINFC301_DBG_NAND_LOG_LENGTH */

#  ifndef CONFIG_HINFC301_DBG_NAND_DATA_LENGTH
#    define CONFIG_HINFC301_DBG_NAND_DATA_LENGTH      (8)
#  endif /* CONFIG_HINFC301_DBG_NAND_DATA_LENGTH */
...
struct dbg_nand_logs_t
{
	...
	unsigned char  data[CONFIG_HINFC301_DBG_NAND_LOG_LENGTH];
};
...
struct dbg_nand_proc_t
{
	...
	struct dbg_nand_logs_t logs[CONFIG_HINFC301_DBG_NAND_NUM_OF_LOGS];
...
{
	.length = CONFIG_HINFC301_DBG_NAND_DATA_LENGTH,
};
/*****************************************************************************/
/*
...
)
{
	...
	*eof = 1;
	...
	if (off == 0)
	{
		if (dbg_nand_proc.NumOfLogs < CONFIG_HINFC301_DBG_NAND_NUM_OF_LOGS)
		{
	...
	for (read_index = nand_proc->read_index;
		...
		++read_index)
	{
		if (read_index >= CONFIG_HINFC301_DBG_NAND_NUM_OF_LOGS)
			read_index = 0;
		...
		ppage += sprintf(ppage,
			...
			logs->hour, logs->min, logs->sec, logs->msec, logs->op,
		...
		else
		{
			...
			for (ix = 0; ix < logs->length; ix++)
			{
				if ((ix % 16) == 15)
				{
					ppage += sprintf(ppage, "%02X-", logs->data[ix]);
				}
				else
				{
					ppage += sprintf(ppage, "%02X ", logs->data[ix]);
...
static int dbg_nand_proc_init(void)
{
	...
	printk("nand debug: CONFIG_MTD_HISI_SD5xxx_NFC_DBG_PROC_FILE\n");
...
{
	struct dbg_nand_logs_t *logs = &dbg_nand_proc.logs[dbg_nand_proc.index];
...
#  endif /* DBG_NAND_CORRECT_ERROR */
	...
	if (*op != 'E')
	{
		...
		if (logs->length > CONFIG_HINFC301_DBG_NAND_LOG_LENGTH)
			logs->length = CONFIG_HINFC301_DBG_NAND_LOG_LENGTH;
	...
	if (++dbg_nand_proc.index >= CONFIG_HINFC301_DBG_NAND_NUM_OF_LOGS)
	{
...
#if defined(CONFIG_MTD_HISI_SD5115_NFC_DBG_STAT_PROC_FILE) || defined(CONFIG_MTD_HISI_SD5610_NFC_DBG_STAT_PROC_FILE)
									...
									unsigned long count, void *data)
{
	...
	unsigned long value;
	unsigned long pos = 0;
	struct dbg_nand_stat_proc_t *nand_stat = (struct dbg_nand_stat_proc_t *)data;
	...
	while (pos < count)
	{
		...
		{
		case 'o':
			if (!memcmp(&buf[pos], CMD_WORD_OFFSET, sizeof(CMD_WORD_OFFSET) - 1)) 
			{
				pos += sizeof(CMD_WORD_OFFSET) - 1;
				p = (char *)(buf + pos);
				if ((value = simple_strtoul(p, &p, 16)) < 0)
					value = 0;

				value /= nand_stat->page_size;
				nand_stat->start_block = value / nand_stat->num_page_block;
				nand_stat->start_page  = 
					(nand_stat->start_block * nand_stat->num_page_block);

				memset(nand_stat->page, 0, sizeof(nand_stat->page));
				memset(nand_stat->block, 0, 
					nand_stat->block_num * sizeof(unsigned short));
			}
			break;
		case 'c':
			if (!memcmp(&buf[pos], CMD_WORD_CLEAN, sizeof(CMD_WORD_CLEAN) - 1))
			{
				memset(nand_stat->page, 0, sizeof(nand_stat->page));
				memset(nand_stat->block, 0, 
					nand_stat->block_num * sizeof(unsigned short));
...
/*****************************************************************************/
static int dbg_nand_stat_read_proc
(
	char *page, 
	char **start, 
	off_t off, 
...
)
{
	...
	*eof = 1;
	...
	if (off == 0)
	{
		...
		ppage += sprintf(ppage, "Statistic address: 0x%08lX - 0x%08lX\n",
			(nand_stat->start_page * nand_stat->page_size),
			((nand_stat->start_page + nand_stat->page_num) * nand_stat->page_size));
	}

	for (flash_page = nand_stat->read_index; 
		(flash_page < nand_stat->page_num) && ((ppage - page) < (count - 100));
		flash_page++)
	{
		if (!(flash_page % nand_stat->num_page_block))
		{
			ppage += sprintf(ppage, "address     page  read  write  erase\n"); 
			ppage += sprintf(ppage, "0x%08lX  %-5lu %-5u %-5u  %-5u\n",
				((nand_stat->start_page + flash_page) * nand_stat->page_size),
				(nand_stat->start_page + flash_page),
				nand_stat->page[flash_page].read,
				nand_stat->page[flash_page].write,
				nand_stat->block[flash_page/nand_stat->num_page_block]);
		}
		else
		{
...
static int dbg_nand_stat_proc_init(int page_size, int block_size)
{
	...
	dbg_nand_stat_proc.block = (unsigned short *)
		kmalloc(dbg_nand_stat_proc.block_num * sizeof(unsigned short), GFP_KERNEL);
	...
	printk("nand debug: CONFIG_MTD_HISI_SDXXXX_NFC_DBG_STAT_PROC_FILE\n");
