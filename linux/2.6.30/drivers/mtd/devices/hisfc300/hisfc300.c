...
static void hisfc200_map_iftype_and_clock(struct hisfc_spi *spi)
{
	...
	for (jx = 0; (jx < MAX_SPI_OP) && spi->erase[jx].iftype; jx++)
	{
		...
		DBG_MSG("map: spi->erase[%d].clock:0x%08X size:%s\n",
			jx, spi->erase[jx].clock, int_to_size(spi->erase[jx].size));
	}
}
#endif
/*****************************************************************************/
...
static void hisfc200_enable_4byte_mode(struct hisfc_host *host, struct hisfc_spi *spi, 
	int enable)
{
...
static int hisfc200_reg_read_one_buffer(struct hisfc_host *host, struct hisfc_spi *spi,
	unsigned int offset, unsigned char *buf, int count)
{
	int num = 0;
	int regindex = 0;
	unsigned int regval;
	...
	size_t *retlen, u_char *buf)
{
	...
	*retlen = 0;
	...
	{
		if ((num = HISFC200_REG_BUF_SIZE - (from & HISFC200_REG_BUF_MASK)) > len)
		{
			num = len;
		}
...
static int hisfc200_spi_probe(struct hisfc_host *host)
{
	...
	for (chipselect = (CONFIG_MTD_HISI_SFC_CHIPNUM - 1);
		chipselect >= 0; chipselect--)	
	{
		...
		printk("Spi(cs%d) ID: 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X\n",
			chipselect,
		...
		if (spiinfo)
		{
			...
			hisfc_write(host, (HISFC200_CS0BASEADDR + (chipselect << 2)),
				(CONFIG_MTD_HISI_SFC_BUFFER_BASE_ADDRESS + total));
			...
			printk("Block:%sB ",  int_to_size(spi->erasesize));
			printk("Chip:%sB ",   int_to_size(spi->chipsize));
			printk("(Name:%s)\n", spi->name);
...
static void hi_setup_mtd_partitions(struct mtd_info *mtd, struct mtd_partition **parts, int *nr_parts)
{
	...
	if (((HI_TYPE_DOUBLE==ui_sys_type) && (SLAVE_ACTIVE==ui_upg_flag))	/* 双系统,备系统激活 */
        || ((HI_TYPE_BIG_SMALL == ui_sys_type) && (SLAVE_ACTIVE==ui_upg_flag)))
	{ 
        printk("SLAVE SYSTEM\n");
        ulMainOrSlave = 1;
    }
    ...
    atp_setup_mtd_partitions(mtd, g_pc_flash_info+2, ulMainOrSlave, parts, nr_parts);
...
static int __devinit hisfc300_driver_probe(struct platform_device * pltdev)
{
	...
	host = (struct hisfc_host *)kmalloc(sizeof(struct hisfc_host), GFP_KERNEL);
	if (!host)
		return -ENOMEM;
	...
	mtd->writesize = 256;   //设置为spiflash的页大小以提高mtd写速度
	...
	if (mtd_has_partitions())
	{
...
#else        
		hi_setup_mtd_partitions(mtd, &parts, &nr_parts);
#endif
		if (!nr_parts)
		{
		...
		if (nr_parts > 0) 
		{
			...
			result = add_mtd_partitions(host->mtd, parts, nr_parts);

#ifdef SUPPORT_ATP_WLAN
            extern int ATP_SYS_GetInitWlanRF();
...
{
	[0] = {
		.start = CONFIG_MTD_HISI_SFC_REG_BASE_ADDRESS,
		.end   = CONFIG_MTD_HISI_SFC_REG_BASE_ADDRESS + HISFC300_REG_BASE_LEN - 1,
		.flags = IORESOURCE_MEM,
	},

	[1] = {
		.start = CONFIG_MTD_HISI_SFC_BUFFER_BASE_ADDRESS,
		.end   = CONFIG_MTD_HISI_SFC_BUFFER_BASE_ADDRESS + HISFC300_BUFFER_BASE_LEN - 1,
		.flags = IORESOURCE_MEM,

	},
	[2] = {
       .start = CONFIG_MTD_HISI_SFC_PERIPHERY_REGBASE,
		.end   = CONFIG_MTD_HISI_SFC_PERIPHERY_REGBASE + HISFC300_PERIPHERY_REG_LEN - 1,
		.flags = IORESOURCE_MEM,
	},	
};
/*****************************************************************************/
...
static int __init hisfc300_module_init(void)
{
	...
	printk("Hisilicon Spi Flash Controller V300 Device Driver, Version 1.10\n");
...
MODULE_DESCRIPTION("Hisilicon Spi Flash Controller V300 Device Driver, Version 1.10");
