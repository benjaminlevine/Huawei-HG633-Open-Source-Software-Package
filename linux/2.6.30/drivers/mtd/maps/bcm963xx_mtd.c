...
#define MTD_MIDDLEWARE      12
#define MTD_WEB             13
#define MTD_WEBBAK          14
...
static struct mtd_partition bcm963xx_parts[] = {
	...
	{ name: "middleware",   offset: 0,  size: 0,    mask_flags: 0 },
    { name: "web",          offset: 0,  size: 0,    mask_flags: 0 },
    { name: "webbak",       offset: 0,  size: 0,    mask_flags: 0 },
	{ name: "all",          offset: 0,  size: 0,    mask_flags: 0 },
...
static int __init bcm963xx_mtd_init(void)
{
	...
	bcm963xx_parts[MTD_ALL].offset = 0;
	bcm963xx_parts[MTD_ALL].size = 0;
    ...
    //MiddleWare
    /* START ADD: l81004871 FOR Create MiddleWare Area 2013-02-28 */
#ifdef MIDDLE_WARE
    bcm963xx_parts[MTD_MIDDLEWARE].offset = ATP_KILO_SIZE * st_tag.flashLayout.stFlashLayoutInfo.astFlashInfo[ATP_TAG_FLASH_MIDDLE_WARE_INDEX].ulAddress;
    bcm963xx_parts[MTD_MIDDLEWARE].size = ATP_KILO_SIZE * st_tag.flashLayout.stFlashLayoutInfo.astFlashInfo[ATP_TAG_FLASH_MIDDLE_WARE_INDEX].ulLen;
#endif
    /* END ADD: l81004871 FOR Create MiddleWare Area 2013-02-28 */

    //web
    bcm963xx_parts[MTD_WEB].offset = ATP_KILO_SIZE * st_tag.flashLayout.stFlashLayoutInfo.astFlashInfo[ATP_TAG_FLASH_MIAN_WEB_INDEX].ulAddress;
    bcm963xx_parts[MTD_WEB].size = ATP_KILO_SIZE * st_tag.flashLayout.stFlashLayoutInfo.astFlashInfo[ATP_TAG_FLASH_MIAN_WEB_INDEX].ulLen;

    //webbak
    bcm963xx_parts[MTD_WEBBAK].offset = ATP_KILO_SIZE * st_tag.flashLayout.stFlashLayoutInfo.astFlashInfo[ATP_TAG_FLASH_SLAVE_WEB_INDEX].ulAddress;
    bcm963xx_parts[MTD_WEBBAK].size = ATP_KILO_SIZE * st_tag.flashLayout.stFlashLayoutInfo.astFlashInfo[ATP_TAG_FLASH_SLAVE_WEB_INDEX].ulLen;
	...
	printk("===Part name=%s, size=%lx, ofs=%lx\n", bcm963xx_parts[MTD_ALL].name,
	        (uLong)(bcm963xx_parts[MTD_ALL].size), (uLong)(bcm963xx_parts[MTD_ALL].offset));
    ...
    printk("===Part name=%s, size=%lx, ofs=%lx\n", bcm963xx_parts[MTD_WEB].name,
            (uLong)(bcm963xx_parts[MTD_WEB].size), (uLong)(bcm963xx_parts[MTD_WEB].offset));
 	printk("===Part name=%s, size=%lx, ofs=%lx\n", bcm963xx_parts[MTD_WEBBAK].name,
	        (uLong)(bcm963xx_parts[MTD_WEBBAK].size), (uLong)(bcm963xx_parts[MTD_WEBBAK].offset));
 #endif   

    if(bcm963xx_parts[MTD_ROOTFSTAG].size == 0)
    {
         bcm963xx_parts[MTD_ROOTFS].size = bcm963xx_parts[MTD_ROOTFS].size - 0x200;
         bcm963xx_parts[MTD_ROOTFS].offset = bcm963xx_parts[MTD_ROOTFS].offset + 0x200;
    }
