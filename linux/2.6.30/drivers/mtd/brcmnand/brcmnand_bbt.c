...
printk("%s: bbt_td = bbt_slc_bch4_main_descr\n", __FUNCTION__);	
	...
	if (device_size(mtd) <= ( 512ULL << 20)) {
		...
		// If ATP's BBT partition size is smaller, then match the ATP's BBT size.
		if((ATP_BHAL_BBT_SIZE<<10) < (1<<20)) {
			this->bbt_td->maxblocks = this->bbt_md->maxblocks = (ATP_BHAL_BBT_SIZE<<10) / this->blockSize;
		}
	}
	/*
	...
	printk("%s: BBT size=0x%08X\n", __FUNCTION__, this->bbtSize);
