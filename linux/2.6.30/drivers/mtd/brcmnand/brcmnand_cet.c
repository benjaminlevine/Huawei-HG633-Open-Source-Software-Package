...
if (gdebug) printk("%s: CET = cet_descr\n", __FUNCTION__);
	...
	if (NAND_IS_MLC(this)) {
		...
		if (this->mtdSize < (1<<29)) {
			...
			if (cet->sign) {
				cet->startblk = CET_START_BLK_SLC(this->mtdSize, this);
				if((ATP_BHAL_BBT_SIZE * ATP_KILO_SIZE) < BBT_SLC_PARTITION) {
					cet->startblk = (this->mtdSize >> this->bbt_erase_shift)
							- ((ATP_BHAL_BBT_SIZE * ATP_KILO_SIZE)/this->blockSize);
				}
			} else {
				cet->startblk = (uint32_t) (this->mtdSize >> this->bbt_erase_shift)-1;
			}
	...
	printk("brcmnandCET: start blk = %x, numblks = %x\n", cet->startblk, cet->numblks);
