...
/* Begin: Added by j00199467 for flash protect, 2013/04/10 */
#ifdef CONFIG_SUPPORT_SPIFLASH_PROTECT
#include "spiflash_protect.h" 
#endif
/* End: Added by j00199467 for flash protect, 2013/04/10 */
...
#define FLASH_RDSR2         0x35    /* read status register2 */
#define FLASH_MXIC_RDSR2    0x15    /* read status register2 for mxic */
...
static int BcmSpiflash_init(void);
#endif
...
/* Begin: Added by j00199467 for flash protect, 2013/04/10 */
#ifdef CONFIG_SUPPORT_SPIFLASH_PROTECT
static int s_flash_type = FLASH_TYPE_UNKNOWN;
static unsigned char spiflash_get_status_reg(void)
{
    unsigned char buf[4];
...
static unsigned char spiflash_get_config_reg(void)
{
    unsigned char buf[4];
    struct spi_transfer xfer;
    ...
    if (FLASH_TYPE_MXIC_16M == s_flash_type)
    {
        buf[0]           = FLASH_MXIC_RDSR2;
    }
    else
    {
        buf[0]           = FLASH_RDSR2;
...
static int spiflash_set_status_reg(unsigned char regBuf[], int buf_len)
{
    unsigned char buf[4];
    unsigned char status_reg2;
    int status_reg2_data_len;
    struct spi_transfer xfer;
    ...
    if (my_spi_write(buf, 1) == SPI_STATUS_OK) {
        ...
        if (my_spi_write(buf, buf_len+1) == SPI_STATUS_OK)
...
int spiflash_protect_init(unsigned char manufacturerID, int flashSize)
{
     T_SPIFLASH_PROTECT_INIT sfInit;
     
     sfInit.flash_size = flashSize;
     sfInit.flash_area = CONFIG_SPIFLASH_PROTECT_RATIO;
     sfInit.get_config_reg = spiflash_get_config_reg;
     sfInit.get_status_reg = spiflash_get_status_reg;
     sfInit.set_status_reg = spiflash_set_status_reg;
     ...
     switch(manufacturerID) {
        ...
        case SPANPART:
            ...
            break;    
        case MACRONIXPART:
            if (0x1000000 == flashSize) //16M flash
            {
                sfInit.flash_type = FLASH_TYPE_MXIC_16M;
                s_flash_type = FLASH_TYPE_MXIC_16M;
            }
            else
            {
        ...
        case NUMONYXPART:
            ...
            else if (0x800000 == flashSize) //8M flash
            {
    ...
    switch( device_id >> 8 ) {
        ...
        case SPANPART:
            ...
            switch ((unsigned short)(device_id & 0x00ff)) {
                ...
                case ID_SPAN25FL064:
                    numsector = 128;
                    s_sflash_type = (ID_SPAN25FL064|(SPANPART << 8));
                    sectorsize = SECTOR_SIZE_4K;
                    numsector = 128*16;
                    break;
                case ID_SPAN25FL128:
...
/* Begin: Added by j00199467 for flash protect, 2013/04/10 */
#ifdef CONFIG_SUPPORT_SPIFLASH_PROTECT
    if (0 != spiflash_protect_init(device_id >> 8, totalSize))
    {
        return FLASH_API_ERROR;
    }
#endif
...
static int spi_flash_sector_erase_int(unsigned short sector)
{
    ...
    unsigned int addr;
#if 1
    if(s_sflash_type == (ID_SPAN25FL064|(SPANPART << 8)))
    {
        if(sector%16)
        {
            if(sector>(2*16))
...
/* Begin: Added by j00199467 for flash protect, 2013/04/10 */
#ifdef CONFIG_SUPPORT_SPIFLASH_PROTECT   
    if (spiflash_unlock(meminfo.sec[sector].base))
    {
        up(&spi_flash_lock);
        return -1;
    }
...
/* Begin: Modified by j00199467 for flash protect, 2013/04/10 */
#ifndef CONFIG_SUPPORT_SPIFLASH_PROTECT  
    do {
        buf[0]           = FLASH_RDSR;
        memset(&xfer, 0, sizeof(struct spi_transfer));
        xfer.tx_buf      = buf;
...
#endif
/* End: Modified by j00199467 for flash protect, 2013/04/10 */
...
static unsigned char *spi_get_flash_memptr(unsigned short sector)
{
    unsigned char *memptr = (unsigned char*)(meminfo.sec[sector].base);
