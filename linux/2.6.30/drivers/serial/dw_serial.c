...
#if ((defined CONFIG_HSAN) && defined(CONFIG_MAGIC_SYSRQ))
...
#if (defined CONFIG_HSAN)
//    uap->port.uartclk = HSAN_AHBCLK_DEFAULT;
...
#else
    pui_reg_gpiosel = (unsigned int *)ioremap(HI_REG_BASE_IOMUX + 0x1a8,4);
    pui_reg_iosel = (unsigned int *)ioremap(HI_REG_BASE_IOMUX + 0x194,4);
#endif
    /*使能UART0 用于DECT控制通信*/
...
MODULE_AUTHOR("HSAN Ltd/Deep Blue Solutions Ltd");
MODULE_DESCRIPTION("HSAN  DW serial port driver");
MODULE_LICENSE("GPL");
