...
#endif
#if defined(CONFIG_BUILD_CUSTOM_FMC_PRODUCT)
#ifdef CONFIG_PRODUCT_FMC_B880  /*B880_PRODUCT*/
...
static struct usb_device_id option_ids[] = {
	...
	{ USB_DEVICE(HUAWEI_VENDOR_ID, 0x1C05) }, /*E173*/
     { USB_DEVICE(HUAWEI_VENDOR_ID, 0x158E) }, /*E3726*/
#endif
	...
	//{ USB_DEVICE(ONDA_VENDOR_ID, ONDA_PRODUCT_MT503HS) },//ZTE数据卡MF190适配，该PRODUCTid会导致数据卡被忽略
...
{ USB_DEVICE_AND_INTERFACE_INFO(ZTE_DATACARD_VID_MF190, ZTE_DATACARD_PID_MF190, 0xff, 0xff, 0xff) },
{ USB_DEVICE_AND_INTERFACE_INFO(ZTE_DATACARD_VID_MF193, ZTE_DATACARD_PID_MF193, 0xff, 0xff, 0xff) },
{ USB_DEVICE_AND_INTERFACE_INFO(ZTE_DATACARD_VID_MF112, ZTE_DATACARD_PID_MF112, 0xff, 0xff, 0xff) },
//j00202470 for zte mf serial support end

//适配K5006-Z数据卡
{ USB_DEVICE_AND_INTERFACE_INFO(ZTE_DATACARD_VID_K5006_Z, ZTE_DATACARD_PID_K5006_Z, 0xff, 0x86, 0x10)}, //Diag口
{ USB_DEVICE_AND_INTERFACE_INFO(ZTE_DATACARD_VID_K5006_Z, ZTE_DATACARD_PID_K5006_Z, 0xff, 0x02, 0x05)}, //PCUI口
{ USB_DEVICE_AND_INTERFACE_INFO(ZTE_DATACARD_VID_K5006_Z, ZTE_DATACARD_PID_K5006_Z, 0xff, 0x02, 0x01)}, //Modem口


//适配HA35 LTE数据卡
//HUAWEI Mobile Connect - Control Interface
{ USB_DEVICE_AND_INTERFACE_INFO(HA35_LTE_MODEL_VID, HA35_LTE_MODEL_PID, 0xff, 0x02, 0x06)}, 
//HUAWEI Mobile Connect - 3G Application Interface
{ USB_DEVICE_AND_INTERFACE_INFO(HA35_LTE_MODEL_VID, HA35_LTE_MODEL_PID, 0xff, 0x02, 0x03)}, 
//HUAWEI Mobile Connect - PC UI Interface
{ USB_DEVICE_AND_INTERFACE_INFO(HA35_LTE_MODEL_VID, HA35_LTE_MODEL_PID, 0xff, 0x02, 0x12)}, 
//HUAWEI Mobile Connect - 3G PC Voice Interface
{ USB_DEVICE_AND_INTERFACE_INFO(HA35_LTE_MODEL_VID, HA35_LTE_MODEL_PID, 0xff, 0x02, 0x14)},
//HUAWEI Mobile Connect - Application Interface
{ USB_DEVICE_AND_INTERFACE_INFO(HA35_LTE_MODEL_VID, HA35_LTE_MODEL_PID, 0xff, 0x02, 0x13)}, 

//适配E3131数据卡
{ USB_DEVICE_AND_INTERFACE_INFO(HUAWEI_VENDOR_ID, 0x1001, 0xff, 0x02, 0x01)},
{ USB_DEVICE_AND_INTERFACE_INFO(HUAWEI_VENDOR_ID, 0x1001, 0xff, 0x02, 0x02)},
{ USB_DEVICE_AND_INTERFACE_INFO(HUAWEI_VENDOR_ID, 0x1001, 0xff, 0x02, 0x03)},
{ } /* Terminating entry */
};
#endif
...
static void option_instat_callback(struct urb *urb)
{
	...
	if (status == 0) {
		...
		if ((req_pkt->bRequestType == 0xA1) &&
            ...
            /* start for DTS2013112809769 huzhemin 00250612 2013/11/29                          */
            /* 此问题单针对huawei K4203数据卡打开tty端口失败造成首次拨号失败                    	*/
            /* 原因是因为数据卡更换自研驱动以后,打开端口的过程需要握手过程                      	*/
            /* 而以往的jungo驱动该过程较为宽松,linux的驱动会在握手过程中挂死                    	*/
            /* 数据卡方面表示该卡已经AT,所以不会进行修改                                        	*/
            /* 修改方法为针对切换端口后的K4203数据卡PID 1001 进行规避,将返回的握手过程取消      	*/
            /* 其他切换端口后PID为1001的数据卡,E1786 K3772 K4201 K4305 验证没有影响                */
            if (0x1001  == le16_to_cpu(serial->dev->descriptor.idProduct)
                && 0x12d1 == le16_to_cpu(serial->dev->descriptor.idVendor))
            {
                 if(1 == old_dcd_state)
                {
                    old_dcd_state = 0;
            ...
            /* end for DTS2013112809769 huzhemin 00250612 2013/11/29 */            
...
#if defined(CONFIG_ATP_SUPPORT_B560) || defined(CONFIG_BUILD_CUSTOM_FMC_PRODUCT)
	{
