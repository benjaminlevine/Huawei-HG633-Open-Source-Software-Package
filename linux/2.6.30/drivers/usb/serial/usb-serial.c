...
#if defined(CONFIG_PRODUCT_FMC_B880)
...
/*Start of modified by lvxin 00135113 for support hspa 2009年4月30日*/
	...
	/* END:   Modified by c106292, 2009/8/8 */
	{
		...
		while (0 != hw_datacard_pids[i].pid)
		{
			...
			if (0x0 == id->idProduct)
			{
				printk("\nfind the special device.pid=0x%x, for HG658V2, we treat it as E369\n",hw_datacard_pids[i].pid);
				find = 1;	/*置成功找到匹配标识*/
				memset(szUmts_type, 0, sizeof(szUmts_type));
				sprintf(szUmts_type, "modem:ttyUSB0\nvoice:ttyUSB1\npcui:ttyUSB2\n");   // 格式固定
				strcpy(acHspaFactory ,HUAWEI_HSPA_CARD );
      ...
      else if((ZTE_VENDOR_ID == id->idVendor)||(ZTE_VENDOR_LITTLE_ENDIAN_ID == id->idVendor))//用于适配中兴数据卡
      {
            printk("\nThis is ZTE device.id->idProduct=0x%x\n",id->idProduct);
            g_ulNumOfDataCard ++;
            i = 0;
            
            iUsbTtyState++;
            
            while(hw_datacard_pids[i].portString)
            {
                if(id->idProduct == hw_datacard_pids[i].pid)
                {
                    printk("\nfind the match device.pid=0x%x\n",hw_datacard_pids[i].pid);
                    memset(szUmts_type, 0, sizeof(szUmts_type));
                    memset(acHspaFactory, 0, sizeof(acHspaFactory));
                    
                    sprintf(szUmts_type,hw_datacard_pids[i].portString);   // 格式固定

                    find = 1;   /*置成功找到匹配标识*/
                    
                    strcpy(acHspaFactory ,ZTE_HSPA_CARD);
                    break;
                }
...
void usb_serial_disconnect(struct usb_interface *interface)
{
	...
	memset(szUmts_type, 0, sizeof(szUmts_type));
#if defined(CONFIG_BUILD_CUSTOM_FMC_PRODUCT)
      syswatch_nl_send(ATP_MSG_MONITOR_EVT_WM_STOP, NULL, 0);
#else
