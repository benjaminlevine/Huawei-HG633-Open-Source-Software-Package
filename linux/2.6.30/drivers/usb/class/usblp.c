...
//add by zhouyu 20101029 支持hub方式同时访问打印机及USB存储设备
...
/*Start of added by lvxin00135113@20100114 for support usb-printer vendor*/
extern struct proc_dir_entry proc_root;
...
static void proc_user_ipp_create(void)
{
    ...
    proc_ipp = create_proc_entry("proc_user_ipp", S_IFREG | S_IRUSR , &proc_root ); 
    ...
    proc_printerdev = create_proc_entry("proc_user_printer", S_IFREG | S_IRUSR , &proc_root ); 
...
//start add by zhouyu 20101029 支持hub方式同时访问打印机及USB存储设备
...
{
    proc_printermedium = create_proc_entry("proc_usb_printer", S_IFREG | S_IRUSR , &proc_root ); 
		       ...
		       const struct usb_device_id *id)
{
	...
	usblp->intf = intf;
       ...
       disk = get_freediskslot();
        ...
        /*START:Modified by j00199467 for DTS2013032101042 20130325*/
        if (NULL != dev->manufacturer)
        {
            sprintf(disk->vendor , "%s" , dev->manufacturer);
        }  
        /*END:Modified by j00199467 for DTS2013032101042 20130325*/
...
next1: 
	...
	printk(KERN_INFO "usblp%d: USB %sdirectional printer dev %d "
        ...
        /*Start of added by lvxin00135113@20100114 for support usb-printer vendor*/
        {
            /* Modified by y00183935@20130313 for DTS2013030500782 */
            static char acAtpDeviceMFG[512] = "";
            static char acAtpDeviceMDL[512] = "";
            char * pAtpDeviceId = &usblp->device_id_string[2];
            char * pAtpTmp = NULL;
        ...
        /*ipp start*/
        syswatch_nl_send(ATP_MSG_MONITOR_EVT_IPP_START, NULL, 0);
        g_USB_PROBE = 1;
        /*End of added by lvxin00135113@20100114 for support usb-printer vendor*/        
	return 0;
...
static void usblp_disconnect(struct usb_interface *intf)
{
	...
	struct usblp *usblp = usb_get_intfdata (intf);
       ...
       /*ipp stop*/
       syswatch_nl_send(ATP_MSG_MONITOR_EVT_IPP_STOP, NULL, 0);
       /*End of added by lvxin00135113@20100114 for support usb-printer vendor*/
	...
	if (!usblp->used)
       {   
		usblp_cleanup (usblp);
            ...
            {
                //if(!strcmp(usblp->dev->manufacturer,((struct usb_printerinfo *)printerinfos+n)->diskname))//支持多个USB打印机的情况
                //{
                    //printk("\r\nprinter(%s) is removed from printerinfos",((struct usb_printerinfo *)printerinfos+n)->vendor);
...
static int __init usblp_init(void)
{
	...
	/*End of added by lvxin00135113@20100114 for support usb-printer vendor*/    
    //add by zhouyu 20101029 支持hub方式同时访问打印机及USB存储设备
    //创建进程文件proc_usb_printer，用来记录USB打印机的相关信息
...
static void __exit usblp_exit(void)
{
    ...
    remove_proc_entry("usbmount_pid", NULL /* parent dir */);
    //add by zhouyu 20101029 支持hub方式同时访问打印机及USB存储设备
