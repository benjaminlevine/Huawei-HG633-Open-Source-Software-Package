...
/* ipoa模式下DFE只负责剥离以太网头，不能回复HG的arp请求，这样HG不能收到arp应答，导致ipoa业务不通。
因此HG需对ipoa的wan发出的arp查询直接返回指定mac（即TRI指定的mac  "06:05:04:03:02:01"）*/
#if (defined CONFIG_HSAN)
//判断设备是否有ipv4地址
 int dev_has_addr(struct net_device *dev)
{
...
/*ipoa的wan 发出的arp查询直接返回创达特指定的mac  "06:05:04:03:02:01"  */
struct neighbour *neigh_create_5610x(struct neigh_table *tbl, const void *pkey,
			       struct net_device *dev)
{
	struct neighbour *n = neigh_create(tbl, pkey, dev);

	if(!IS_ERR(n) && !strncmp(dev->name,"ipa",3) && dev_has_addr(dev))
	{
		...
		unsigned char sha[]={0x06,0x05,0x04,0x03,0x02,0x01};
...
#endif 
/* end   by z00190439 for  hg5610x 2013-4-8*/
