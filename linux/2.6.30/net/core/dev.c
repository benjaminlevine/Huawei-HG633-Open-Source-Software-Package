...
/* start of新增BT需求SPECIAL_VLAN的WAN的下行报文可以带SPECIAL_VLAN 和可以不带SPECIAL_VLAN下来 by m00132836 2010-08-20 */
#include "atp_interface.h"
extern u16 g_vlanid;
/* end of新增BT需求SPECIAL_VLAN的WAN的下行报文可以带SPECIAL_VLAN 和可以不带SPECIAL_VLAN下来 by m00132836 2010-08-20 */

#if defined(CONFIG_HISI_HW_IPSEC) || defined(CONFIG_HISI_HW_IPSEC_MODULE)
#include "sec_drv.h"

unsigned int g_sec_dbg_level = SEC_DBG_ERR;
EXPORT_SYMBOL(g_sec_dbg_level);

int g_sec_alg_seq = SEC_TYPE_CRYPTO; 
EXPORT_SYMBOL(g_sec_alg_seq);

int (*hisec_cli_process_hook)(void *in1, void *out1, unsigned int len, void *in2, void *out2) = NULL;
EXPORT_SYMBOL(hisec_cli_process_hook);
...
/* start of wan down with qos enable by m00132836 2013/02/08, 补充另外两种类型报文2013/2/26 */
#ifdef CONFIG_HSAN
#define ETH_P_CTL		0x8889
#define ETH_P_DBG		0x888A
#define ETH_P_INF		0x888B //DSL_Link_Status
#endif
/* end of wan down with qos enable by m00132836 2013/02/08, 补充另外两种类型报文2013/2/26 */

#if (defined(CONFIG_MIPS_BRCM) && defined(CONFIG_BCM_SMUX)) || (defined(CONFIG_SMUX))
...
/* BEGIN: Added by  w00182511, 2011/8/29   PN:HG656C INTERNET 点灯适配*/
...
static int oam_loopback_enable = 0;

#define MAX_MIRROR_PORT_MUM 7
static int port_id_mirror_from[MAX_MIRROR_PORT_MUM] = {0,0,0,0,0,0,0};
/*Start for DTS2014040104262  mirror页面wlan镜像抓包功能不生效 by z00190439 2014-04-02*/
#ifdef CONFIG_HSAN
static char *mirror_port_name[MAX_MIRROR_PORT_MUM] = {HSAN_LAN1_IFNAME,HSAN_LAN2_IFNAME,HSAN_LAN3_IFNAME,HSAN_LAN4_IFNAME,"nas","wl","rmnet"};
#else 
#if SUPPORT_ATP_WANETH
#ifdef CONFIG_LAN_PORT_REVERSE
static char *mirror_port_name[MAX_MIRROR_PORT_MUM] = {"eth0.5","eth0.4","eth0.3","eth0.2",ATP_WANETH_INTERFACE_NAME,"wl","rmnet"};
#else
static char *mirror_port_name[MAX_MIRROR_PORT_MUM] = {"eth0.2","eth0.3","eth0.4","eth0.5",ATP_WANETH_INTERFACE_NAME,"wl","rmnet"};
#endif
#else
static char *mirror_port_name[MAX_MIRROR_PORT_MUM] = {"eth0.2","eth0.3","eth0.4","eth0.5","nas","wl","rmnet"};
#endif
#endif
/*End for DTS2014040104262  mirror页面wlan镜像抓包功能不生效 by z00190439 2014-04-02*/
...
#ifdef CONFIG_HSAN
/* 
  * 海思方案中，内核收到的数据包都已经剥离了VLAN，如果需要得到VLAN信息，可以分以下两种情况处理:
  * 1. 有对应的VLAN接口
  *     VLAN信息从对应的接口名中获得；
  * 2. 没有对应的VLAN接口 
  *     VLAN信息从hi_cb中获得
...
#define HISI_PTM_ROOT_DEV           "nas_p1"
#define HISI_VLAN_DELIM                '.'

int hisi_get_vlan(struct sk_buff *skb, unsigned short *vlanid)
{
    char * vlanptr = NULL;
    unsigned long vlan = 0;

    vlanptr = strchr(skb->dev->name, HISI_VLAN_DELIM);
    //1. nas_p1 接口收到的数据包, VLAN值从hi_cb中获得
    if ((NULL == vlanptr) && (0 == strcasecmp(skb->dev->name, HISI_PTM_ROOT_DEV)))
    {
    ...
    //2. nas_p1.xxx 接口收到的数据包，VLAN值直接从接口名中获得
    else
    {
        if(0 == strict_strtoul(vlanptr+1, 10, &vlan) )
        {
            *vlanid = vlan;
            return 0;
        }

        printk(" interface:%s error!!!!\n",skb->dev->name);
        return 1;
    }
}
#endif
...
#ifdef CONFIG_VLAN0
/*
 * VLAN0接收不带VLAN报文的条件如下:
 * 1. 有带VLAN0的接口；
 * 2. 无不带VLAN的接口(不带VLAN的报文默认走该接口)；
 * 3. 应用层配置允许VLAN0接口接收不带VLAN的报文；
 */

/* 用户层配置项信息 */
int vlan0_untag    __read_mostly = 1;                   /* 1: 允许接收；0: 不允许接收 */

// 记录系统中的接口情况，避免处理报文时查询
/* 系统中是否有不带VLAN的(以太网)接口 */
static int has_untag_intf __read_mostly = 0;            /* 1: 有不带VLAN接口； 0: 没有不带VLAN接口 */
/* 系统中是否有带VLAN0的(以太网)接口 */
static int has_vlan0_intf __read_mostly = 0;            /* 1: 有带VLAN0接口；  0: 没有带VLAN0接口 */

/* VLAN0接口名 */
#define VLAN0_INTF_NAME     "nas1.0"
#if 0
static char vlan0_inf[IFNAMSIZ];
#endif


/* 判断接口是否为某种类型的接口,在接口注册与去注册是使用，不用过分关注效率 */
static void set_inf_flag(const char* name, const char* needle, int inf_flag)
{
    /*
     * NOTE: 
     * 此处根据接口名来判断是否为某种类型的接口，接口命名方式不同则需要修改(此处使用海思方案半桥接口命名);
     */
    ...
    //printk("function:%s line:%d name:%s needle:%s\n", __FUNCTION__, __LINE__, name, needle);    
    ...
    if (strstr(name, needle))
    {
        ...
        if ('_' == name[4])
        {
            has_untag_intf = inf_flag;        
        }
        // nas1.0_x，needle传入时已经确定了模式，不需要做多处判断
        else if ('.' == name[4])
        {
            has_vlan0_intf = inf_flag;
        }
        else
        {
            //printk(">>>>function:%s line:%d new needle need process\n", __FUNCTION__, __LINE__);
        }
    }
    //printk("function:%s line:%d name:%s needle:%s untag_inf:%d vlan0_inf:%d\n", __FUNCTION__, __LINE__, name, needle, has_untag_intf, has_vlan0_intf);

    return;
}

static void flow_to_vlan0_device(struct sk_buff *skb)
{
    char * vlanptr = NULL;
    ...
    // 非根接口收包, 非WAN接口
    vlanptr = strchr(skb->dev->name, '.');      //nas1_x
    if(!(NULL == vlanptr)                       //带vlan
     ||!('n' == skb->dev->name[0]))             //非wan口 
    {
        return;
    }
    ...
    if ((0 == has_untag_intf)           // 没有不带vlan接口
      &&(1 == has_vlan0_intf)           // 有带VLAN0的接口
      &&(1 == vlan0_untag))             // 用户层配置允许接收不带VLAN报文  
    {
        struct net_device *vlan0_dev = NULL;
    	if (vlan0_dev = __dev_get_by_name(&init_net, VLAN0_INTF_NAME))
        {   
            skb->dev = vlan0_dev;
        }
    }
...
/*START MODIFY:t00176367 20131207 for DTS2013113001315*/
#if defined(CONFIG_MIPS_BRCM) && (defined(CONFIG_BCM_P8021AG) || defined(CONFIG_BCM_P8021AG_MODULE)) || defined(CONFIG_HW_COMMON_8021AG)
/*END MODIFY:t00176367 20131207 for DTS2013113001315*/
...
static inline void tdumpHexData(uint8_t *pHead, uint32_t len)
{
    uint32_t i;
    uint8_t *c = pHead;
    for (i = 0; i < len; ++i) {
...
static inline void dumppkt(const char * fname, uint8_t * pBuf, uint32_t len)
{
    int dump_len = ( len < 64) ? len : 64;

    printk("\n%s: data<0x%08x> len<%u>\n", fname, (int)pBuf, len);
    tdumpHexData(pBuf, dump_len);
}
...
static int skbFree_thread_func(void *thread_data)
{
    ...
    {
        budget = MAX_SKB_FREE_BUDGET;/*TODO decide on proper value */

    update_list:
        spin_lock_irq(&skbfree_lock);
        if(free_list == NULL)
        {
...
static void bind_mark(struct sk_buff *skb)
{
   /*和fengdayang初步确认0x200000暂时作为lan侧报文的mark值，后续需进一步调整和优化by g00121640 2012-04-06*/  
    if (skb->dev->name[0] == 'e')   /* 判断是否为以太网端口 */
    {
	    skb->mark = skb->mark | 0x200000;
       
    }    
    else if (skb->dev->name[0] == 'w') /* 判断是否为wifi端口 */
    {
	    skb->mark = skb->mark | 0x200000;
#ifdef CONFIG_ATP_GRE_FON
        if (skb->dev->name[3] != '\0') //wl0        
        {
        	switch(skb->dev->name[4])
        	{        	    
        	    case '1':    //wl0.1
        	    {
        	    	skb->mark = skb->mark | WLAN2_MARK;
        	    	break;
        	    }	
        	    case '2':    //wl0.2
        	    {
        	    	skb->mark = skb->mark | WLAN3_MARK;
        	    	break;
        	    }
                default:
                break;
    ...
    else if (skb->dev->name[0] == 'r' && skb->dev->name[1] == 't' && skb->dev->name[2] == 'k') //该方案LAN口名为rtksw
    {
        skb->mark = skb->mark | 0x200000;
    }
}

static struct sk_buff * lanswitch_put_vlan_tag(struct sk_buff *skb, int portNum)
{   
#ifdef CONFIG_RTK_EXT_SWITCH
    int iLogicPort = 0;
    iLogicPort = PHYPORT_SEQUENCE[(portNum-1)*2] - '0';

    skb = __vlan_put_tag(skb,iLogicPort);
    if (NULL == skb)
    {
        return NULL;
    }
    skb->mark = skb->mark | 0x00800000;    
#endif
    return skb;
}
...
static void MirrorPacket( struct sk_buff *skb, char *intfName )
{
    ...
    if( (skbClone = skb_clone(skb, GFP_ATOMIC)) != NULL )
    {
...
#endif
        {
			...
			if (( 'e' == netDev->name[0]) || ( 't' == netDev->name[1]) || ( 'h' == netDev->name[2]))
            {
                porNum = intfName[5] - '0' -1;
                skbClone = lanswitch_put_vlan_tag(skbClone, porNum);
				if (NULL == skbClone)
				{
					return;
				}
...
/* start added by t00191685 for 代码整改,2012-1-9 */
...
/* end added by t00191685 for 代码整改,2012-1-9 */

/*start added by t00191685 for 关键性报文走最高优先级,2012-1-11*/
...
static inline void SkbMarkForHighestQueue(struct sk_buff *skb)
{
	...
	unsigned short tcp_len = 0;   /*add by w00182844 for tcp ack mark @2012/10/11*/
	...
	/*Start: add by w00182844 for tcp ack mark @2012/10/11*/	
    /* TCP payload len is 0 for pppoe mode*/
    if (0x6 == skb->data[31])
    {
		tcp_len = (skb->data[25] + (skb->data[24]<< 8)) - (skb->data[54] >> 2) - 20; 	
    ...
    /* TCP payload len is 0 for iproute or bridge mode*/
    if (0x6 == skb->data[23])
    {
		tcp_len = (skb->data[17] + (skb->data[16]<< 8)) - (skb->data[46] >> 2) - 20; 	
	...
	/*End: add by w00182844 for tcp ack mark @2012/10/11*/	

/* start of wan down with qos enable by m00132836 2013/02/08, 补充另外两种类型报文2013/2/26 */
#ifdef CONFIG_HSAN
    if (__constant_htons(ETH_P_CTL) == skb->protocol || __constant_htons(ETH_P_DBG) == skb->protocol
            || __constant_htons(ETH_P_INF) == skb->protocol)
    {
        iControlPkt = 1;
    }
#endif
...
/*end added by t00191685 for 关键性报文走最高优先级,2012-1-11*/

/* start DT一线死锁问题 by f00110348 20130223 */
int dev_is_spinlocked(struct sk_buff *skb)
{
	struct net_device *dev = skb->dev;
	struct netdev_queue *txq;
	struct Qdisc *q;
...
EXPORT_SYMBOL(dev_is_spinlocked);
/* end DT一线死锁问题 by f00110348 20130223 */


static int oam_loopback_send_packets_handler(struct sk_buff *skb)
{
	skb_reset_mac_header(skb);
	const unsigned char *dest = eth_hdr(skb)->h_dest;
    const u8 oam_group_address[ETH_ALEN] = { 0x01, 0x80, 0xc2, 0x00, 0x00, 0x02 }; 

	//printk("skb->dev->name is %s\n", skb->dev->name);
    if (!IS_WAN_DEVICE(skb->dev->name))
    {
    	return 0;
    }
    //printk("dest is %x\n", dest);
    if (((!compare_ether_addr(dest, oam_group_address))
         && (htons(ETH_P_SLOW) == eth_hdr(skb)->h_proto))
         || (OAM_MARK == skb->mark))
    {
		return 0;
    }
 ...
 *          --BLG
 */
 int skb_to_downlinkqos(struct sk_buff *skb,struct net_device *dev)
 {   
#ifdef  CONFIG_DOWN_RATE_CONTROL
    int ret = 0;
...
int dev_queue_xmit(struct sk_buff *skb)
{
    ...
    /*start added by t00191685 for 关键性报文走最高优先级,2012-1-9*/
    SkbMarkForHighestQueue(skb);
    /*end added by t00191685 for 关键性报文走最高优先级,2012-1-9*/

#ifdef CONFIG_BROADCOM_RELEASE_VERSION_4_12_L08
    //解决wifi多SSID临时开启时下行软加速qos无效
    if((0 == strcmp(dev->name,"wl0.1"))
            ||(0 == strcmp(dev->name,"wl1.1")))
    {
        #ifdef CONFIG_BLOG
                blog_skip(skb);
        #endif
...
#endif    
	if (oam_loopback_enable)
    {
    	/*oam loopback demand that CPE can not send out any other packets except loopback packets whose mark is 0x00080000*/
        if (oam_loopback_send_packets_handler(skb))
        {
        	//printk("other packets, just free\n");
        	goto out_kfree_skb;
        }
        //printk("dev_queue_xmit oam leave\n");
    }
    /* start added by t00191685 for 代码整改,2012-1-9 */
...
{
/* start of by d00107688 开启组播之后插拔DSL线然后插上导致系统重启 */
...
#endif
/* End of by d00107688 开启组播之后插拔DSL线然后插上导致系统重启 */
...
int drop_vlan_header(struct sk_buff *pstSkb)
{
    ...
    puszSrc = (unsigned char *)pstEHdr;//puszSrc指向以太网头部
    puszDst = puszSrc + VLAN_HLEN; //pstSkb->data - 10 ;//
    memmove(puszDst, puszSrc, 12);
    pstSkb->mac_header += VLAN_HLEN; //mac.raw要指向以太网的头部，由于以太网头部是往下移了4个字节，所以要从新定位下
 ...
 *	NET_RX_DROP: packet was dropped
 */
 /* Add by d00167548 for WS320 易用性 DNS重定向 2012.3.29 */
 
/*此处是wlan在repeater模式下二层直接返回DNS解析报文的初始化声明和函数*/
int net_msg_dns = 0;
char acdefaultdnsdomain[64] = {0};/*默认域名*/
#define DNS_PORT         0x35
#define DNS_POINT        '.'
#define IP_ADDR_LEN      4

/*UDP协议的标示位*/
static unsigned char upd_protocal[] = {0x00, 0x11};
/*IP 协议的表示位*/
static unsigned char IP_protocal[] = {0x08, 0x00};
/*diff到flag中间的部分*/
static unsigned char diff_flags[] = {0x45, 0x00, 0x00, 0x4a, 0x00, 0xd2, 0x00, 0x00, 0x80, 0x11};
/*Flags到query中间的部分*/
static unsigned char flags_query[] = {0x81, 0x00, 0x00, 0x01, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00};
/*answer头域中不包含地址的部分*/
static unsigned char answer[] = {0xc0, 0x0c, 0x00, 0x01, 0x00, 0x01, 0x00, 0x00, 0x00, 0xb4, 0x00, 0x04};

static inline SendDnsResponds(struct sk_buff *skb)
{
    struct sk_buff* DnsResponds = NULL;
    struct net_device* brdev = NULL;
    struct in_device* indev = NULL;
    struct in_ifaddr* ifa = NULL;
    int    dns_ip_found = 0;
    struct net_device* wl0_dev_to = NULL;
    //struct udphdr *uh = NULL;
    int i = 0;
    short ip_header[10] = {0};
    static short upd_check[512] = {0};
    short check_sum = 0;
    short ip_length = 0;
    short udp_lengthhex = 0;
    long int  check_base = 0;
    long int  udp_length = 0;
    long int  query_length = 0;
    unsigned char  dns_ip[IP_ADDR_LEN] = {0};
    static unsigned char  dns_reply[512] = {0};
    struct net_device* dev = skb->dev;
    struct net* net = dev_net(dev);

    if (NULL == (DnsResponds = dev_alloc_skb(512)))
    {
        printk("Reserve DnsResponds skb buffer fails\r\n");
        return;
    }
    /*初始化dns_reply,算出需要回复的报文长度*/
    query_length = skb->len;

    memset(dns_reply, 0, sizeof(dns_reply));
    memset(ip_header, 0, sizeof(ip_header));
    memset(dns_ip, 0, sizeof(dns_ip));
    memset(upd_check, 0, sizeof(upd_check));

    /*dst mac*/
    memcpy(&(dns_reply[0]), eth_hdr(skb)->h_source, ETH_ALEN);

    /*src mac*/
    memcpy(&(dns_reply[6]), eth_hdr(skb)->h_dest, ETH_ALEN);

    /*ip protocal*/
    memcpy(&(dns_reply[12]), IP_protocal, sizeof(IP_protocal));

    /*ip header diff-flags*/
    memcpy(&(dns_reply[14]), diff_flags, sizeof(diff_flags));

    /*ip报文的总长度*/
    ip_length = (query_length + 16) & 0xffff;
    memcpy(&(dns_reply[16]), &ip_length, sizeof(ip_length));

    /*src ip addrss*/
    memcpy(&(dns_reply[26]), &(skb->data[12 + IP_ADDR_LEN]), IP_ADDR_LEN);

    /*dest ip addrss*/
    memcpy(&(dns_reply[26 + IP_ADDR_LEN]), &(skb->data[12]), IP_ADDR_LEN);
    ...
    if (brdev)
    {
        ...
        if (indev && (NULL != (ifa = indev->ifa_list)))
        {
            for (; ifa; ifa = ifa->ifa_next)
            {
                /*Check whether dns requeset src ip is the same ip range with BR0*/
                if ((ifa->ifa_local & ifa->ifa_mask) == ((*((unsigned int*)(&(dns_reply[26 + IP_ADDR_LEN])))) & ifa->ifa_mask))
                {
                    memcpy(dns_ip, &(ifa->ifa_local), sizeof(dns_ip));
                    dns_ip_found = 1;
                    break;
                }
            }
    ...
    if (!dns_ip_found)
    {
        kfree_skb(DnsResponds);
        printk("dns ip not found\r\n");
        return;
    }

    /*source port*/
    memcpy(&(dns_reply[34]), &(skb->data[22]), 2);

    /*dest port*/
    memcpy(&(dns_reply[36]), &(skb->data[20]), 2);

    /*transport id*/
    memcpy(&(dns_reply[42]), &(skb->data[28]), 2);

    /*flags-query 44-53*/
    memcpy(&(dns_reply[44]), flags_query, sizeof(flags_query));

    /*query*/
    memcpy(&(dns_reply[54]), &(skb->data[40]), query_length - 40);

    /*DNS IP address*/
    memcpy(&(dns_reply[query_length + 26]), dns_ip, IP_ADDR_LEN);

    /*answer*/
    memcpy(&(dns_reply[query_length + 14]), answer, sizeof(answer));

    /*udp length = 收到的UDP报文长度+回答的长度(包括IP)*/
    udp_length = query_length - 20 + sizeof(answer) + 4;
    udp_lengthhex = (short)udp_length & 0xffff;
    memcpy(&(dns_reply[38]), &udp_lengthhex, 2);

    /*Caculate IP header checksum */
    /*1. init checksum to zero*/
    memset(&(dns_reply[24]), 0, sizeof(check_sum));

    /*2. copy ip_header*/
    memcpy(ip_header, &(dns_reply[14]), sizeof(ip_header));
    for (i = 0; i < 10; i++)
    {
    ...
    memcpy(&(dns_reply[24]), &check_sum, sizeof(check_sum));

    /*Caculate UDP checksum */
    /*1. init checksum to zero*/
    /*UDP报文校验和:源地址+目的地址+0x00+0x11+UDP报文长度+udp数据部分*/
    check_base = 0;
    memset(&(dns_reply[40]), 0, sizeof(check_sum));
    memset(upd_check, 0, udp_length + 12);
    memcpy(upd_check, &(dns_reply[26]), 8);
    memcpy(&(upd_check[8]), upd_protocal, 2);
    memcpy(&(upd_check[10]), &(dns_reply[38]), 2);
    memcpy(&(upd_check[12]), &(dns_reply[34]), udp_length);
    for (i = 0; i < udp_length + 12; i++)
    {
        check_base += (long int)(upd_check[i] & 0xffff);
    }
    ...
    check_sum = 0xffff - (short)(check_base);
    memcpy(&(dns_reply[40]), &check_sum, sizeof(check_sum));

    memcpy(DnsResponds->data, dns_reply, query_length + 30);
    DnsResponds->len  = query_length + 30;
    DnsResponds->tail = DnsResponds->data + query_length + 30;
    wl0_dev_to = __dev_get_by_name(net, skb->dev->name);
    if (NULL != wl0_dev_to)
    {
        DnsResponds->dev = wl0_dev_to;
        DnsResponds->protocol  =  eth_type_trans(DnsResponds,  wl0_dev_to);
        skb_push(DnsResponds, ETH_HLEN);
        dev_queue_xmit(DnsResponds);
    }
    return;
}
static  char dns_tmpdomain[sizeof(acdefaultdnsdomain) + 1];
static  char dns_domain[sizeof(acdefaultdnsdomain) + 1];
static  short str_length[10] = {0};

static inline int IsDnsNeedResponds(struct sk_buff *skb)
{
    int  i = 0;
    int  j = 0;
    int  pointlocal = 40;
	...
	memset(dns_tmpdomain, 0, sizeof(dns_tmpdomain));
	memset(dns_domain, 0, sizeof(dns_domain));
	memset(str_length, 0, sizeof(str_length));
    if (skb->data[9] == 0x11) //IPPROTO_UDP
    {
        if (( skb->data[20 + 2] == 0x00 ) && ( skb->data[20 + 3] == DNS_PORT ))/*判断0x35口*/
        {
            //dns name len
            substrbegin = skb->data[pointlocal];
            if (substrbegin > sizeof(dns_domain))
            {
                printk("\n WARN!!! %s() Line %d: substrbegin = %d \n", __func__, __LINE__, substrbegin);
                return false;
            }

            memset(dns_tmpdomain, 0, sizeof(dns_tmpdomain));
            memcpy(dns_tmpdomain, acdefaultdnsdomain, sizeof(dns_tmpdomain));

            //check wheather start with www.
            if (3 == substrbegin)
            {
                if (0 == strncmp(&(skb->data[pointlocal + 1]), "www", 3))
                {
                    snprintf(dns_tmpdomain, sizeof(dns_tmpdomain), "www.%s", acdefaultdnsdomain);//default dns domain should not contains www.
                }
            }
            pszhead = dns_tmpdomain;
            //check wheather start with www. , if yes skill www. than check
            for ( i = 0; NULL != pszhead && 0 != substrbegin; i++ )
            {
                pszpoint = strchr(pszhead, DNS_POINT);
                if (NULL == pszpoint)
                {
                    if (0 == count)
                    {
                ...
                memcpy(dns_domain, &(skb->data[pointlocal + 1]), substrbegin);
                dns_domain[substrbegin] = '\0';
                str_length[i] = strlen(dns_domain);
                if (0 != strcmp(dns_domain, pszhead))
                {
                    return false;
                }
                /*当'.'的值为0时，说明'.'之后已经没有域名的字符了*/
                if (0 == skb->data[pointlocal + substrbegin + 1])
                {
                    if  (NULL != pszpoint)
                    {
                        printk("\n WARN!!! %s() Line %d: substrbegin = %d \n", __func__, __LINE__, substrbegin);
                ...
                substrbegin = skb->data[pointlocal + substrbegin + 1];
                pointlocal = pointlocal + str_length[i] + 1;
...
/*此处是wlan在repeater模式下二层直接返回DNS解析报文的初始化声明和函数*/
/* END by d00167548 for WS320 易用性 DNS重定向 2012.3.29 */

#ifdef CONFIG_TAG_NOVLAN_IPTV_PACKETS
/*start added by l00184769 for et*/
/*Igmp control packets have vlan but igmp data have no vlan, we should
  tag the igmp data packets with igmp control vlan
  **Make sure the drive didn't set the vlan before, do not tag igmp control packets***/
static void tag_igmp_vlan_for_iptv_packets(struct sk_buff *skb)
{
    const unsigned char *dest = eth_hdr(skb)->h_dest;
    unsigned char mc_mac[6];
    memset(mc_mac, 0xff, 6);
    if ((htons(ETH_P_IP) == eth_hdr(skb)->h_proto)
        && (htons(IPPROTO_IGMP) != skb->data[9])
        && is_multicast_ether_addr(dest) 
        && memcmp(mc_mac, dest, ETH_ALEN) 
        && IS_WAN_DEVICE(skb->dev->name)
        && g_Vlan_tci)
    {
    	skb->data -= 14;
        skb = __vlan_put_tag(skb, g_Vlan_tci);
    	skb = __vlan_hwaccel_put_tag(skb, g_Vlan_tci);
        skb->data += 14;
        skb->mark = 10;
    }
}
...
static int oam_loopback_recv_packets_handler(struct sk_buff *skb)
{
	const unsigned char *dest = eth_hdr(skb)->h_dest;
    const u8 oam_group_address[ETH_ALEN] = { 0x01, 0x80, 0xc2, 0x00, 0x00, 0x02 }; 
    struct sk_buff *pstTmp = NULL;
    ...
    if (NULL != pstTmp)
    {
	    ...
	    if( (skbClone = skb_clone(pstTmp, GFP_ATOMIC)) != NULL )
	    {
		    local_irq_save(flags);
		    local_irq_enable();
		    dev_queue_xmit(skbClone);
...
typedef int (*netif_voip_hook)(struct sk_buff     *skb, 
                               struct net_device  *dev);
netif_voip_hook g_netif_voip_hook;
...
#if defined(CONFIG_HISI_HW_IPSEC) || defined(CONFIG_HISI_HW_IPSEC_MODULE)
int netdev_rebuild_icmp_pkt(struct sk_buff *skb)
{
    u32 len  = 0;
    u8 *in1  = NULL;
    u8 *out1 = NULL;
    ...
    /* 处理网卡收上来的icmp包 */
    if (!(strncmp(skb->dev->name, "eth0", 4)) && (0x01 == skb->data[9]))
	{
        SECDRV_DEBUG("original packet .\r\n");
        hisec_dump_hex_data(skb->data, skb->len);

        /* 拷贝一份数据包 */
        xmit_skb = skb_copy(skb, GFP_ATOMIC);
        if (SEC_TYPE_CRYPTO == g_sec_alg_seq)
        {
            /** 
             * 加密或解密:
             * in1(待加密数据), icmp包数据部分
             * out1(加密后数据), 替换原icmp数据部分
             * in2, NULL
             * out2, NULL
             */
            in1  = &xmit_skb->data[28];
            out1 = in1;
            len  = xmit_skb->len - 28;
        }
        else if (SEC_TYPE_AUTH == g_sec_alg_seq)
        {
            /* 增加认证数据长度并填充0 */
            if ((skb_cow_data(xmit_skb, SEC_AUTH_MAC_SIZE_MAX, &tmp_skb)) < 0)
            {     
                SECDRV_ERROR("skb_cow_data error !\r\n");
    	        return -1;
            }
            /** 
             * 认证:
             ...
             */
            in1  = &xmit_skb->data[28];
            out1 = skb_tail_pointer(tmp_skb);
            len  = xmit_skb->len - 28;

            memset(out1, 0x00, SEC_AUTH_MAC_SIZE_MAX);
            pskb_put(xmit_skb, tmp_skb, SEC_AUTH_MAC_SIZE_MAX);
        }
        else if (SEC_TYPE_DEAUTH == g_sec_alg_seq)
        {
            /** 
             * 去认证:
             * in1(待认证数据), icmp包数据部分
             * out1(携带认证数据地址), 报文携带认证数据地址
             * in2, NULL
             * out2, NULL
             */
            in1  = &xmit_skb->data[28];
            out1 = skb_tail_pointer(xmit_skb);
            len  = xmit_skb->len - 28;
        }
        else if (SEC_TYPE_CRYPT_AUTH == g_sec_alg_seq)
        {
            /* 增加认证数据长度并填充0 */
            if ((skb_cow_data(xmit_skb, SEC_AUTH_MAC_SIZE_MAX, &tmp_skb)) < 0)
            {
             ...
             * out1(加密后数据), 替换原icmp数据部分
             * in2(待认证数据), 加密后的数据部分
             * out2(认证后数据), 存放在加密数据后面
             */
            in1  = &xmit_skb->data[28];
            out1 = in1;
            in2  = in1;
            out2 = skb_tail_pointer(tmp_skb);
            len  = xmit_skb->len - 28;
            
            memset(out2, 0x00, SEC_AUTH_MAC_SIZE_MAX);
            pskb_put(xmit_skb, tmp_skb, SEC_AUTH_MAC_SIZE_MAX);   
        }
        else if (SEC_TYPE_DEAUTH_DECRYPT == g_sec_alg_seq)
        {
            /** 
             * 先认证后加密:
             * in1(待认证数据), icmp数据部分
             * out1(携带认证数据地址), 报文携带认证数据地址
             * in2(待加密数据), icmp数据部分(不包含原认证数据)
             * out2(加密后数据), 替换原icmp数据部分
             */
            in1  = &xmit_skb->data[28];
            out1 = skb_tail_pointer(xmit_skb);
            in2  = in1;
        ...
        {
            SECDRV_ERROR("undefined process type !\r\n");
            return -1;
        }
        
        /* 根据hook函数返回值缩减skb */
        len = hisec_cli_process_hook(in1, out1, len, in2, out2);
        if (-1 == len)
        {
            SECDRV_ERROR("hisec_cli_process_hook error !\r\n");
            return -1;
        }
        else
        {
        ...
        SECDRV_DEBUG("rebuild packet .\r\n");
        hisec_dump_hex_data(xmit_skb->data, xmit_skb->len);
        
		skb_push(xmit_skb, ETH_HLEN);
		MirrorPacket(xmit_skb, "eth0.5");
		kfree_skb(xmit_skb);
	}
...
#endif
void ATP_Mirror_Packet(struct sk_buff *skb)
{
    struct sk_buff *pstTmp = NULL;
	int i = 0;
    if (1 == port_mirror_enable)
...
static inline int gre_kernel_accel_recv(struct sk_buff *skb)
{
    ...
    if (IPPROTO_GRE == IPGRE_SKB_CB(skb)->ipgre_proto)
    {
        ...
        if (HYBRID_ACCELED == gre_kernel_accel(skb))
        {
            return HYBRID_ACCELED;
        }
    }
    else
    {
        ...
        switch (ntohs(skb->protocol))
        {
            case ETH_P_IP:
            {
                pst_ipv4hdr = (struct iphdr*)skb->data;
                ...
                else if (IS_LAN_DEV(skb->dev->name))
                {
            ...
            case ETH_P_IPV6:
            {
                pst_ipv6hdr = (struct ipv6hdr*)skb->data;
                if (IPPROTO_GRE == pst_ipv6hdr->nexthdr)
                {
#if 0  /*open after ipv6 gre explanted*/
                 	skb_reset_network_header(skb);
                	skb_reset_mac_len(skb);
...
int netif_receive_skb(struct sk_buff *skb)
{
    ...
    /*Igmp control packets have vlan but igmp data have no vlan, we should
      tag the igmp data packets with igmp control vlan
      **Make sure the drive didn't set the vlan before***/
#ifdef CONFIG_TAG_NOVLAN_IPTV_PACKETS
    tag_igmp_vlan_for_iptv_packets(skb);
#endif
    ...
    ATP_Mirror_Packet(skb);

#if defined(CONFIG_HISI_HW_IPSEC) || defined(CONFIG_HISI_HW_IPSEC_MODULE)
	if (NULL != hisec_cli_process_hook)
	{
        netdev_rebuild_icmp_pkt(skb);
	}
#endif
    ...
    if (oam_loopback_enable)
    {
    	...
    	if (oam_loopback_recv_packets_handler(skb))
        {
        	kfree_skb(skb);
            goto out;
        }
    ...
    /* Add by h00190880 for WS323 易用性 DNS重定向 2012.9.15 */
    if(1 == net_msg_dns)
    {
        if (true == IsDnsNeedResponds(skb))
        {
            kfree_skb(skb);
            goto out;
        }
    }
/*此处是wlan在repeater模式下二层DNS解析直接返回结果的代码处理，如果没有repeater模式则不生效*/
...
#endif
	/* START MODIFY by y00181549 for DTS2013092901584 at 2013/9/29*/
#ifdef CONFIG_ATP_HYBRID
    if ((skb->protocol == htons(ETH_P_8021Q)) && (!strcmp(skb->dev->name,"nas_p1")))
    {
        list_for_each_entry_rcu(ptype, &ptype_all, list) {
            if (!ptype->dev || ptype->dev == skb->dev) {
                if (pt_prev)
...
#endif
   /* END MODIFY by y00181549 for DTS2013092901584 at 2013/9/29*/
...
/*END MODIFY:t00176367 20131207 for DTS2013113001315*/
	if (((skb->protocol == __constant_htons(ETH_P_8021AG)) || 
		 (skb->protocol == __constant_htons(ETH_P_8021Q) && *(unsigned short *) &skb->data[2] ==__constant_htons(ETH_P_8021AG))))
    {   
    
#ifdef CONFIG_HSAN    
        //wan-side packet
        if (strstr(skb->dev->name, HISI_PTM_ROOT_DEV))
        {
            unsigned short vlanid = 0;
            //a. put_vlan_tag
            if (!hisi_get_vlan(skb, &vlanid))
            {
            	skb->data -= ETH_HLEN;
                skb = __vlan_put_tag(skb, vlanid);
            	skb = __vlan_hwaccel_put_tag(skb, vlanid);
                skb->data += ETH_HLEN;
            }
            //b. change skb->dev to root dev
            skb->dev = __dev_get_by_name(&init_net, HISI_PTM_ROOT_DEV);            
        }
#endif

        if (p8021ag_hook && !p8021ag_hook(skb))
        {
            goto out;                
        }
...
#if defined(CONFIG_BCM_SMUX) || defined(CONFIG_SMUX)
        /* 
...
/*<DTS2014101502698 t00176367 20141014 begin*/
//统计广播组播收发包数据因涉及netdevice.h头文件修改需要海思更新驱动，这里暂时只返回0值
static void dev_seq_printf_stats(struct seq_file *seq, struct net_device *dev)
{
	const struct net_device_stats *stats = dev_get_stats(dev);

	seq_printf(seq, "%6s:%8lu %7lu %4lu %4lu %4lu %5lu %10lu %9lu %9lu"
		   " %8lu %7lu %4lu %4lu %4lu %5lu %7lu %10lu %9lu %9lu\n",
		   ...
		   stats->rx_compressed, stats->multicast,0,
...
static int dev_seq_show(struct seq_file *seq, void *v)
{
	if (v == SEQ_START_TOKEN)
		seq_puts(seq, "Inter-|   Receive                            "
			      ...
			      "compressed multicast broadcast|bytes    packets errs "
			      "drop fifo colls carrier compressed multicast broadcast\n");
	else
		dev_seq_printf_stats(seq, v);
	return 0;
}
/*DTS2014101502698 t00176367 20141014 end>*/
...
static int dev_ifsioc(struct net *net, struct ifreq *ifr, unsigned int cmd)
{
	...
	switch (cmd) {
        ...
        /*set wan service:1.Internet, 2.Other */
        case SIOCSWANSERVICE:
            dev->flags |= ifr->ifr_ifru.ifru_ivalue;
            return 0;
#endif            
		...
		case SIOCSIFSTATS:
			...
			{
				struct net_device_stats * pStats;			
				if (dev->netdev_ops == NULL || dev->netdev_ops->ndo_get_stats == NULL || dev->flags & IFF_UP)
				{
					return 0;
				}
				else
...
int dev_ioctl(struct net *net, unsigned int cmd, void __user *arg)
{
	...
	switch (cmd) {
        ...
        */
		...
		case SIOCOAMLOOPBACK:
            oam_loopback_enable = (int)ifr.ifr_data;
            printk("oam_loopback_enable is %d\n", oam_loopback_enable);
            return 0;
		case SIOCGIFMEM:
...
#ifdef CONFIG_VLAN0
    // nas1_x
    set_inf_flag(dev->name, "nas1_", 1);
    // nas1.0_
    set_inf_flag(dev->name, "nas1.0_", 1);
#endif
...
static void netdev_wait_allrefs(struct net_device *dev)
{
	...
	while (atomic_read(&dev->refcnt) != 0) {
		...
		if (time_after(jiffies, warning_time + 10 * HZ)) {
            ...
            /*start of Hub测试中遇到ADSL激活后PPPOA始终无法不能上线的问题，ADSL重新激活也不行，重启后可恢复 2010-08-16 */
            atomic_set(&dev->refcnt, 0);            
            /*end of Hub测试中遇到ADSL激活后PPPOA始终无法不能上线的问题，ADSL重新激活也不行，重启后可恢复 2010-08-16 */
...
#ifdef CONFIG_VLAN0
    // nas1_x
    set_inf_flag(dev->name, "nas1_", 0);
    // nas1.0_
    set_inf_flag(dev->name, "nas1.0_", 0);
#endif
