...
/* Start of modified by f00120964 for dscpcount 2012-03-22 */
#if defined(CONFIG_DSCP_QOS)
#include <linux/ppp_defs.h>
#include <linux/if_vlan.h>
#include <linux/ip.h>
#include <linux/if_ether.h>
#define MAX_HEADER_LAYER    (8)
#define MAX_DSCP_VALUE      (64)
...
{
    __u8 iEnable;
    uint32_t u32DSCPValue;
};

struct DSCPArray
{
    int32_t iEnable;
    struct DSCPdata astDSCPValue[MAX_DSCP_VALUE];
};

static struct DSCPArray g_stDSCPGlobal;

// TODO: 注意这里与bcmenet中 BcmEnet_hdr 要保持一致
#ifndef BRCM_TYPE
...
// TODO: 注意这里需要与fap4ke_packet.h定义保持一致
typedef struct {
    uint32_t brcmTag;
    uint16_t etherType;
...
static unsigned int dscpcountarray[DSCP_COUNT_STATMAX] = {0};
#endif
/*End of modified by f00120964 for dscpcount 2012-03-22*/
...
#if !defined(CONFIG_BROADCOM_RELEASE_VERSION_4_12_L08) // Use for L07
...
#if defined(CONFIG_BROADCOM_RELEASE_VERSION_4_12_L08) // Use for L08
...
#define PPPOE_HLEN  (6)
#endif
#define GET_DSCP_ERROR (-1)
#define PPP_IP_PROTO(data) ((data[0] << 8) + data[1])
#define PARSE_PACKET(proto, offset)             encap = proto;          \
                                                puszData += (offset);
static int get_dscp_value(FkBuff_t * fkb_p, struct net_device * dev_p,
                         uint32_t encap, uint32_t phyHdr)
{
    //struct sk_buff *skb = NULL;
    unsigned char *puszData = NULL;
    unsigned char *puszDataEnd = NULL;
    int iHdrLyerCnt = 0;

    if (!fkb_p || !fkb_p->data)
    {
        return GET_DSCP_ERROR;
    }

#if 0
    fkbC_p = fkb_clone( fkb_p );
    if (!fkbC_p)
    {
        return GET_DSCP_ERROR;
    }
...
#if 0
    //skb = nbuff_xlate(FKBUFF_2_PNBUFF(fkb_p));    /* translate to skb */
    skb = fkb_xlate(fkb_p);
    if (skb == (struct sk_buff *)NULL)
    {
        return GET_DSCP_ERROR;
    }
    puszData = (unsigned char *)(skb->data);
#endif

    puszData = fkb_p->data;
    puszDataEnd = fkb_p->data + fkb_p->len;
    
    while (1)
    {
        if ((MAX_HEADER_LAYER < iHdrLyerCnt)
            || (puszData > puszDataEnd))
        {
            return GET_DSCP_ERROR;
        }
        switch(encap)
        {
            ...
            case TYPE_PPP:
                if (PPP_IP != PPP_IP_PROTO(puszData))
                {
                    return GET_DSCP_ERROR;
                }
                //COLOR_DEBUG("type PPP\r\n");
                PARSE_PACKET(PPP_IP, 2);
                break;
            ...
            case TYPE_ETH:
                ...
                //COLOR_DEBUG("type ethernet\r\n");
                PARSE_PACKET(((struct ethhdr *)puszData)->h_proto, ETH_HLEN);
                break;
#if 0
            case TYPE_ETH_P_BCM:
                PARSE_PACKET(((fap4kepkt_bcmheader_t *)puszData)->etherType, sizeof(fap4kepkt_bcmheader_t));
                break;
#endif
            case BRCM_TYPE:
                PARSE_PACKET(((bcmenet_hdr *)puszData)->encap_proto, sizeof(bcmenet_hdr));
                break;
                
            case TYPE_ETH_P_8021Q:
                if (!iHdrLyerCnt)
                {
                    return GET_DSCP_ERROR;
                }
                //COLOR_DEBUG("type vlan\r\n");
                PARSE_PACKET(((struct vlan_hdr *)puszData)->h_vlan_encapsulated_proto, VLAN_HLEN);
                break;
                
            case TYPE_ETH_P_PPP_SES:
                if (!iHdrLyerCnt)
                {
                    return GET_DSCP_ERROR;
                }
                //COLOR_DEBUG("type pppoe session\r\n");
                PARSE_PACKET(TYPE_PPP, PPPOE_HLEN);
                break;

            default:
                //COLOR_DEBUG("unknown type: %0x\r\n", encap);
                return GET_DSCP_ERROR;
        }
        iHdrLyerCnt++;
    }
...
get_dscp:
    return (((struct iphdr *)puszData)->tos >> 2);
}
#endif
/*End of modified by f00120964 for dscpcount 2012-03-22*/
...
/* Start of modified by f00120964 for dscpcount 2012-03-22 */
    ...
    int iDscp = 0;
    #if 0
    if (g_stDSCPGlobal.iEnable 
        && ((iDscp = get_dscp_value(fkb_p, dev_p, encap, phyHdr)) != GET_DSCP_ERROR)
        && g_stDSCPGlobal.astDSCPValue[iDscp].iEnable)
    #endif

    if (g_stDSCPGlobal.iEnable && dev_p
        && (iDscp = get_dscp_value(fkb_p, dev_p, encap, phyHdr)) != GET_DSCP_ERROR)
    {
    /*start of modify by h00170023 at 20101117 for DTS2010111500064 网关DSCP报文统计功能
    可以统计LAN到WAN方向的报文，BT要求只统计WAN到LAN方向的报文，不符合BT要求*/
        if ((0 <= iDscp) && (MAX_DSCP_VALUE > iDscp) && ((struct net_device *)dev_p)->name[0] == 'n')
    /*end of modify by h00170023 at 20101117 for DTS2010111500064 网关DSCP报文统计功能
    可以统计LAN到WAN方向的报文，BT要求只统计WAN到LAN方向的报文，不符合BT要求*/
        {
            g_stDSCPGlobal.astDSCPValue[iDscp].u32DSCPValue++;
            //printk("get dscp value: %d  %d\r\n", 
                 //iDscp, g_stDSCPGlobal.astDSCPValue[iDscp].u32DSCPValue);
        }
    }
#endif
/* End of modified by f00120964 for dscpcount 2012-03-22 */
...
#if defined(CONFIG_SMP) || defined(CONFIG_PREEMPT)
    /* start 启用QoS后，大流量发包导致死锁 DTS2012010301569 by f00110348 20120105 */
    if (spin_is_locked(&blog_lock_g))
    {
        blog_free( skb_p );
        goto bypass;
    }
    /* end 启用QoS后，大流量发包导致死锁 DTS2012010301569 by f00110348 20120105 */
#endif
...
/* Start of modified by f00120964 for dscpcount 2012-03-22 */
#if defined(CONFIG_DSCP_QOS)
int32_t blog_getdscpstatisticdata(int32_t index)
{
#if 0
    if (!g_stDSCPGlobal.iEnable
        || (0 > index || MAX_DSCP_VALUE <= index)
        || !g_stDSCPGlobal.astDSCPValue[index].iEnable)
 #endif
    if (!g_stDSCPGlobal.iEnable
        || (0 > index || MAX_DSCP_VALUE <= index))
    {
    ...
    return g_stDSCPGlobal.astDSCPValue[index].u32DSCPValue;
}

void blog_setdscpswitch(int32_t iEnable)
{
    g_stDSCPGlobal.iEnable = iEnable;
}

void blog_cleardscpstatisticdata(int32_t index)
{    
    g_stDSCPGlobal.astDSCPValue[index].u32DSCPValue = 0;
}
...
/* Start of modified by f00120964 for dscpcount 2012-03-22 */
#if defined(CONFIG_DSCP_QOS)
EXPORT_SYMBOL(blog_getdscpstatisticdata);
EXPORT_SYMBOL(blog_setdscpswitch);
EXPORT_SYMBOL(blog_cleardscpstatisticdata);
#endif
/* End of modified by f00120964 for dscpcount 2012-03-22 */
