...
u32 ps_4k_alloc_cnt = 0;
u32 ps_4k_free_cnt = 0;
#endif

#define ATHR_SRAM_START                      0xbd000000
volatile uint32_t *s_ram = (volatile uint32_t *)ATHR_SRAM_START;
...
void priv_skb_init(void)
{
    ...
    ps_2k_free_cnt = 1024;
    ps_4k_alloc_cnt = 0;
    ps_4k_free_cnt = PRIV_SKB4K_MAX ; //1536;
    s_ram[0] = ps_4k_alloc_cnt ;
    s_ram[1] = ps_4k_free_cnt ;
#endif
...
void priv_skb_dump_4k(void)
{
    u8 *skbmem;
    u32 ps_count = 0;
    unsigned long flags = 0;
 
     while (ps_count < PRIV_SKB4K_MAX) {
         skbmem = s_ram[ps_count +2 ];
         if (skbmem != NULL) {
			struct ethhdr *eth;
             struct sk_buff *skb = (struct sk_buff *)skbmem;
             printk("%s the %d 4k_skb and name is %s \n",__func__,ps_count,skb->dev->name);
			if ( eth = skb_mac_header( skb)   != NULL ) {
				printk("%s the %d ps_count packet src is 0x%02x and dest is 0x%02x \n",
						__func__,ps_count , eth->h_source, eth->h_dest );
			}
         }
         ps_count++;           
     }
			    ...
			    int fclone, int node)
{
...
#ifdef PRIV_SKB_DEBUG
	{
		...
		if(interval >= 5000) {
			printk("\n%s() Line %d: watchdog control[0xB8060008 = 0x%08X] timer[0xB806000C = 0x%08X] \n", __func__, __LINE__,
					*(volatile unsigned int *)(0xB8060008), *(volatile unsigned int *)(0xB806000C));

			printk("%s interval %lu: alloc size %d data %d skbuff 2k_alloc is %d 2k_free is %d(+%d), 4k_alloc is %d 4k_free is %d(+%d) \n",\
					__func__, interval, size, size +  sizeof(struct skb_shared_info),
					ps_2k_alloc_cnt, ps_2k_free_cnt, ps_2k_alloc_cnt - ps_2k_free_cnt,
					ps_4k_alloc_cnt, ps_4k_free_cnt, ps_4k_alloc_cnt - ps_4k_free_cnt);
			interval = 0;
		}
		interval++;
	}
...
#ifdef CONFIG_PRIV_SKB_MEM
    if (unlikely(((size+ sizeof(struct skb_shared_info)) > PRIV_BUFSIZE_4K) || ((data = priv_skbmem_get(size + 
            sizeof(struct skb_shared_info))) == NULL))) {

#ifdef PRIV_SKB_DEBUG 
	   printk("%s alloc size %d data is %d skbuff failed 2k_alloc is %d 2k_free is %d 4k_alloc is %d 4k_free is %d ps_head4k :%d ps_tail4k:%d \n",\
		    __func__,size ,size+sizeof(struct skb_shared_info) ,ps_2k_alloc_cnt,ps_2k_free_cnt,ps_4k_alloc_cnt,ps_4k_free_cnt,ps_head4k,ps_tail4k);
	   //priv_skb_dump_4k();
#endif
...
#endif
	if (!data){
		printk("*********SKB mem fail from SLAB memory too...\n");
		goto nodata;
	}
	...
	if (!skb->cloned ||
		...
		if (skb_shinfo(skb)->nr_frags) {
			...
			for (i = 0; i < skb_shinfo(skb)->nr_frags; i++) {
				/*BRCM使用线程释放skb，导致此处出现空指针释放，暂时规避DTS2013082103912 */
#if defined(CONFIG_MIPS_BRCM) && defined(CONFIG_SMP)
				if (NULL == skb_shinfo(skb)->frags[i].page) {
					continue;
				}
#endif
        ...
        if (likely((skb->head - priv_skb_mem) < sizeof(priv_skb_mem))) {
        	...
        	printk("%s() Line %d: called kfree(), skb->head = 0x%08X, priv_skb_mem = 0x%08X \n",
				__FUNCTION__, __LINE__, skb->head, priv_skb_mem);
...
static void __copy_skb_header(struct sk_buff *new, const struct sk_buff *old)
{
	...
	new->lanindev		= old->lanindev;  /* add for lan interface iptables by f00110348 */
...
#if defined(CONFIG_MIPS_BRCM) && defined(CONFIG_BLOG) && defined(CONFIG_PPPOL2TP) && defined(CONFIG_L2TP_FAST_FORWARD)
		     ...
		     gfp_t gfp_mask)
{
    ...
    if (unlikely(((size+sizeof(struct skb_shared_info)) > PRIV_BUFSIZE_4K) || ((data = priv_skbmem_get(size + 
            sizeof(struct skb_shared_info))) == NULL))) {
#ifdef PRIV_SKB_DEBUG 
	    printk("%s alloc size %d data is %d skbuff failed 2k_alloc is %d 2k_free is %d 4k_alloc is %d 4k_free is %d(+%d) \n",\
		    __func__,size ,size+sizeof(struct skb_shared_info) ,ps_2k_alloc_cnt,ps_2k_free_cnt,ps_4k_alloc_cnt,ps_4k_free_cnt, ps_4k_alloc_cnt - ps_4k_free_cnt);
	    //priv_skb_dump_4k();
#endif
