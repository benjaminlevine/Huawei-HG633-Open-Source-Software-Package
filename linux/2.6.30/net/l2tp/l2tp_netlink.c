...
static struct l2tp_session *l2tp_nl_session_find(struct sk_buff *skb, struct genl_info *info)
{
	u32 tunnel_id;
	u32 session_id;
	char *ifname;
			       ...
			       struct l2tp_tunnel *tunnel)
{
	...
	switch (tunnel->encap) {
	case L2TP_ENCAPTYPE_UDP:
		NLA_PUT_U16(skb, L2TP_ATTR_UDP_SPORT, ntohs(inet->sport));
		NLA_PUT_U16(skb, L2TP_ATTR_UDP_DPORT, ntohs(inet->dport));
		NLA_PUT_U8(skb, L2TP_ATTR_UDP_CSUM, (sk->sk_no_check != UDP_CSUM_NOXMIT));
		/* NOBREAK */
	case L2TP_ENCAPTYPE_IP:
		NLA_PUT_BE32(skb, L2TP_ATTR_IP_SADDR, inet->saddr);
		NLA_PUT_BE32(skb, L2TP_ATTR_IP_DADDR, inet->daddr);
		break;
	}
...
static int l2tp_nl_cmd_session_delete(struct sk_buff *skb, struct genl_info *info)
{
	...
	session = l2tp_nl_session_find(skb, info);
	if (session == NULL) {
		ret = -ENODEV;
		goto out;
	}
