...
int l2tp_xmit_skb(struct l2tp_session *session, struct sk_buff *skb, int hdr_len)
{
	...
	case L2TP_ENCAPTYPE_UDP:
		...
		else if ((skb->dst && skb->dst->dev) &&
			 (!(skb->dst->dev->features & NETIF_F_V4_CSUM))) {
			skb->ip_summed = CHECKSUM_COMPLETE;
			csum = skb_checksum(skb, 0, udp_len, 0);
			uh->check = csum_tcpudp_magic(inet->saddr,
						      inet->daddr,
...
int l2tp_tunnel_create(struct net *net, int fd, int version, u32 tunnel_id, u32 peer_tunnel_id, struct l2tp_tunnel_cfg *cfg, struct l2tp_tunnel **tunnelp)
{
	...
	if (encap == L2TP_ENCAPTYPE_UDP) {
        ...
        /*START ADD: h00169677 2014219 for DTS2014021900641 */
		sk->sk_no_check = UDP_CSUM_NOXMIT;
        /*END ADD: h00169677 2014219 for DTS2014021900641 */
	}
...
static int __init l2tp_init(void)
{
	...
	rc = register_pernet_gen_device(&l2tp_net_id, &l2tp_net_ops);
	if (rc)
		goto out;
