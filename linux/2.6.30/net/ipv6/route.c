...
#ifdef CONFIG_HSAN
struct fib6_hook ip6_route_hook={NULL, NULL};

int fib6_hook_reg(struct fib6_hook* hook)
{
    ip6_route_hook.tb_fib6_add = hook->tb_fib6_add;
    ip6_route_hook.tb_fib6_del = hook->tb_fib6_del;
    return 0;
}

int fib6_hook_unreg(void)
{
    ip6_route_hook.tb_fib6_add = NULL;
    ip6_route_hook.tb_fib6_del = NULL;
    return 0;
}
EXPORT_SYMBOL(fib6_hook_reg);
EXPORT_SYMBOL(fib6_hook_unreg);
#endif
...
/*start by HSANV200R001C01B057 增加组播硬件加速功能 2013-3-30*/
#ifdef CONFIG_HSAN
static int __ip6_ins_rt2(struct rt6_info *rt, struct nl_info *info)
#else
...
static int __ip6_ins_rt(struct rt6_info *rt, struct nl_info *info)
{
    ...
    ret = __ip6_ins_rt2(rt, info);
    if (ret == 0 && ip6_route_hook.tb_fib6_add != NULL)
    {
        ip6_route_hook.tb_fib6_add(rt);
    }
...
/* start of by d00107688 2009-12-08 for AU4D02299 网关配置静态的IP地址，设置默认网关为24::2，网关不会自动添加默认路由，且添加指向该wan的静态路由也没有生效 */
#if 0
            ...
            if (gwa_type & (IPV6_ADDR_MULTICAST | IPV6_ADDR_LOOPBACK | IPV6_ADDR_ANY))
                goto out;
#endif
/* end of by d00107688 2009-12-08 for AU4D02299 网关配置静态的IP地址，设置默认网关为24::2，网关不会自动添加默认路由，且添加指向该wan的静态路由也没有生效 */
...
/*start by HSANV200R001C01B057 增加组播硬件加速功能 2013-3-30*/
#ifdef CONFIG_HSAN
static int __ip6_del_rt2(struct rt6_info *rt, struct nl_info *info)
#else
...
static int __ip6_del_rt(struct rt6_info *rt, struct nl_info *info)
{
    if (ip6_route_hook.tb_fib6_del != NULL)
    {
        ip6_route_hook.tb_fib6_del(rt);
    }

    return __ip6_del_rt2(rt, info);
}
#endif
/*end by HSANV200R001C01B057 增加组播硬件加速功能 2013-3-30*/
