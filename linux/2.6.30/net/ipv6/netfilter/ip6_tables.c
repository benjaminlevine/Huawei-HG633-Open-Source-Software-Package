...
#endif
#ifdef CONFIG_FIREWALL_LOG
static bool findudpfloodv6rule = false;
static int udpfloodv6time = 0;
#endif
	 ...
	 struct xt_match_param *par)
{
	...
	{
#ifdef CONFIG_FIREWALL_LOG	
		if  (0 == strcmp(m->u.kernel.match->name, "limit"))
		{
			if (true == findudpfloodv6rule)
			{
				/* ipv6 UDP Flood attack */
				if(0 == udpfloodv6time 
				|| time_after(jiffies, udpfloodv6time))
				{
					udpfloodv6time = jiffies + FIREWALL_ATTACK_LOG_INTERVAL*HZ;
					pszLogBuf = (u_int8_t*)kmalloc(ATP_LOG_LENGTH_128, GFP_ATOMIC);
					if (NULL != pszLogBuf)
					{
						memset(pszLogBuf, 0, ATP_LOG_LENGTH_128);
						snprintf(pszLogBuf, ATP_LOG_LENGTH_128, "Detect ipv6 UDP Flood Attack, packet from %pI6.\n", &(ipv6_hdr(skb)->saddr));
...
#ifdef CONFIG_FIREWALL_LOG
	if (('m' == table->name[0]) && (NF_INET_PRE_ROUTING == hook) && in && out
        && ('p' == in->name[0]) && (0 == strlen(out->name)))
	{
		if (IPPROTO_UDP == ipv6_hdr(skb)->nexthdr)
		{
			wanudpv6packet = true;
		}
	}
...
#ifdef CONFIG_FIREWALL_LOG
			findudpfloodv6rule = false;
			if ((true == wanudpv6packet) && (IPPROTO_UDP == e->ipv6.proto))
			{
				findudpfloodv6rule = true;
			}
#endif	
