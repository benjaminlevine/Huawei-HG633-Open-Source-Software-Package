 ...
 */
/* 注意，如果需要将某一个接口设置为具有转发功能，那么这种情况下
 * 就需要将该接口通过MIF相关的setscokopt加入到本数组中
 * 这样该接口才具有多播转发的功能，也就是说，如果没有加入到该表中的接口，那么就
 * 没有转发功能
 */
...
/* 因为一个多播报文加入被加入到某一个转发接口vif之后，那么该接口可能由于某种原因暂时无法转发该报文，因此
 * 就将该报文暂时加入缓存mfc中，
 */
					   ...
					   struct in6_addr *mcastgrp)
{
	...
	/*start by t00176367 for DTS2013011500879 MLD组播失效修改*/
    	/* for ASM multicast source does not matter so need to check
	   for an entry with NULL origin as well */
	if(c == NULL) {
		static struct in6_addr nullOrigin;
	...
	/*end by t00176367 for DTS2013011500879 MLD组播失效修改*/
...
ip6mr_cache_unresolved(struct net *net, mifi_t mifi, struct sk_buff *skb)
{
	...
	if (c == NULL) {
		/*
		 *	Create a new entry if allowable 注意这里，写死的
		 */
		...
		/*
		 *	Fill in the new cache entry  构造一个新的cache
		 */
		c->mf6c_parent = -1;
		...
		/*
		 *	Reflect first query at pim6sd 告知用户态需要创建这个cache，因此此时用户态就需要接受这个报文然后添加对应的路由信息
		 */
	...
	/*
	 *	See if we can append the packet，如果该cache缓存的报文个数大于3个了，那么就不要缓存了直接丢弃
	 */
...
static int ip6mr_forward2(struct sk_buff *skb, struct mfc6_cache *c, int vifi)
{
   ...
   /* skb->dev is the soruce device. It should not be 
	...
	/*start by t00176367 for DTS2013011500879 MLD组播失效修改*/
	return NF_HOOK(PF_INET6, NF_INET_FORWARD, skb, indev, dev,
...
static int ip6_mr_forward(struct sk_buff *skb, struct mfc6_cache *cache)
{
	...
	int vif, ct;
	struct net *net = mfc6_net(cache);
/* BEGIN: Added by z67728, 2010/2/6 ready认证中多播报文测试项. */
    struct dst_entry *dst = skb->dst;
/* END:   Added by z67728, 2010/2/6 */
...
/* BEGIN: Added by z67728, 2010/2/6 ready认证中多播报文测试项. */
#if 0
    printk("\r\n skbdev[%s] dstdev[%s] packetlength[%u] mtu[%u] \r\n",skb->dev->name,dst->dev->name,skb->len,dst_mtu(dst));
#endif
	if (skb->len > dst_mtu(dst)) {
		/* Again, force OUTPUT device used as source address */
		skb->dev = dst->dev;
		icmpv6_send(skb, ICMPV6_PKT_TOOBIG, 0, dst_mtu(dst), skb->dev);
		IP6_INC_STATS_BH(dev_net(dst->dev),ip6_dst_idev(dst), IPSTATS_MIB_INTOOBIGERRORS);
		IP6_INC_STATS_BH(dev_net(dst->dev),ip6_dst_idev(dst), IPSTATS_MIB_FRAGFAILS);
		goto dont_forward;
	}
/* END:   Added by z67728, 2010/2/6 */
...
int ip6_mr_input(struct sk_buff *skb)
{
	...
	struct net_device *dev = skb->dev;
      ...
      /* start of by d00107688 2009-12-15 对于MLD V1报文来说是没有源的，因此这种情况下也需要可以找到路由才行 */    
      int vif;
      /* end of by d00107688 2009-12-15 对于MLD V1报文来说是没有源的，因此这种情况下也需要可以找到路由才行 */    
    ...
    {
     /*Add by DHS00169988 */
        int line = MFC6_HASH(&ipv6_hdr(skb)->daddr, &ipv6_hdr(skb)->saddr);
        vif = ip6mr_find_vif(skb->dev);
              
        for (cache = net->ipv6.mfc6_cache_array[line]; cache; cache = cache->next)
        {
            if (IN6_ARE_ADDR_EQUAL(&cache->mf6c_mcastgrp, &ipv6_hdr(skb)->daddr)
                && (cache->mf6c_parent == vif))
            {
                break;
            }
        }
