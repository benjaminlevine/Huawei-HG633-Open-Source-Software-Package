...
#define ICMP_DEBUG 0
#define ICMP_CHECKSUM_MAX_BUFFER_LEN  2048
...
typedef struct tag_ICMP_RS_PACK_CHECKSUM_S
{
    ...
    unsigned char  uszTemBuff[ICMP_CHECKSUM_MAX_BUFFER_LEN];     /* icmp packet content */
...
IcmpRsCheckSum(ICMP_RS_PACK_CHECKSUM_S * pstCheckSum)
{
    ...
    for ( lLoopi = 0 ; lLoopi < pstCheckSum->usLength; lLoopi++ )
    {
        ...
        printk("%02x ",pstCheckSum->uszTemBuff[lLoopi]);    
    }
#endif
...
static int icmpv6_rcv(struct sk_buff *skb)
{
    ...
    { 
        ...
        /* BEGIN: Added by z67728, 2009/12/14 for ready test, if the checksum of the packet is wrong,
        discard the packet. */
        ICMP_RS_PACK_CHECKSUM_S* pstCheckSum = NULL;
        struct ipv6hdr *ipv6hdr;
        ipv6hdr = ipv6_hdr(skb);
        
        pstCheckSum = (ICMP_RS_PACK_CHECKSUM_S*)kmalloc(sizeof(ICMP_RS_PACK_CHECKSUM_S), GFP_ATOMIC);
        if (NULL == pstCheckSum)
        {
        	break;
        }
        
        memset(pstCheckSum, 0, sizeof(ICMP_RS_PACK_CHECKSUM_S));
        
        pstCheckSum->usType   = 58;
        pstCheckSum->usLength = ntohs(ipv6hdr->payload_len);

        /*超长包不做校验*/
        if (ICMP_CHECKSUM_MAX_BUFFER_LEN <= pstCheckSum->usLength)
        {
            ndisc_rcv(skb);
            kfree(pstCheckSum);
            break;
        ...
        memcpy(&(pstCheckSum->sin6_saddr), saddr, sizeof(struct in6_addr));
        memcpy(&(pstCheckSum->sin6_daddr), daddr, sizeof(struct in6_addr));
        memcpy(&(pstCheckSum->uszTemBuff), (unsigned char *)hdr, pstCheckSum->usLength);
        
        if ( 0 > IcmpRsCheckSum(pstCheckSum) )
        {
        	kfree(pstCheckSum);
            goto discard_it;
        }
        /* END:   Added by z67728, 2009/12/14 */
