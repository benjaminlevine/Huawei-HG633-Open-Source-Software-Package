...
/* BEGIN: Added by h00169677, 2012/6/6   PN:DTS2012052605775 */
#define BIND6_FWMARKRULEDEV_TABLE_START  200
#define	RTF_UP		0x0001		/* route usable		  	*/
#define	RTF_GATEWAY	0x0002		/* destination is a gateway	*/
static void ndisc_del_policy_route(struct in6_addr *addr,struct net_device *dev)
{
	struct net_device *temdev ;
	struct net *net;
	int bridgeindex = 0;
	char bridgename[TASK_COMM_LEN] = {0};
	
	for_each_net(net) 
	{
        for_each_netdev(net, temdev) 
		{
			if(temdev->priv_flags & IFF_EBRIDGE)
			{
				memset(bridgename,0,sizeof(bridgename));
				memcpy(bridgename,temdev->name,strlen(temdev->name));
				
				//bridgename like br0,br1,br2....  and count > 10 ? NO!NO!
				sscanf(bridgename+2,"%d",&bridgeindex);
				{
					struct rt6_info *rt;
					struct fib6_table *table;
					struct neighbour *neigh = NULL;

					table = fib6_get_table(dev_net(dev), BIND6_FWMARKRULEDEV_TABLE_START+bridgeindex);
					if (table == NULL)
					{
						continue;
					}
					...
					{
						if (dev == rt->rt6i_dev &&
						    ((rt->rt6i_flags & (RTF_UP | RTF_GATEWAY)) == (RTF_UP | RTF_GATEWAY)) &&
...
/* END:   Added by h00169677, 2012/6/6 */
static void ndisc_router_discovery(struct sk_buff *skb)
{
...
#ifdef CONFIG_SUPPORT_ATP
    if ((!in6_dev->dev) 
        || (!IS_WAN_DEVICE(in6_dev->dev->name)) 
	...
	if (lifetime == 0)
	{
		ndisc_del_policy_route(&ipv6_hdr(skb)->saddr,skb->dev);
	}
	/* END:   Added by h00169677, 2012/6/6 */
	if (rt == NULL && lifetime) {
...
static void ndisc_redirect_rcv(struct sk_buff *skb)
{
    ...
    /* BEGIN: Added by z67728, 2009/11/27 IPv6Ready- Host Test v6LC.2.3.6 Part G: Target Address is Multicast */    
    else if (ipv6_addr_is_multicast(target)) {
		ND_PRINTK2(KERN_WARNING
			   "ICMPv6 Redirect: target address is multicast.\n");
		return;
    /* END:   Added by z67728, 2009/11/27 */
...
/*Begin of  DTS2010093001627. liyang l00168621 modify2010-10-27 .PPPoE路由WAN侧收到NS不回NA但记录邻居缓存  */
    struct inet6_dev *idev = NULL;
    struct net_device *pstDev = NULL;
    
    if (!skb->dev)
    {
        pstDev = skb->dst ?  skb->dst->dev  : NULL;
    }
    else
    {
        pstDev = skb->dev;
    ...
    if (pstDev)
    {   
        idev = __in6_dev_get(pstDev);
        if (idev)
        {
            if (IPV6_ENABLE != GetDevIpv6Enable(idev))
            {
                ND_PRINTK2(KERN_WARNING
			   "ICMPv6 NDISC: Drop the package.\n");
                return 0;
            }
        }
    }
/*End of  DTS2010093001627. liyang l00168621 modify2010-10-27 */
