...
#define BR_F_VLAN_PASS         0x00000002
...
extern int br_add_if(struct net_bridge *br,
	      struct net_device *dev,int ptype);
#else
extern int br_add_if(struct net_bridge *br,
	      struct net_device *dev);
#endif
...
extern struct sk_buff *br_handle_frame(struct net_bridge_port *p,
				       struct sk_buff *skb);
extern int IsSkbfromDhcpsPort(struct sk_buff *skb, int iIptype);
extern int DevDhcpRelayOn(const char* name);
...
/* BEGIN: Added by z67728, 2010/10/29 For BT IGMPv3 query packet. */
