...
<修改人>   <修改时间>   <问题单号>          <修改描述>
l00171355  2015/07/10  DTS2015071007527   【MLDv2】发送畸形icmpv6 mldv2报文导致网关死机
***********************************************************************/
...
void br_mld_snooping_init(void)
{
	...
	mld_snooping_list.mld_snooping_enable = 0;
}
...
static void br_mld_snooping_query_timeout(unsigned long ptr)
{
	...
	list_for_each_entry_rcu(dst_entry, &mld_list->mld_list, list) 
    {
    ...
    const unsigned char *grp_mac, unsigned char *host_mac)
{
	...
	mld_entry = 
        kmalloc(sizeof(struct net_bridge_mld_snooping_entry), GFP_KERNEL);
	if (!mld_entry)
	{
	    return ENOMEM;
	}	
...
#if defined(CONFIG_MIPS_BRCM) && defined(CONFIG_BLOG)
	blog_notify(MCAST_CONTROL_EVT, (void*)mld_entry,
...
/* <DTS2015071007527 同步自 DTS2015070208650 l00171355 20150710 begin */
/*!
  \author      l00180987
  \brief       mldv2报文校验，避免畸形报文导致死机
  \param[in]  *pstMLDv2Ptr: mldv2报文头
  \param[in]  *pstIP6Hdr: ipv6头
  \return 0:畸形mldv2报文;1:合法mldv2报文
  \retval ERROR_CODE 错误码说明
*/
static int br_mld_valid_v2(const struct mld2_reportpkt *pstMLDv2Ptr,
                           const struct ipv6hdr *pstIP6Hdr)
{
    int cur_group_num = 0;
    struct mld2_grecrd *pstGrec = NULL;
    int avail_len = 0;

    avail_len = ntohs(pstIP6Hdr->payload_len) - MLD_HOP_LEN - sizeof(struct mld2_reportpkt);

    /*第一个group最小长度*/
    avail_len -= sizeof(struct mld2_grecrd);
    if (avail_len < 0)
    {
        return 0;
    }

    pstGrec = &pstMLDv2Ptr->grec[0];

    cur_group_num = 1;
    while (cur_group_num <= ntohs(pstMLDv2Ptr->ngrec))
    {
        /*group整体长度*/
        avail_len -= (ntohs(pstGrec->grec_nsrcs) * sizeof(struct in6_addr));
        if (avail_len < 0)
        {
            return 0;
        }
        ...
        /*还有group需要处理*/
        if (cur_group_num < ntohs(pstMLDv2Ptr->ngrec))
        {
            /*剩余长度不得少于group 头长度*/
            avail_len -= sizeof(struct mld2_grecrd);
            if (avail_len < 0)
            ...
            pstGrec = (struct mld2_grecrd *)((char *)pstGrec + sizeof(struct mld2_grecrd) + ntohs(pstGrec->grec_nsrcs) * sizeof(struct in6_addr));
        }
...
/* DTS2015071007527 同步自 DTS2015070208650 l00171355 20150710 end> */
        ...
        struct net_bridge *br,const unsigned char *dest,int forward)
{
             ...
             || (ICMPV6_MLD2_REPORT == data[48]))) 
    {
        ...
        if (ICMPV6_MGM_REPORT == data[48])
        {
            ...
            br_mld_snooping_add(skb->dev->br_port, dest, eth_hdr(skb)->h_source);
        }
        else if (ICMPV6_MLD2_REPORT == data[48])        /* mldv2 报告报文 */
        {
			...
			/* <DTS2015071007527 同步自 DTS2015070208650 l00171355 20150710 begin */
            /*校验报文*/
            if (!br_mld_valid_v2(pstMLDv2Ptr, pstIP6Hdr))
            {
                MLD_SNOOPING_P("mld version 2 packet invalid...\r\n");
                return 0;
            }
			/* DTS2015071007527 同步自 DTS2015070208650 l00171355 20150710 end> */
            ...
            for (k = 0; k < iNum; k++)
            {
                ...
                {
        		    br_mld_snooping_add(skb->dev->br_port, mc_mac, eth_hdr(skb)->h_source);
        		}
                        ...
                        || (MLD_V2_MART_BLOCK_OLD_SOURCES == pstRdPtr->grec_type)) 
                {
        		    ...
        		    br_mld_snooping_del(skb->dev->br_port, mc_mac, eth_hdr(skb)->h_source);
        		}
