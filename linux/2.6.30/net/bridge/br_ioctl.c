...
#else
struct net_device *bridge_get_next_port(struct net_bridge *br, struct net_bridge_port **p)
{
	struct net_bridge_port *cp = *p;
	struct net_bridge_port *np;
    ...
    if (cp == NULL)
    {
        np = list_first_entry(&br->port_list, struct net_bridge_port, list);
        *p = np;
    } else if (list_is_last(&cp->list, &br->port_list))
        return NULL;
    else
    {
        np = list_first_entry(&cp->list, struct net_bridge_port, list);
...
static int add_del_if(struct net_bridge *br, int ifindex, int isadd, int ptype)
#else
...
static int old_dev_ioctl(struct net_device *dev, struct ifreq *rq, int cmd)
{
	...
	switch (args[0]) 
    {
    	...
    	case BRCTL_DEL_IF:
#ifdef CONFIG_ATH_WRAP
    		return add_del_if(br, args[1], args[0] == BRCTL_ADD_IF,args[3]);
#else
        ...
        {
            int iEnable = args[2];
            br_set_port_relay(args[1], iEnable);
            return 0;
        }
#endif
	}
...
/* BEGIN: Added by z67728, 2010/7/8 For BT IGMP statistic*/
#ifdef CONFIG_BR_IGMP_STATISTIC
    case BRCTL_SHOW_IGMP_STATIC :
	{
		br_igmp_statistic_print();
		return 0;
	}
	case BRCTL_SET_IGMP_STATIC :
	{
		if (!capable(CAP_NET_ADMIN))
			return -EPERM;
		br_igmp_statistic_setenable((int)args[1]);
		return 0;
	}
    case BRCTL_CLEAR_IGMP_STATIC :
    {
        br_igmp_statistic_clearallchain();
        return 0;
    }
    case BRCTL_SET_PORT_IGMP_STATIC :
    {
        unsigned int temargs[7] = {0};
        if ( copy_from_user(temargs, uarg, sizeof(temargs)) )
        {
            return -EFBIG;
        }

        br_igmp_statistic_setinterfaceport((char *)temargs[1],temargs[2],temargs[3],temargs[4],temargs[5]);
        return 0;
    }
#endif
/* END:   Added by z67728, 2010/7/8 */
...
int br_dev_ioctl(struct net_device *dev, struct ifreq *rq, int cmd)
{
	...
	int ptype = 0;
#ifdef CONFIG_ATH_WRAP	
	if (copy_from_user(&ptype, rq->ifr_data, sizeof(int)))
	{
		return -EFAULT;
	}
#endif
	...
	case SIOCBRDELIF:
#ifdef CONFIG_ATH_WRAP	
		return add_del_if(br, rq->ifr_ifindex, cmd == SIOCBRADDIF,ptype);
#else
