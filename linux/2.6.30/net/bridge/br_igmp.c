...
static int br_igmp_lan2lan_snooping = 1;
...
static void br_igmp_query_timeout(unsigned long ptr)
{
    ...
    /*组播抑制需同时解决lan侧设备report抑制后snooping被老化问题*/
...
#endif
/*start 插拔以太网线概率性死机DTS2012080401932*/
                if (dst->from_dev)
                {
                    dev_put(dst->from_dev);
                }
/*end 插拔以太网线概率性死机DTS2012080401932*/
...
/*start 插拔以太网线概率性死机DTS2012080401932*/
            if (mc_fdb_robo->from_dev)
            {
                dev_put(mc_fdb_robo->from_dev);
            }
/*end 插拔以太网线概率性死机DTS2012080401932*/
...
/******************************************************************************
  函数名称  : is_igmp_type_need_process
  功能描述  : 校验igmp type类型，判断是否需要博通snooping处理
  输入参数  : 
              1.  type:igmp type字段
  调用函数  : 
  被调函数  : 
  输出参数  : 无
  返 回 值  : 1:博通snooping需要处理的igmp报文类型
             0:博通snooping不需要处理的igmp报文类型
  修改历史      :
   1.日    期   : 2013-10-10
     作    者   : l00180987
     修改内容   : 完成初稿
...
******************************************************************************/
static int is_igmp_type_need_process(__u8 type)
{
	if (IGMPV3_HOST_MEMBERSHIP_REPORT == type)
	{
		return 1;
	}
	...
	if (IGMP_HOST_MEMBERSHIP_REPORT == type)
	{
		return 1;
	}

	if (IGMPV2_HOST_MEMBERSHIP_REPORT == type)
	{
		return 1;
	}
...
/******************************************************************************
  函数名称  : is_igmp_v3_valid
  功能描述  : 判断igmp v3报文是否合法
  输入参数  : 
              1.  *pstRepV3:igmp v3报文
              2.  *pip:数据包ip头
  调用函数  : 
  被调函数  : 
  输出参数  : 无
...
******************************************************************************/
static int is_igmp_v3_valid(struct igmpv3_report *pstRepV3, const struct iphdr *pip)
{
	int cur_group_num = 1;
	struct igmpv3_grec   *pstGrec = NULL;
	int avail_len = pip->tot_len - (pip->ihl << 2) - sizeof(struct igmpv3_report);

	/*第一个group最小长度*/
	avail_len -= sizeof(struct igmpv3_grec);
	if (avail_len < 0)
	{
		br_igmp_debug("is_igmp_v3_valid: invalid len about first group,line:%d\n", __LINE__);
		return 0;
	}
	...
	cur_group_num = 1;
	while (cur_group_num <= pstRepV3->ngrec)
	{
		/*group整体长度*/
		avail_len -= (pstGrec->grec_nsrcs * sizeof(struct in_addr));
		if (avail_len < 0)
		{
			br_igmp_debug("is_igmp_v3_valid: invalid len about group src,line:%d\n", __LINE__);
			return 0;
		}

		/*还有group需要处理*/
		if (cur_group_num < pstRepV3->ngrec)
		{
			/*剩余长度不得少于group 头长度*/
			avail_len -= sizeof(struct igmpv3_grec);
			if (avail_len < 0)
			{
				br_igmp_debug("is_igmp_v3_valid: invalid len about next head,line:%d\n", __LINE__);
				return 0;
			}
...
/******************************************************************************
  函数名称  : is_igmp_need_process
  功能描述  : 判断igmp报文是否需要博通snooping处理
              输入保证skb合法且是igmp报文，此处不重复判断
  输入参数  : 
              1.  *skb:igmp 报文
  调用函数  : 
  被调函数  : 
  输出参数  : 无
  返 回 值  : 1:需要snooping处理;0:不需要snooping处理
...
******************************************************************************/
static int is_igmp_need_process(const struct sk_buff *skb)
{
	...
	/*只检查博通加速需要处理的几种类型*/
	if (0 == is_igmp_type_need_process(pstRepV3->type))
	{
		return 0;
	}
	...
	{
		return is_igmp_v3_valid(pstRepV3, pip);
	}
                       ...
                       int is_routed)
{
		...
		// (br->igmp_proxy || br->igmp_snooping))
	{
		...
		/*当前博通驱动未设置IFF_WANDEV，使用ATP的WAN判断条件DTS2013103006577*/
		...
		else
		{
			...
			{ 
#if 1
				/*codenomicon igmp测试挂死修改:博通snooping处理之前进行必要的合法性校验*/
	...
	hlist_for_each_entry(dst, h, head, hlist) {
		...
		if (dst->grp.s_addr == pip->daddr) {
			...
			if ( is_routed ) {
				...
				if (br_igmp_lan2lan_snooping)
				{
					//使能br_igmp_lan2lan_snooping 时  ，增加处理类型MCPD_IF_TYPE_UNKWN for DTS2013012910652
					if (dst->type != MCPD_IF_TYPE_BRIDGED && dst->type != MCPD_IF_TYPE_UNKWN) {
						continue;
					}
				}
				else
				{
					//去使能br_igmp_lan2lan_snooping时，只处理类型MCPD_IF_TYPE_BRIDGED for DTS2013012910652
...
/*start 插拔以太网线概率性死机DTS2012080401932*/
            dev_hold(new_igmp_fdb->from_dev);
/*end 插拔以太网线概率性死机DTS2012080401932*/
		}
		else
		{
...
/*start 插拔以太网线概率性死机DTS2012080401932*/
    if (igmp_fdb->from_dev)
    {
        dev_put(igmp_fdb->from_dev);
    }
/*end 插拔以太网线概率性死机DTS2012080401932*/
