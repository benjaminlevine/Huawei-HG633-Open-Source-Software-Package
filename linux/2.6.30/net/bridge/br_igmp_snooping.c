...
{
    /*start for igmp snooping cpu 0 by m00132836 2012-11-24*/
	struct net_bridge_igmp_snooping_entry *dst_entry, *tmp_entry;
	struct net_bridge_igmp_snooping *igmp_list;

	igmp_list = (struct net_bridge_igmp_snooping *) ptr;

	spin_lock_bh(&igmp_list->igmp_lock);
	list_for_each_entry_safe(dst_entry, tmp_entry, &igmp_list->igmp_list, list) 
    /*end for igmp snooping cpu 0 by m00132836 2012-11-24*/
    {
...
int br_igmp_snooping_del_by_nbp(struct net_bridge_port *dev_port)
{
	...
	spin_lock_bh(&igmp_snooping_list.igmp_lock);
	list_for_each_entry_safe(dst_entry, tmp_entry, &igmp_snooping_list.igmp_list, list)
    /*end for igmp snooping cpu 0 by m00132836 2012-11-24*/
	{
		if (dst_entry ->dev_dst == dev_port) 
		{
...
unsigned int br_igmp_snooping_no_v1v2report(const struct net_bridge_port *dev_port, struct sk_buff *skb)
{
	...
	list_for_each_entry_rcu(dst_entry, &igmp_snooping_list.igmp_list, list) 
	{
        ...
        {
            /* 已经加入组的report报文允许向wan侧转发，否则会导致断流 */
			break;
        }
        /* END:   Added by z67728, 2010/7/13 */
	}
...
static int is_igmp_v3_valid(struct igmpv3_report *pstRepV3, const struct iphdr *pip)
{
	...
	int avail_len = ntohs(pip->tot_len) - (pip->ihl << 2) - sizeof(struct igmpv3_report);
	...
	cur_group_num = 1;
	while (cur_group_num <= ntohs(pstRepV3->ngrec))
	{
		/*group整体长度*/
		avail_len -= (ntohs(pstGrec->grec_nsrcs) * sizeof(struct in_addr));
		if (avail_len < 0)
		{
			return 0;
		}
		...
		/*还有group需要处理*/
		if (cur_group_num < ntohs(pstRepV3->ngrec))
		{
			/*剩余长度不得少于group 头长度*/
			avail_len -= sizeof(struct igmpv3_grec);
			if (avail_len < 0)
...
int br_igmp_snooping_forward(struct sk_buff *skb, struct net_bridge *br,unsigned char *dest,int forward)
{
    ...
    if (data[9] == IPPROTO_IGMP) 
    {
        ...
        {   
        	/*start added by l00184769 IGMP SSM for DTS2013072203563*/
#ifdef CONFIG_IGMP_SSM
        	pstRepIgmp = (struct igmphdr *)&data[usIpHeadLen];
		    if (br_igmpv12_ssm_validate_grec(pstRepIgmp->group))
		    {
		    	return 0;
		    }
#endif
			/*end added by l00184769 IGMP SSM for DTS2013072203563*/
            ...
            protocol == __constant_htons(ETH_P_IP))
        {
            ...
            pstGrec = &pstRepV3->grec[0];
            /*start for igmp snooping cpu 0 by m00132836 2012-11-24*/
            int kloop = ntohs(pstRepV3->ngrec);
            for (k = 0; k < kloop; k++)
            /*end for igmp snooping cpu 0 by m00132836 2012-11-24*/
            {
            	/*start added by l00184769 IGMP SSM for DTS2013072203563*/
                    ...
                    (IGMPV3_ALLOW_NEW_SOURCES == pstGrec->grec_type)) 
                {
                    ...
                    if (!br_igmp_snooping_empty(mc_mac))
        		    {
            		    ...
            		    /* END:   Added by z67728, 2010/10/29 */
                        /*start of added by w00174830 20130419 IGMP V3 Report Packet could not be transport on bridge mode*/
                        ulSendv3ReportFlag = BR_IGMP_SNOOPING_SEND_V3_REPORT_TRUE;
                        /*end of added by w00174830 20130419 IGMP V3 Report Packet could not be transport on bridge mode*/
                        iCnt += 1;
        		    }
                    else
                    {
              ...
              && IS_WAN_DEVICE(skb->dev->name))
        {
            ...
            g_igmpquerylist.querytimer.igmp_timer.expires  = jiffies + IGMPV3_MRC(data[usIpHeadLen + 1])*(HZ/IGMP_TIMER_CHECK_TIMEOUT);
            //printk("expires after:%u,  expires before:%u\r\n", IGMPV3_MRC(data[usIpHeadLen + 1])*(HZ/IGMP_TIMER_CHECK_TIMEOUT),  (data[usIpHeadLen + 1])*(HZ/IGMP_TIMER_CHECK_TIMEOUT));
