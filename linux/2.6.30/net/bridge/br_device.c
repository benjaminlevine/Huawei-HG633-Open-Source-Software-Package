...
#else
int hi_kernel_br_find_mac(struct net_device *dev, unsigned char *addr)
#endif
{
	struct net_bridge_fdb_entry *dst; 

    if((NULL == dev) || (NULL == dev->br_port)|| (NULL == dev->br_port->br)|| (NULL == addr))
    {
        return -1; 
    }
    
    dst = br_fdb_get(dev->br_port->br, addr); 
    if ((NULL == dst) || (dst->is_local))
    {
        if(NULL != dst)
        {
...
#else
EXPORT_SYMBOL(hi_kernel_br_find_mac); 
#endif

#ifdef CONFIG_BUILD_SD5610T  //fix me to CONFIG_HSAN future
struct net_bridge_fdb_entry * hi_br_find_fdb(struct net_device *dev, unsigned char *addr)
#else
struct net_bridge_fdb_entry * hi_kernel_br_find_fdb(struct net_device *dev, unsigned char *addr)
#endif
{
	struct net_bridge_fdb_entry *dst; 
...
#else
EXPORT_SYMBOL(hi_kernel_br_find_fdb); 
#endif

#ifdef CONFIG_BUILD_SD5610T  //fix me to CONFIG_HSAN future
void hi_br_fdb_put(struct net_bridge_fdb_entry *ent)
#else
void hi_kernel_br_fdb_put(struct net_bridge_fdb_entry *ent)
#endif
{
    br_fdb_put(ent);
}
#ifdef CONFIG_BUILD_SD5610T  //fix me to CONFIG_HSAN future
EXPORT_SYMBOL(hi_br_fdb_put); 
#else
EXPORT_SYMBOL(hi_kernel_br_fdb_put); 
#endif
#endif
