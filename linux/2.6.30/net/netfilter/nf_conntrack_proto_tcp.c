...
__be16 g_sH323H245Port = 0;/* network order */
#endif

/* Protects ct->proto.tcp */
static DEFINE_RWLOCK(tcp_lock);

#if (defined CONFIG_HSAN) || defined(CONFIG_MIPS_BRCM)
...
#if defined(CONFIG_BCM96362)/* close h323 conntrack quickly */
	else if (TCP_CONNTRACK_CLOSE == new_state)
	{
		if ((ct->tuplehash[IP_CT_DIR_ORIGINAL].tuple.dst.u.tcp.port == htons(Q931_PORT)) ||
			(ct->tuplehash[IP_CT_DIR_ORIGINAL].tuple.dst.u.tcp.port == g_sH323H245Port))
		{
			timeout = 1;    
		}
	}
