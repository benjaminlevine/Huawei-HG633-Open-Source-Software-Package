...
mark_tg_v1(struct sk_buff *skb, const struct xt_target_param *par)
{
	...
	case XT_MARK_SET:
		/* Start of modified by f00120964 for qos tr069 8021p problem 2012-4-19 */
        if (markinfo->mark & 0x000ffff0)
        {
    		mark = (skb->mark & (~0x000ffff0)) | markinfo->mark;
        }
        /* End of modified by f00120964 for qos tr069 8021p problem 2012-4-19 */
        #ifdef CONFIG_DT_QOS
        else if  (markinfo->mark & 0x000000ff)
        {
    		mark = (skb->mark & (~0x000000ff)) | markinfo->mark;
        }
        #endif /* CONFIG_DT_QOS */
        else
        {
