...
//skb_dequeue_tpj为规避组播加速死机问题，暂时新添函数，原函数为skb_dequeue，如果这问题彻底解决了可以恢复
//目前，问题的根因没有找到，现象是mfc_un.unres.unresolved双向链表被其他地方破坏
...
int ip_mr_input(struct sk_buff *skb)
{
	...
	read_lock(&mrt_lock);
	cache = ipmr_cache_find2(net, ip_hdr(skb)->saddr, ip_hdr(skb)->daddr, skb->dev);
		   ...
		   struct sk_buff *skb, struct rtmsg *rtm, int nowait)
{
	...
	read_lock(&mrt_lock);
	cache = ipmr_cache_find2(net, rt->rt_src, rt->rt_dst, skb->dev);
