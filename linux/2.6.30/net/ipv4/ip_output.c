			  ...
			  __be32 saddr, __be32 daddr, struct ip_options *opt)
{
	...
	skb->mark = sk->sk_mark;
	#endif /* CONFIG_DT_QOS */
	/*Start of added by f00120964 for qos function 2012-1-6 */
	...
	if (sk->sk_mark & 0xff000000) {
		skb->mark |= (sk->sk_mark & 0xff000000);
	}

        /* DT g00121640 add for extra mark like tr069 */
        if (sk->sk_mark & 0x0000ff00) {
            skb->mark |= (sk->sk_mark & 0x0000ff00);
        }
        /* DT g00121640 end */
	#endif
	/*End of added by f00120964 for qos function 2012-1-6 */
...
#endif
    /*来自DSLITE隧道的报文不进行V4分片by d00191326*/
...
struct sk_buff_head g_fwd_tcp_ack_queue;
struct timer_list g_tcp_waittimer;

static void ip_forward_queue()
{
    struct sk_buff *skb = NULL;

    while ((skb = skb_dequeue(&g_fwd_tcp_ack_queue)) != NULL)
    {
		dst_output(skb);
    }
}

void ip_forward_queue_run_timer(unsigned long data)
{
    init_timer(&g_tcp_waittimer);        /*reset timer */
    g_tcp_waittimer.expires = jiffies + 1; 
    g_tcp_waittimer.function = ip_forward_queue_run_timer;
    add_timer(&g_tcp_waittimer);
    ip_forward_queue();
}
#endif
...
#ifdef CONFIG_TCP_GATHER_ACK
    skb_queue_head_init(&g_fwd_tcp_ack_queue);
    ip_forward_queue_run_timer(0);
#endif
