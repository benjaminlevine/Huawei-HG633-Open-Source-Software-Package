...
#endif
/* START of add  for HG526v3 强制门户 by z00195436 at 2013.1.4*/
#ifdef CONFIG_ATP_PRSITE_MODULE
int (*prsite_add_hook)(char __user *optval) = NULL;
int (*prsite_del_hook)(char __user *optval) = NULL;
int (*prsite_modi_hook)(char __user *optval) = NULL;

int (*prsite_dump_hook)(char __user *optval) = NULL;
int (*prsite_init_hook)(char __user *optval) = NULL;

EXPORT_SYMBOL(prsite_add_hook);
EXPORT_SYMBOL(prsite_del_hook);
EXPORT_SYMBOL(prsite_modi_hook);
EXPORT_SYMBOL(prsite_dump_hook);
EXPORT_SYMBOL(prsite_init_hook);
#endif
/* END of add  for HG526v3 强制门户 by z00195436 at 2013.1.4*/

char g_acWanBind[8][128] = {{0}};

#ifdef CONFIG_TAG_NOVLAN_IPTV_PACKETS
u16 g_Vlan_tci = 0;
EXPORT_SYMBOL(g_Vlan_tci);
#endif

#ifdef CONFIG_CT_QOS
char g_acCTVlanInfo[8][128] = {{0}};
EXPORT_SYMBOL(g_acCTVlanInfo);
#endif
/*
...
void ip_cmsg_recv(struct msghdr *msg, struct sk_buff *skb)
{
    ...
    /*Start of ATP 2013-3-12 for get original dev by d00191326*/
    if ((flags>>=1) == 0)
		return;
	if (flags & 1)
		ip_cmsg_recv_indev(msg, skb);
    /*End of ATP 2013-3-12 for get original dev by d00191326*/
}
			    ...
			    int optname, char __user *optval, int optlen)
{
	...
	case MCAST_SSM_ADD:
	    ...
	    err = ip_mc_add_ssm((__be32)val);
#endif
    	break;
	case MCAST_SSM_DEL:
#ifdef CONFIG_IGMP_SSM
	    err = ip_mc_del_ssm((__be32)val);
#endif
    	break;
	case MCAST_ASM_ADD:
#ifdef CONFIG_IGMP_SSM
	    err = ip_mc_add_asm((__be32)val);
#endif
    	break;
	case MCAST_ASM_DEL:
#ifdef CONFIG_IGMP_SSM
	    err = ip_mc_del_asm((__be32)val);
#endif
    	break;
...
/* START of add  for HG526v3 强制门户 by z00195436 at 2013.1.4*/
    ...
    case PRSITE_INFO_ADD:
    {
        if (prsite_init_hook)
        {
            if((*prsite_init_hook)(optval))
            {
                release_sock(sk);
                return -EFAULT;
            }  
...
/* END of add  for HG526v3 强制门户 by z00195436 at 2013.1.4*/
    ...
    /*从用户态传入WAN的绑定关系*/
    case WAN_BIND_BRIDGE:
        if (copy_from_user(g_acWanBind, optval, sizeof(g_acWanBind))) {
			release_sock(sk);
			return -EFAULT;
		}
        break;
...
#ifdef CONFIG_CT_QOS
    case CTQOS_ADSL_VLAN:
        if (copy_from_user(g_acCTVlanInfo, optval, sizeof(g_acCTVlanInfo))) {
			release_sock(sk);
			return -EFAULT;
		}
        break;
...
#if defined(CONFIG_SIPALG_RECOREDDSTIP)
EXPORT_SYMBOL(g_ulSipPortInfo);
EXPORT_SYMBOL(g_ulSiproxdStatus);
EXPORT_SYMBOL(g_SipRouteListCount);
EXPORT_SYMBOL(UpdateSipRouteByCallid);
#endif
