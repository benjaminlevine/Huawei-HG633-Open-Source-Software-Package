...
#if 0
#define BCMNAT_DEBUG_MSG(fmt, arg...)  printk (KERN_DEBUG "BCMNAT[%u]: " fmt , __LINE__ , ## arg)
#else
#define BCMNAT_DEBUG_MSG(fmt, arg...) 
#endif

#if defined(CONFIG_NF_CONNTRACK_RTCP) || (defined(CONFIG_NF_CONNTRACK_RTCP_MODULE) && defined(CONFIG_MODULES))
#define FCC_RTCP_PORT  (8027)
#define IsFCCRTCPSKB(dir) (htons(FCC_RTCP_PORT) == ct->tuplehash[dir].tuple.dst.u.udp.port)
#else
#define IsFCCRTCPSKB(dir) (0)
#endif
			...
			struct nf_conn *ct, enum ip_conntrack_info ctinfo)
{
    ...
    if (IsFCCRTCPSKB(dir))
    {
        ...
        //exp->helper = &nf_conntrack_helper_bcm_nat;
    }
#endif
    ...
    {
        BCMNAT_DEBUG_MSG("bcm_nat: related core exp return NO-zero.\n");
    }
    else
    {
        BCMNAT_DEBUG_MSG("bcm_nat: related core exp ok.\n");
    }     
			...
			struct nf_conn *ct, enum ip_conntrack_info ctinfo)
{
    ...
    if (IsFCCRTCPSKB(dir))
    {
        ...
        if (nf_set_rtcp_help && ct->status & IPS_NAT_MASK)
        {
            ...
            BCMNAT_DEBUG_MSG("bcm_nat: nf_set_rtcp_help done.\n\n\n\n\n");         
        }
    }
...
static inline int find_exp(__be32 ip, __be16 port, struct nf_conn *ct)
{
	...
	rcu_read_lock();
	i = __nf_ct_expect_find(nf_ct_net(ct), &tuple);
	rcu_read_unlock();
...
static inline int fullcone_exp_match(struct nf_conntrack_expect * i, 
                    struct nf_conntrack_tuple * tp)
{
    return (i->saved_ip == tp->src.u3.ip &&
...
#if defined(CONFIG_NF_CONNTRACK_RTCP) || (defined(CONFIG_NF_CONNTRACK_RTCP_MODULE) && defined(CONFIG_MODULES)) 
static bool 
core_find_ct(struct nf_conn *ct, __be16 port, __be32 newsrc)
{
    struct nf_conntrack_tuple tuple;
    struct net *net;
    ...
    if (NULL != nf_conntrack_find_get(net, &tuple))
    {        
        BCMNAT_DEBUG_MSG("bcm_nat: find the same ct[ src[%d.%d.%d.%d] src_port[%d] -> dst[%d.%d.%d.%d] dst_port[%d] ]\n", 
            NIPQUAD(tuple.src.u3), ntohs(tuple.src.u.all),
            NIPQUAD(tuple.dst.u3), ntohs(tuple.dst.u.all));
        return true;
    }
...
masquerade_tg(struct sk_buff *skb, const struct xt_target_param *par)
{
    ...
    if (IsFCCRTCPSKB(IP_CT_DIR_ORIGINAL))
    {
        BCMNAT_DEBUG_MSG("bcm_nat: org src[%d.%d.%d.%d : %d] -> dst[%d.%d.%d.%d : %d]\n",
            NIPQUAD(ct->tuplehash[IP_CT_DIR_ORIGINAL].tuple.src.u3), ntohs(ct->tuplehash[IP_CT_DIR_ORIGINAL].tuple.src.u.all),
            NIPQUAD(ct->tuplehash[IP_CT_DIR_ORIGINAL].tuple.dst.u3), ntohs(ct->tuplehash[IP_CT_DIR_ORIGINAL].tuple.dst.u.all));

        BCMNAT_DEBUG_MSG("bcm_nat: rep src[%d.%d.%d.%d : %d] -> dst[%d.%d.%d.%d : %d]\n", 
            NIPQUAD(ct->tuplehash[IP_CT_DIR_REPLY].tuple.src.u3), ntohs(ct->tuplehash[IP_CT_DIR_REPLY].tuple.src.u.all),
            NIPQUAD(ct->tuplehash[IP_CT_DIR_REPLY].tuple.dst.u3), ntohs(ct->tuplehash[IP_CT_DIR_REPLY].tuple.dst.u.all));
    }
...
#if defined(CONFIG_MIPS_BRCM) || defined(CONFIG_HSAN)
/*这里默认fullcone 的标记min_ip为1，不去判断 mr->range[0].min_ip != 0  nat_mode == full cone */ 
	if ((nfct_help(ct) == NULL || nfct_help(ct)->helper == NULL)
		...
		if (exp) {
            ...
            if (IsFCCRTCPSKB(IP_CT_DIR_ORIGINAL))
            {
    			BCMNAT_DEBUG_MSG("bcm_nat: existing mapped port = %hu\n", ntohs(minport));
            }            
            ...
            if (IsFCCRTCPSKB(IP_CT_DIR_ORIGINAL))    
            {
    			for (newport = ntohs(minport),tmpport = ntohs(maxport); 
    			     newport <= tmpport; newport += 4) {
			     	if ((!find_exp(newsrc, htons(newport), ct))
                        && (!find_exp(newsrc, htons(newport - 1), ct))
                        && (!core_find_ct(ct, htons(newport), newsrc))) {
    					pr_debug("bcm_nat: new mapped port = "
    					       	 "%hu\n", newport);
                        BCMNAT_DEBUG_MSG("bcm_nat: new mapped port = "
    					       	 "%hu\n", newport);
