...
#ifdef CONFIG_HSAN
hi_nf_conntrack_confirm_hook pf_hi_nf_conntrack_confirm_hook;

unsigned int hi_nf_conntrack_register_confirm_hook(hi_nf_conntrack_confirm_hook hook)
{
    pf_hi_nf_conntrack_confirm_hook = hook;
    return 0;
}

unsigned int hi_nf_conntrack_unregister_confirm_hook(void)
{
    pf_hi_nf_conntrack_confirm_hook = NULL;
    return 0;
}

EXPORT_SYMBOL(hi_nf_conntrack_register_confirm_hook);
EXPORT_SYMBOL(hi_nf_conntrack_unregister_confirm_hook);
#endif
...
out:
	...
	if (NULL != pf_hi_nf_conntrack_confirm_hook)
	{
        pf_hi_nf_conntrack_confirm_hook(hooknum, skb, in, out, okfn, ret);
	}
...
/* start of add by d00107688 2009-04-15 to support alg switch */
...
/* end of add by d00107688 2009-04-15 to support alg switch */
...
#ifdef CONFIG_NF_CONNTRACK_H323
        {
            .ctl_name   = NET_IPV4_NF_ALG_H323_SWITCH,
...
#ifdef CONFIG_NF_CONNTRACK_RTSP
        {
            .ctl_name   = NET_IPV4_NF_ALG_RTSP_SWITCH,
...
#if defined(CONFIG_NF_CONNTRACK_SIP) || defined(CONFIG_NF_CONNTRACK_SIP_MODULE)
        {
            .ctl_name   = NET_IPV4_NF_ALG_SIP_SWITCH,
...
#ifdef CONFIG_NF_CONNTRACK_PPTP
        {
            .ctl_name   = NET_IPV4_NF_ALG_PPTP_SWITCH,
...
#ifdef CONFIG_NF_CONNTRACK_IPSEC
        {
            .ctl_name   = NET_IPV4_NF_ALG_IPSEC_SWITCH,
...
#ifdef CONFIG_NF_CONNTRACK_FTP
        {
            .ctl_name   = NET_IPV4_NF_ALG_FTP_SWITCH,
...
/* end of add by d00107688 2009-04-15 to support alg switch */
...
#ifdef CONFIG_FIREWALL_LOG
        {
	        .ctl_name   = NET_IPV4_NF_FIREWALL_LOG_SWITCH,
            .procname   = "log_limit",
            .data       = &log_limit,
            .maxlen     = sizeof(int),
            .mode       = 0644,
    	...
    	{
    		.ctl_name	= NET_IPV4_NF_CT_RESERVE_CNT_MIN,
    		.procname	= "ip_ct_reserve_cnt_min",
    		.data		= &nf_reserve_ct_min,
    		.maxlen		= sizeof(int),
    		.mode		= 0644,
    		.proc_handler	= proc_dointvec,
    	},
        {
            .ctl_name   = NET_IPV4_NF_CT_RESERVE_COUNT,
            .procname   = "ip_conntrack_reserve_count",
            .data       = &init_net.ct.reserve_cnt,
            .maxlen     = sizeof(int),
            .mode       = 0644,
            .proc_handler   = proc_dointvec,
        },
...
#if defined(CONFIG_IMQ) || defined(CONFIG_IMQ_MODULE)
        {
            .ctl_name   = NET_IPV4_NF_DOWNQOS_SWITCH,
...
/* Add by l00171355 for per-flow bonding @ 2013-07-23 */
#ifdef  CONFIG_ARCH_SD56XX
        {
            .ctl_name   = NET_IPV4_NF_DSL_THRESHOLD,
        ...
        {
            .ctl_name   = NET_IPV4_NF_DSL_THRESHOLD_FLAG,
...
#ifdef CONFIG_DPI_PARSE
            {
                .ctl_name   = NET_IPV4_NF_SMARTQOS_SWITCH,
